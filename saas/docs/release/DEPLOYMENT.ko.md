...
후보는 **0.5.0-rc.1, 중앙 migrations 1–25와 HOT schema 1**을 요구한다. 2026-09-10 15:20 UTC 기록에서 production `memory.allenlabs.org`는 중앙 1–25/HOT 1 이관을 확인했으며 rc.3 Worker의 교체는 대기 중이었다. Staging `memory-staging.allenlabs.org`는 `f1cb578`/`7efa79bf`, 중앙 1–25/HOT 1과 1분 cron을 사용한다. 기존 개인용 Worker와 DB는 별도 서비스다. 이후 배포 상태와 관찰 시각, 검증 범위는 [날짜별 배포 기록](INTEGRATION.md#migration-record--2026-09-10-1520-utc)에서 확인한다.
...
| 중앙 D1 | `DB`, `migrations_dir: migrations`. 새 환경은 0001–0025 전부 적용. 기존 환경은 실제 ledger를 대조해 누락된 migration만 적용하며 HOT을 먼저 처리. 15:20 UTC 기록에서 production/staging 모두 중앙 0001–0025 적용 완료 |
...
| 메일 | 네이티브 `EMAIL`, `MAIL_FROM`, 등록된 발신 도메인. 최종 staging의 실제 수신·동일 session proof 소비와 잘못된 session/재사용 거절, 별도 자원 정리 확인 완료 |
| Identity lifecycle | Memory `IDENTITY_WEBHOOK_SECRET`와 중앙의 해당 환경 delivery secret. 중앙 `5590836`/`bbb66ddb` 발행기의 staging 전달은 확인됨. Production 대상 활성화는 별도 미완료 단계 |
...
1. revision·dirty 상태·대상 config를 확인하고 `npm ci`, `npm run check`, `npm run test:d1`을 실행한다. 실행별 source와 검증 범위는 통합 기록에 남긴다.
...
중앙 lifecycle은 private `5590836`/migration 0010으로 배포되었고 1분 cron이 staging에 전달한다. 실제 synthetic 검증은 순서 있는 event 6개와 지연·재전송·회수/재개를 포함한다. Production 대상은 해당 receiver 배포 확인 뒤 별도로 활성화하고 검증한다. 환경별 secret을 나누고 대상은 정확히 production 및 hyphenated staging origin만 허용한다. 이 기록은 실사용자 브라우저 SSO나 모든 대상의 전달 완료 증명이 아니다.
