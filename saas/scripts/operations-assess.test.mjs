...
async function setup(t) {
  ...
  const snapshot = async () => ({ format: 1, environment: 'staging', resourceFingerprint: deploymentFingerprint(selected.config), collectedAt: at,
    database: { observedAt: at, month: '2026-09', results: await collect() },
...
test('query plan executes against actual central/HOT schemas and all collection queries remain read-only/bounded', async t => {
  const f = await setup(t); const results = await f.collect();
  assert.equal(Object.keys(results).length, f.plan.d1.length);
  for (const q of f.plan.d1) { assert.match(q.sql, /^SELECT /); assert.match(q.sql, /LIMIT \d+/); assert.doesNotMatch(q.sql, /SELECT \*|token_digest|space_id|memory_id|last_error AS/i); await f.db.raw.prepare(q.binding === 'DB' ? 'EXPLAIN ' + q.sql : 'SELECT 1').all(...(q.binding === 'DB' ? q.params : [])); }
  const queue = f.plan.d1.find(q => q.id === 'jobs-pending-upsert-0');
  const explain = JSON.stringify(await f.db.raw.prepare('EXPLAIN ' + queue.sql).all(...queue.params)); assert.match(explain, /release_jobs_pending_claim/); assert.doesNotMatch(explain, /Seq Scan on \S*release_jobs|Sort/);
...
test('healthy complete actual captures produce exit0 without inventing alert delivery or hard budget cap', async t => {
  const f = await setup(t), report = assessOperations(f.selected, await f.snapshot(), { now: at });
...
for (const [metric, field] of [['heartbeat', 'lastSuccessAt'], ['payload-backfill', 'updatedAt'], ['payload-backfill', 'lastErrorAt']]) test(`historical ${field} cannot follow its database observation even when assessment is later`, async t => {
  const f = await setup(t); f.selected.config.vars.STORAGE_BACKFILL_ENABLED = 'true'; const s = await f.snapshot();
...
for (const key of ['toString', 'constructor', '__proto__']) test(`threshold policy rejects inherited Object key ${key}`, async t => {
  const f = await setup(t), policy = JSON.parse('{"' + key + '":1}'), s = await f.snapshot();
  assert.throws(() => assessOperations(f.selected, s, { now: at, policy }), /Unknown threshold policy/);
});
...
]) test(`assessment flags ${name}`, async t => { const f = await setup(t), s = await f.snapshot(); mutate(s); const report = assessOperations(f.selected, s, { now: at }); assert.equal(report.exitCode, 2); assert.ok(report.issues.some(i => i.code === code), JSON.stringify(report)); });
test('old/retry/dead provider jobs are observed without disclosing retained errors or identifiers', async t => {
  const f = await setup(t);
  await f.db.setClock(() => at - 8000000);
  await f.db.raw.prepare("INSERT INTO release_jobs(id,kind,state,attempt,available_at,created_at,last_error) VALUES('private-job','upsert','pending',4,?,?,?)").run(at - 8000000, at - 8000000, 'https://secret.example/?token=PRIVATE');
  await f.db.setClock(() => at);
  await f.db.raw.prepare("INSERT INTO release_jobs(id,kind,state,attempt,available_at,created_at) VALUES('dead-job','ingest','dead',5,?,?)").run(at, at);
  const report = assessOperations(f.selected, await f.snapshot(), { now: at });
...
test('queue-episode trigger stamps inserts and done/dead restarts, preserving mid-episode retries', async t => {
  // The PostgreSQL lineage carries the episode stamp natively (migration
  // 0015): no upgraded-legacy ambiguity exists because every INSERT and every
  // done/dead→pending transition is stamped by the row trigger.
  let now = at; const db = new DB(() => now); await db.migrate(); t.after(() => db.close());
  const insert = db.raw.prepare('INSERT INTO release_jobs(id,kind,state,attempt,available_at,created_at) VALUES(?,?,?,?,?,?)');
  await insert.run('initial', 'upsert', 'pending', 0, at - 10000, at - 10000);
  await insert.run('completed', 'delete', 'done', 0, at - 100, at - 10000);
  // An explicit queued_at (seeded history) survives the INSERT trigger.
  await db.raw.prepare("INSERT INTO release_jobs(id,kind,state,attempt,available_at,created_at,queued_at) VALUES('seeded','delete','pending',1,?, ?, ?)").run(at - 5000, at - 10000, at - 9000);
  assert.deepEqual((await db.raw.prepare('SELECT id,created_at,queued_at FROM release_jobs ORDER BY id').all()).map(r => ({ ...r })), [
    { id: 'completed', created_at: at - 10000, queued_at: now }, { id: 'initial', created_at: at - 10000, queued_at: now }, { id: 'seeded', created_at: at - 10000, queued_at: at - 9000 }]);
  now += 1000; await db.raw.prepare("UPDATE release_jobs SET state='pending',available_at=? WHERE id='completed'").run(now);
  assert.equal((await db.raw.prepare("SELECT queued_at FROM release_jobs WHERE id='completed'").get()).queued_at, now);
  const start = now; now += 1000; await db.raw.prepare("UPDATE release_jobs SET state='leased',attempt=1,lease_until=? WHERE id='completed'").run(now + 120000);
  now += 1000; await db.raw.prepare("UPDATE release_jobs SET state='pending',attempt=2,available_at=? WHERE id='completed'").run(now + 10000);
  assert.equal((await db.raw.prepare("SELECT queued_at FROM release_jobs WHERE id='completed'").get()).queued_at, start);
  await db.raw.exec("UPDATE release_jobs SET state='dead' WHERE id='completed'"); now += 1000; await db.raw.exec("UPDATE release_jobs SET state='pending' WHERE id='completed'");
  assert.equal((await db.raw.prepare("SELECT queued_at FROM release_jobs WHERE id='completed'").get()).queued_at, now);
  // A column-level episode reset (legacy unknown) is not overwritten by
  // non-state updates.
...
test('routine completed-delete reconciliation starts a fresh episode and retries preserve its real age', async t => {
  ...
  await store.remove('a'.repeat(64), 's1', memory.id, 1, 'episode-remove'); await f.db.raw.prepare("UPDATE release_jobs SET state='done',available_at=?").run(at);
  let now = at + 2 * 86400000; await f.db.setClock(() => now);
  const report = async () => { const s = await f.snapshot(); s.collectedAt = now; for (const k of ['database','capacity','traffic','budget']) s[k].observedAt = now; s.database.results.heartbeat[0].lastSuccessAt = now; return assessOperations(f.selected, s, { now }); };
  assert.equal((await report()).exitCode, 0);
  const jobs = new Jobs({ DB: f.db, BACKGROUND_JOBS_ENABLED: 'true', MEMORY_INDEX: {} }, () => now); await jobs.maintain();
  const fresh = await report(); assert.equal(fresh.exitCode, 0, JSON.stringify(fresh.issues)); assert.equal(fresh.queues.find(q => q.metric === 'jobs-pending-delete-0').oldestObservedAgeSeconds, 0);
  await f.db.raw.prepare('INSERT INTO release_vector_refs(memory_id,vector_id,revision) VALUES(?,?,2)').run(memory.id, 'pending-provider-delete');
  now += 3601000; await jobs.drain(1);
  const delayed = await report(); assert.ok(delayed.issues.some(i => i.code === 'provider_job_age_critical')); assert.equal(delayed.queues.find(q => q.metric === 'jobs-pending-delete-0').oldestObservedAgeSeconds, 3601);
});
test('positive one-microusd stop passes configuration checks and rejects both current AI reservations', async t => {
  ...
  assert.equal((await f.db.raw.prepare('SELECT count(*) AS n FROM release_provider_budgets').get()).n, 0);
  const report = assessOperations(f.selected, await f.snapshot(), { now: at }); assert.ok(report.issues.some(i => i.code === 'ai_provider_admission_stopped')); assert.equal(report.exitCode, 2);
  f.selected.config.vars.AI_MONTHLY_BUDGET_MICROUSD = '0'; assert.ok(assessOperations(f.selected, await f.snapshot(), { now: at }).issues.some(i => i.code === 'ai_budget_configuration'));
});
test('queue sampling is bounded and saturation is actionable instead of a false exact count', async t => {
  ...
  await f.db.raw.exec('BEGIN'); for (let i = 0; i < 2000; i++) await stmt.run(`j${i}`, at, at); await f.db.raw.exec('COMMIT');
  const s = await f.snapshot(); assert.equal(s.database.results['jobs-pending-upsert-0'].length, 1001);
...
test('AI reservation reports conservative reserved cost separately from billed Cloudflare costs', async t => {
  const f = await setup(t); await f.db.raw.exec("INSERT INTO release_provider_budgets VALUES('2026-09','extraction',6,180000)");
  const report = assessOperations(f.selected, await f.snapshot(), { now: at }); assert.equal(report.ai.reservedUsd, 0.18); assert.equal(report.ai.configuredLimitUsd, 0.2); assert.equal(report.budget.totalUsd, 0.07); assert.ok(report.issues.some(i => i.code === 'ai_reservation_near_limit'));
});
test('disabled backfill with no candidates is valid; enabled failed backfill is actionable', async t => {
  const f = await setup(t); f.selected.config.vars.STORAGE_BACKFILL_ENABLED = 'true'; await f.db.raw.exec("UPDATE release_payload_backfill_progress SET last_error='PRIVATE',last_error_at=1 WHERE kind='current'");
  const report = assessOperations(f.selected, await f.snapshot(), { now: at }); assert.ok(report.issues.some(i => i.code === 'backfill_failed')); assert.doesNotMatch(JSON.stringify(report), /PRIVATE/);
});
test('expired intents and failed pending payload purges/retirements are indexed and actionable', async t => {
  ...
  await intent.run('future', 'future', at - 10000000, at + 10000); await intent.run('expired', 'expired', at - 10000000, at - 2000000);
  await f.db.raw.prepare("INSERT INTO release_payload_stages(id,intent_id,ordinal,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,logical_bytes,created_at) VALUES('p1','future',0,'private-memory','a','payload/v1/p1',?,64,64,?)").run('a'.repeat(64), at);
  for (const table of ['purges', 'retirements']) await f.db.raw.prepare(`INSERT INTO release_payload_${table}(payload_id,space_id,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,created_at,attempts,last_error) VALUES('p1','s1','private-memory','a','payload/v1/p1',?,64,?,4,'PRIVATE ERROR')`).run('a'.repeat(64), at - 8000000);
  const report = assessOperations(f.selected, await f.snapshot(), { now: at });
  ...
  for (const id of ['payload-intents', 'payload-purges', 'payload-retirements']) { const q = f.plan.d1.find(q => q.id === id), explain = JSON.stringify(await f.db.raw.prepare('EXPLAIN ' + q.sql).all()); assert.match(explain, /Index Scan|Index Only Scan|Bitmap/); assert.doesNotMatch(explain, /Seq Scan on \S*(payload_intents|payload_purges|payload_retirements)/); }
});
test('missing query results are critical unknowns, while tombstone samples remain explicit lower bounds', async t => {
  const f = await setup(t), s = await f.snapshot(); delete s.database.results['ai-budget']; s.database.results['tombstones-HOT_A'] = [{ count: 1001 }];
...
test('ambiguous legacy queue start stays unknown instead of reporting a fabricated backlog age', async t => {
  const f = await setup(t); await f.db.raw.prepare("INSERT INTO release_jobs(id,kind,state,available_at,created_at) VALUES('legacy','delete','pending',?,?)").run(at, at - 172800000);
  await f.db.raw.exec("UPDATE release_jobs SET queued_at=NULL WHERE id='legacy'");
  const report = assessOperations(f.selected, await f.snapshot(), { now: at }); assert.ok(report.issues.some(i => i.code === 'queue_episode_unknown'));
...
for (const collectionDelay of [0, 1000]) test(`future queue episode is rejected even when assessment arrives ${collectionDelay}ms later`, async t => {
  const f = await setup(t);
  await f.db.raw.prepare("INSERT INTO release_jobs(id,kind,state,available_at,created_at) VALUES('future-episode','delete','pending',?,?)").run(at, at);
  await f.db.raw.prepare("UPDATE release_jobs SET queued_at=? WHERE id='future-episode'").run(at + 1);
  const s = await f.snapshot(); s.collectedAt += collectionDelay;
...
test('payload intent creation cannot occur after the database observation', async t => {
  const f = await setup(t);
  await f.db.raw.prepare("INSERT INTO release_payload_intents(id,account_id,space_id,client_key,request_hash,action,item_count,reserved_bytes,created_at,expires_at) VALUES('future-intent','alice','s1','future-intent','hash','create',1,128,?,?)").run(at + 1, at + 10000);
  const s = await f.snapshot();
  assert.throws(() => assessOperations(f.selected, s, { now: at }), /future.*queue|queue.*future/i);
});
...
test('dead and exhausted queries preserve sanitized failure counts without retained error text', async t => {
  ...
  await insert.run('dead-with-error', 'delete', 'dead', 5, at, at, null, 'PRIVATE failure URL');
  await insert.run('dead-without-error', 'delete', 'dead', 5, at, at, null, null);
  await insert.run('exhausted-with-error', 'delete', 'leased', 5, at, at, at + 1000, 'PRIVATE prior failure');
  const snapshot = await f.snapshot(), report = assessOperations(f.selected, snapshot, { now: at });
...
test('the first actual provider lease is not a retry; failed pending and second lease are retries', async t => {
  const f = await setup(t);
  await f.db.raw.prepare("INSERT INTO release_jobs(id,kind,state,available_at,created_at) VALUES('first-lease','delete','pending',?,?)").run(at, at);
  const jobs = new Jobs({ DB: f.db, BACKGROUND_JOBS_ENABLED: 'true', MEMORY_INDEX: {} }, () => at);
  assert.equal((await jobs.claim()).attempt, 1);
  let report = assessOperations(f.selected, await f.snapshot(), { now: at });
  assert.equal(report.queues.find(q => q.metric === 'jobs-leased-delete-0').retriesObserved, 0);
  await f.db.raw.prepare("UPDATE release_jobs SET state='pending',lease_until=NULL,lease_token=NULL,last_error='provider_or_processing_failure',available_at=? WHERE id='first-lease'").run(at);
  report = assessOperations(f.selected, await f.snapshot(), { now: at });
...
test('intent queues do not invent retry or failure counters that their schema cannot supply', async t => {
  const f = await setup(t);
  await f.db.raw.prepare("INSERT INTO release_payload_intents(id,account_id,space_id,client_key,request_hash,action,item_count,reserved_bytes,created_at,expires_at) VALUES('intent-counters','alice','s1','intent-counters','hash','create',1,128,?,?)").run(at, at + 10000);
  const q = assessOperations(f.selected, await f.snapshot(), { now: at }).queues.find(q => q.metric === 'payload-intents');
...
for (const [attempt, error, expected] of [[1, null, 0], [1, 'provider_failure', 1], [2, null, 1]]) test(`payload cleanup attempt${attempt} error${Boolean(error)} has ${expected} observed retry`, async t => {
  const f = await setup(t);
  await f.db.raw.prepare("INSERT INTO release_payload_intents(id,account_id,space_id,client_key,request_hash,action,item_count,reserved_bytes,created_at,expires_at) VALUES('cleanup-intent','alice','s1','cleanup-intent','hash','create',1,128,?,?)").run(at, at + 10000);
  await f.db.raw.prepare("INSERT INTO release_payload_stages(id,intent_id,ordinal,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,logical_bytes,created_at) VALUES('cleanup-payload','cleanup-intent',0,'private-memory','a','payload/v1/cleanup',?,64,64,?)").run('a'.repeat(64), at);
  await f.db.raw.prepare("INSERT INTO release_payload_purges(payload_id,space_id,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,created_at,available_at,attempts,last_error) VALUES('cleanup-payload','s1','private-memory','a','payload/v1/cleanup',?,64,?,?,?,?)").run('a'.repeat(64), at, at + 60000, attempt, error);
  const q = assessOperations(f.selected, await f.snapshot(), { now: at }).queues.find(q => q.metric === 'payload-purges');
...
test('empty traffic with independently zero HTTP requests is valid and overrides cannot raise the USD50 ceiling', async t => {
  const f = await setup(t), s = await f.snapshot(); Object.assign(s.traffic, { requests: 0, primaryRequests: 0, p95Ms: null }); assert.equal(assessOperations(f.selected, s, { now: at }).exitCode, 0);
...
test('reported provider byte limits accept 10GiB paid capacity and a smaller free-plan limit', async t => {
  const f = await setup(t), s = await f.snapshot(); s.capacity.databases[0].limitBytes = 10 * 1024 ** 3; s.capacity.databases[1].limitBytes = 500 * 1024 ** 2;
...
]) test(`invalid snapshot rejects ${name}`, async t => { const f = await setup(t), s = await f.snapshot(); mutate(s); assert.throws(() => assessOperations(f.selected, s, { now: at })); });
...
test('CLI uses the actual config and emits a redacted nonzero assessment for old actual captures', async t => {
  ...
  const configPath = join(dir, 'target.jsonc'), input = join(dir, 'snapshot.json'); await writeFile(configPath, JSON.stringify(f.selected.config)); await writeFile(input, JSON.stringify(await f.snapshot()));
...
test('CLI records unknown metrics with exit2 and installs output exclusively', async t => {
  ...
  const s = await f.snapshot(); s.budget = null; await writeFile(input, JSON.stringify(s));
