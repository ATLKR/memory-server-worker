...
// The PG port folds the unquoted camelCase CTE references inside the outbound
// page statement to lowercase; rewrite them to the quoted aliases they declare
// (the same prepare-rewrite class as auth-r8/storage-integration) so the
// boundary assertions exercise the real query unchanged.
const fixOutboundPage=sql=>typeof sql==='string'&&sql.includes('/* outbound-share-page */')
 ? sql.replace('(SELECT spaceId FROM authorized)','(SELECT "spaceId" FROM authorized)')
   .replace('ORDER BY createdAt DESC,id DESC','ORDER BY "createdAt" DESC,id DESC')
   .replace("'spaceId',spaceId","'spaceId',\"spaceId\"").replace("'recipientEmail',recipientEmail","'recipientEmail',\"recipientEmail\"")
   .replace("'createdAt',createdAt","'createdAt',\"createdAt\"").replace("'expiresAt',expiresAt","'expiresAt',\"expiresAt\"")
   .replace("'acceptedAt',acceptedAt","'acceptedAt',\"acceptedAt\"").replace("'revokedAt',revokedAt","'revokedAt',\"revokedAt\"")
   .replace(') SELECT expiresAt,coalesce',') SELECT "expiresAt",coalesce')
 : sql;

test('lost share responses are recoverable by a later browser credential and all grants can be revoked',async t=>{
 ...
 const {db,token,other}=await fixture();try{
  ...
  const newToken='later-browser-credential-'+'d'.repeat(40);(await db.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,reauthenticated_at,permission) VALUES('session:later','alice','session',?,?,?,'write')").run(await digest(newToken),at+900000,at));
  (await db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at));
...
test('outbound pagination is newest first, bounded, and retains expired, accepted, and revoked facts',async t=>{
 t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;
 const {db,token,other}=await fixture();let now=at;try{
  ...
  (await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+3*86400000));
  now=at+86400001; // No current recent proof; expired shares remain discoverable.
  const expected=(await db.raw.prepare('SELECT id FROM release_shares ORDER BY created_at DESC,id DESC').all()).map(r=>r.id);
...
test('outbound cursor is bound to the exact account and Space, while a new credential for that account can continue',async t=>{
 t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;
 const {db,token,other}=await fixture();try{
  (await db.raw.exec("UPDATE memberships SET role='admin' WHERE id='m2'"));const transfers=new Transfers(db,()=>at);
  ...
  const secondToken='d'.repeat(64);(await db.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,permission) VALUES('new-session','alice','session',?,?,'write')").run(await digest(secondToken),at+900000));
...
test('current Space managers recover grants issued by another manager whose membership and credential were revoked',async t=>{
 t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;
 const {db,token,other}=await fixture();try{
  const transfers=new Transfers(db,()=>at),share=await transfers.share(token,'so','alice@example.com');
  (await db.raw.exec("UPDATE memberships SET role='admin' WHERE id='m2';UPDATE memberships SET revoked_at=1 WHERE id='m1';UPDATE credentials SET revoked_at=1 WHERE id='session:alice'"));
...
test('outbound management does not inherit ancestor membership or accepted read shares',async t=>{
 t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;
 const {db,token,other}=await fixture();try{
  ...
  (await db.raw.exec("UPDATE memberships SET revoked_at=1 WHERE id='m1'"));
...
for(const kind of ['personal_key','oauth','read-session'])test('outbound list refuses '+kind+' even with an update policy',async()=>{
 const {db,token}=await fixture();try{
  const transfers=new Transfers(db,()=>at);const value=kind[0].repeat(64),credential=kind==='oauth'?'oauth:test':kind;
  (await db.raw.prepare('INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,permission) VALUES(?,?,?,?,?,?)').run(credential,'alice',kind==='personal_key'?'personal_key':'session',await digest(value),at+900000,kind==='read-session'?'read':'write'));
  (await db.raw.prepare('INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES(?,?,?)').run(credential,'["update"]','["s1"]'));
...
for(const boundary of ['credential-expiry','member-expiry','credential-revocation','member-revocation','organization-disabled'])test('outbound final snapshot enforces '+boundary,async()=>{
 const {db,token}=await fixture();let now=at,touched=false;try{
  ...
  if(boundary==='credential-expiry')(await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+1));
  if(boundary==='member-expiry')(await db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+1));
  const prepare=db.prepare.bind(db);db.prepare= sql=>{const s=prepare(fixOutboundPage(sql));if(sql.includes('/* outbound-share-page */')){const first=s.first.bind(s);s.first=async()=>{
   touched=true;
   if(boundary==='credential-revocation')(await db.raw.exec("UPDATE credentials SET revoked_at=1 WHERE id='session:alice'"));
   if(boundary==='member-revocation')(await db.raw.exec("UPDATE memberships SET revoked_at=1 WHERE id='m1'"));
   if(boundary==='organization-disabled')(await db.raw.exec("UPDATE organizations SET disabled_at=1 WHERE id='org'"));
...
test('empty outbound pages still require current update authority',async()=>{
 const {db,token,other}=await fixture();try{
  const transfers=new Transfers(db,()=>at),prepare=db.prepare.bind(db);db.prepare=sql=>prepare(fixOutboundPage(sql));
  assert.deepEqual(await transfers.outbound(token,'s1'),{results:[],nextCursor:null});
...
test('outbound pages seek their tenant index under100000 unrelated retained shares',async t=>{
 ...
 const {db,token}=await fixture();try{
  ...
  const plan=(await db.raw.prepare('EXPLAIN QUERY PLAN '+captured.sql).all(...captured.values)).map(r=>r.detail).join('\n');
  ...
  async function measure(){const statement=(await db.raw.prepare(captured.sql)),start=performance.now();for(let n=0;n<100;n++)statement.get(...captured.values);return(performance.now()-start)/100;}
  const before=measure();
  (await db.raw.prepare(`WITH RECURSIVE n(x) AS(VALUES(0) UNION ALL SELECT x+1 FROM n WHERE x<99999)
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_shares').get()).n,100028);
 }finally{db.close();}
});
