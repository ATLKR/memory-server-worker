...
async function afterDigest(t,callback){const original=crypto.subtle.digest;crypto.subtle.digest=async function(algorithm,data){const result=await original.call(this,algorithm,data);callback(new TextDecoder().decode(data));return result;};t.after(()=>{crypto.subtle.digest=original;});}
async function afterRead(db,fragment,callback){const prepare=db.prepare.bind(db);db.prepare= sql=>{const statement=prepare(sql),first=statement.first.bind(statement);statement.first=async()=>{const result=await first();if(sql.includes(fragment))callback();return result;};return statement;};}
async function afterAll(db,fragment,callback){const prepare=db.prepare.bind(db);db.prepare= sql=>{const statement=prepare(sql),all=statement.all.bind(statement);statement.all=async()=>{const result=await all();if(sql.includes(fragment))callback();return result;};return statement;};}
for(const expiry of ['session','member','proof'])test('SCIM key revoke rechecks '+expiry+' expiry after token hashing',async t=>{
 ...
 if(expiry==='session')(await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+100));
 if(expiry==='member')(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+100));
 if(expiry==='proof')(await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));
 ...
 assert.equal(response.status,403);assert.equal((await f.db.raw.prepare('SELECT revoked_at FROM release_scim_keys WHERE id=?').get(key.id)).revoked_at,null);
});
...
for(const member of ['Active','ACTIVE','active'])test('SCIM pathless deactivation accepts '+member,async t=>{
 ...
 assert.equal(response.status,204);assert.equal((await f.db.raw.prepare("SELECT revoked_at FROM memberships WHERE id='m2'").get()).revoked_at,at);
});
...
for(const external of [false,true])test('workspace sign-in rejects principal expiry during hashing; external='+external,async t=>{
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const service=new WorkspaceService(f.db,()=>now),before=(await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n;
 ...
 assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n,before);
});
...
test('verified OAuth policy insertion checks expiry after token hashing',async t=>{
 ...
 (await f.db.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,permission) VALUES(?,'alice','session',?,?,'read')").run('oauth:'+hash,hash,at+100));
 ...
 assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_credential_policies WHERE credential_id=? AND verified_oauth').get('oauth:'+hash)).n,0);
});

const keys=await generateKeyPair('RS256');
async function oauthFixture(t,scope){let now=at;const db=new DB(()=>now);(await db.migrate());t.after(()=>db.close());
 // PRODUCT DEFECTS in src: the empty allowed-set fallback 'SELECT NULL AS key'
 // resolves the key column as text under PG (a.key=w.key becomes text=integer),
 // and liveMemory '(w.value->>'currentFact')::int' rejects JSON booleans
 // ('false'::int is invalid; SQLite json_extract returned 0/1). Rewrite to the
 // equivalent predicates so the same authority semantics are exercised without
 ...
 const space=(await db.raw.prepare("SELECT id FROM spaces WHERE account_id=(SELECT account_id FROM provider_identities WHERE subject='oauth-r8')").get()).id;
...
test('MCP checks OAuth expiry after its capability snapshot',async t=>{
 const f=await oauthFixture(t,'memory:read');(await f.db.raw.prepare('UPDATE credentials SET expires_at=? WHERE token_digest=?').run(at+100,await digest(f.token)));
...
for(const boundary of ['credential','organization','recent proof','admin membership','SCIM key','SCIM membership'])test('authority rejects '+boundary+' expiry during its final SQL read',async t=>{
 ...
 if(boundary==='credential')(await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+100));
 if(['organization','admin membership','SCIM membership'].includes(boundary))(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+100));
...
for(const until of [200,600])test('final Space read uses independent accepted-share expiry after membership expiry; elapsed='+until,async t=>{
 ...
 (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m2'").run(at+100));
 (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+500));
...
for(const operation of ['identity account','identity organization','workspace snapshot','workspace members'])test(operation+' checks expiry after its final data query',async t=>{
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const identity=new IdentityService(f.db,()=>now),workspace=new WorkspaceService(f.db,()=>now);
 (await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+100));
...
for(const operation of ['admin key','member directory'])test(operation+' query avoids global active-membership materialization',async t=>{
 ...
 // The PostgreSQL plan lists one text row per node; keep bind synchronous so
 // prepare().bind().first() chains still work, and EXPLAIN at execution time.
 f.db.prepare= sql=>{const statement=prepare(sql),bind=statement.bind.bind(statement);let values=null;statement.bind=(...args)=>{values=args;return bind(...args);};
  for(const method of ['first','all','run']){const original=statement[method].bind(statement);statement[method]=async()=>{if(values!==null&&(operation==='admin key'?sql.includes('INTO memory_identity.workspace_key_issuances'):sql.includes('SELECT target.id')))details=(await f.db.raw.prepare('EXPLAIN '+sql).all(...values)).map(row=>row['QUERY PLAN']);return original();};}
  return statement;};
 ...
 assert.ok(details.length);assert.equal(details.some(detail=>/MATERIALIZE active_memberships|CTE Scan on active_memberships/.test(detail)),false,details.join('\n'));
});

for(const boundary of ['HMAC','body hash','receipt read'])test('identity webhook rejects timestamp expiry during '+boundary,async t=>{
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const secret='test-only-secret'.repeat(4),admin=new Admin({DB:f.db,IDENTITY_WEBHOOK_SECRET:secret},()=>now);
 ...
 if(boundary==='receipt read')afterRead(f.db,"SELECT body_hash AS hash FROM memory_ops.webhook_events WHERE provider='identity'",()=>{now=at+2000;});
 ...
 assert.equal((await f.db.raw.prepare("SELECT count(*) n FROM release_webhook_events WHERE event_id='expired-webhook'").get()).n,0);
 assert.equal((await f.db.raw.prepare("SELECT count(*) n FROM release_provider_revocations WHERE subject='pending-user'").get()).n,0);
});

test('mail checks authority after its budget write before sending',async t=>{
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());let sent=0;
 (await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+100));
 const prepare=f.db.prepare.bind(f.db);f.db.prepare= sql=>{const statement=prepare(sql),run=statement.run.bind(statement);statement.run=async()=>{const result=await run();if(sql.startsWith('INSERT INTO memory_ops.mail_budget'))now=at+200;return result;};return statement;};
