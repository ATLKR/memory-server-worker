...
function clean(f){for(const table of stages)assert.equal(rows(f,table).length,0,table);for(const r of f.calls)if(r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_provider_v1_attempts'))){assert.ok(r.statements.length<=100);assert.ok(Buffer.byteLength(JSON.stringify(r))<=1048576);}assert.equal(f.raw.prepare('SELECT state FROM release_seoul_projection_state').get().state,'backfill-required');assert.deepEqual(f.raw.prepare('PRAGMA foreign_key_check').all(),[]);}
async function setup(t,recursive='ON',mapped=true){
 ...
 for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql','0028_seoul-projection-bootstrap-schema.sql','0029_seoul-projection-preparation-schema.sql','0030_seoul-projection-workspace-schema.sql','0031_seoul-projection-provider-schema.sql','0032_seoul-projection-command-schema.sql'])f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8'));
 const schema=readFileSync(new URL('../../seoul-projection-provider-v1-schema.sql',import.meta.url),'utf8'),migration=readFileSync(new URL('../../migrations/0033_seoul-projection-provider-v1-schema.sql',import.meta.url),'utf8');assert.equal(migration,'-- Forward SaaS migration 33: seoul-projection-provider-v1-schema.sql\n'+schema);f.raw.exec(migration);assert.equal(f.raw.prepare('SELECT version FROM release_meta').get().version,33);
 f.raw.exec(readFileSync(new URL('../../migrations/0034_seoul-projection-target-schema.sql',import.meta.url),'utf8'));assert.equal(f.raw.prepare('SELECT version FROM release_meta').get().version,34);assert.deepEqual(f.raw.prepare('PRAGMA foreign_key_check').all(),[]);f.raw.exec('PRAGMA recursive_triggers='+recursive);
 ...
 if(mapped)for(const subject of ['alice','alias\n\uE000'])f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at-200000);
 f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,permission) VALUES('api:alice','alice','m1','e1','api_key',?,?,'write')").run('1'.repeat(64),at+900000);f.raw.exec("INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES('api:alice','[\"read\"]',NULL)");
...
test('block-only email aliases change only effective shared facts including globally masked block',async t=>{
 ...
 f.raw.prepare("INSERT INTO domains VALUES('domain','org','example.com',?,NULL)").run(at+900000);f.raw.exec("INSERT INTO domain_managers VALUES('domain','m1',NULL)");f.raw.prepare("INSERT INTO revocations VALUES('masked-receipt','domain','session:alice',NULL,'domain','masked@example.com',?)").run(at);await f.deliver({...f.event('masked','email.revoked'),email:'masked@example.com'});const second=bodies(f).slice(first.length);assert.equal(second.length,1);assert.equal(second[0].subject,'alice');clean(f);
});
...
for(const tail of ['\uD800','\uDC00'])for(const type of ['account.disabled','email.revoked'])test('original unpaired '+tail.charCodeAt(0)+' never becomes a provider transition '+type,async t=>{
 const f=await setup(t,'ON',false),subject='original'+tail;f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at);await f.deliver(f.event('surrogate',type,subject));const core=bodies(f);assert.equal(core.length,1);assert.equal(core[0].kind,'subject-source-unrepresentable');assert.deepEqual(rows(f,'release_seoul_account_exclusions').map(r=>r.account_id),['alice']);assert.equal(rows(f,'release_webhook_events').length,1);assert.equal(rows(f,'release_provider_revocations').length,1);clean(f);
});
...
for(const type of ['account.disabled','email.revoked'])test('retained invalid time records only an actual affected account marker '+type,async t=>{
 const f=await setup(t);f.raw.prepare('INSERT INTO release_provider_revocations VALUES(?,?,?,?,?)').run(issuer,'alice',type,type==='email.revoked'?'alice@example.com':'',-1);await f.deliver(f.event('bad-time',type));assert.ok(bodies(f).every(b=>b.kind==='subject-source-unrepresentable'));assert.equal(bodies(f).length,1);const before=sources(f);await f.deliver(f.event('bad-time-audit',type));assert.deepEqual(sources(f),before);clean(f);
});
...
for(const type of ['account.disabled','email.revoked'])test('audit-only unsupported representation creates no exclusion '+type,async t=>{
 const f=await setup(t),email='absent@example.com';f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://unsupported.example','alias','alice',at);f.raw.prepare('INSERT INTO release_provider_revocations VALUES(?,?,?,?,?)').run(issuer,'alice',type,type==='email.revoked'?email:'',at-1);
 if(type==='account.disabled')f.raw.prepare("UPDATE accounts SET disabled_at=? WHERE id='alice'").run(at-1);else f.raw.prepare("INSERT INTO release_external_email_blocks VALUES('alice',?,?)").run(email,at-1);
...
for(const type of ['account.disabled','email.revoked'])test('first mapping after preflight is resolved in the actual batch '+type,async t=>{
 const f=await setup(t,'ON',false);f.beforeExecute=async r=>{if(!r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_provider_v1_attempts')))return;f.beforeExecute=null;f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'alice','alice',at);};await f.deliver(f.event('mapping-race',type));assert.ok(bodies(f).some(b=>b.accountId==='alice'));assert.ok(!bodies(f).some(b=>b.kind==='identity-transition-source'));clean(f);
});
...
test('bad HMAC, application expiry, write-clock expiry and ignored receipt keep original denial',async t=>{
 ...
 f.db.setClock(()=>at+300001);await assert.rejects(f.deliver(event),e=>e.status===401);f.db.setClock(f.clock);f.raw.exec("CREATE TRIGGER ignore_v1_receipt BEFORE INSERT ON release_webhook_events BEGIN SELECT RAISE(IGNORE); END");await assert.rejects(f.deliver(event),e=>e.status===401);assert.equal(rows(f,'release_webhook_events').length,0);assert.equal(rows(f,'release_provider_revocations').length,0);assert.equal(sources(f).length,0);clean(f);
});
...
for(const recursive of ['ON','OFF'])for(const type of ['account.disabled','email.revoked'])test('V1 preserves current captured revision including in-batch progress '+type+'; '+recursive,async t=>{
 const f=await setup(t,recursive),prior=f.raw.prepare('SELECT max(revision) n FROM release_seoul_authority_changes').get().n;f.raw.prepare("INSERT INTO release_seoul_dirty_spaces VALUES('so',?,1)").run(prior);f.raw.exec(`CREATE TRIGGER advance_captured AFTER INSERT ON release_seoul_authority_changes WHEN NEW.source_command_id<>'setup-target' BEGIN UPDATE release_seoul_dirty_spaces SET captured_revision=${prior} WHERE space_id='so'; END`);await f.deliver(f.event('captured',type));const dirty=f.raw.prepare("SELECT * FROM release_seoul_dirty_spaces WHERE space_id='so'").get();assert.equal(dirty.captured_revision,prior);assert.equal(dirty.dirty_revision,Math.max(...sources(f).map(r=>r.revision)));assert.ok(dirty.dirty_revision>prior);clean(f);
});
...
for(const reason of ['identities','combined-keys','vector-product','identity-bytes','policy-bytes','policy-state','spaces','space-id','foreign'])test('complete account representation excluded before normal JSON: '+reason,async t=>{
 ...
 if(['identities','combined-keys','vector-product','identity-bytes'].includes(reason)){const count=reason==='identities'?513:reason==='combined-keys'?256:reason==='vector-product'?80:100;for(let n=0;n<count;n++)f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'alias:'+n+'x'.repeat(reason==='identity-bytes'?450:30),'alice',at);}
 if(reason==='policy-bytes')f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run(JSON.stringify(Array(30000).fill('read')),'api:alice');if(reason==='policy-state')f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run('["admin"]','api:alice');
 if(reason==='spaces')for(let n=0;n<513;n++)f.raw.prepare("INSERT INTO spaces VALUES(?,'Raw','alice',NULL,'managed',?,'session:alice')").run('raw:'+n,at);if(reason==='space-id')f.raw.prepare("INSERT INTO spaces VALUES(?,'Raw','alice',NULL,'managed',?,'session:alice')").run('x'.repeat(129),at);if(reason==='foreign')f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://other.example','alias','alice',at);
...
for(const type of ['account.disabled','email.revoked'])test('irrelevant revoked claim and credential history never excludes '+type,async t=>{
 const f=await setup(t);for(let n=0;n<600;n++){f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,?)').run('old:e'+n,'alice','old'+n+'@example.com','example.com',at-20,at-10);f.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,revoked_at,permission) VALUES(?,'alice','personal_key',?,?,?,'read')").run('old:k'+n,(n+100).toString(16).padStart(64,'0'),at+900000,at-10);}
...
for(const recursive of ['ON','OFF'])for(const phase of ['source','head','dirty','scope-cleanup','spaces-cleanup','identities-cleanup','attempt-cleanup'])test('late '+phase+' rolls back original receipt, permanent row, cascades and capture; '+recursive,async t=>{
 ...
 f.raw.exec(`CREATE TRIGGER fail_v1_late BEFORE ${phase.endsWith('cleanup')?'DELETE':'INSERT'} ON ${target} BEGIN SELECT RAISE(ABORT,'late V1 failure'); END`);await assert.rejects(f.deliver(f.event('rollback','email.revoked')),/late V1 failure/);for(const [table,data]of before)assert.deepEqual(rows(f,table),data,table);clean(f);
});
...
test('fresh staging guard runs even unmapped, and same database identity is mandatory',async t=>{
 ...
 f.raw.prepare("INSERT INTO release_seoul_provider_v1_attempts(id,publisher_event_id,body_hash,issuer,subject,address,event_type,key_supported) VALUES(?,?,?,?,?,'','account.disabled',1)").run(crypto.randomUUID(),'stale','a'.repeat(64),issuer,'stale');const before=rows(f,stages.at(-1));await assert.rejects(f.deliver(f.event('stage','account.disabled')),/staging is not empty/);assert.deepEqual(rows(f,stages.at(-1)),before);assert.equal(rows(f,'release_webhook_events').length,0);
});

for(const type of ['account.disabled','email.revoked'])test('bounded SQL plan and source/request sizes '+type,async t=>{
 const f=await setup(t);await f.deliver(f.event('plan',type));const request=f.calls.find(r=>r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_provider_v1_attempts'))),explain=piece=>{const q=request.statements.find(s=>s.sql.includes(piece));assert.ok(q,piece);return f.raw.prepare('EXPLAIN QUERY PLAN '+q.sql).all(...q.values).map(r=>r.detail);};
...
test('a new exact account tombstone still emits direct deletion when a nonempty legacy address already denies',async t=>{
 const f=await setup(t);f.raw.prepare('INSERT INTO release_provider_revocations VALUES(?,?,?,?,?)').run(issuer,'alice','account.disabled','retained@example.com',at-123);f.raw.prepare("UPDATE accounts SET disabled_at=? WHERE id='alice'").run(at-321);await f.deliver(f.event('exact-tuple','account.disabled'));const core=bodies(f);assert.equal(core.length,1);assert.equal(core[0].subject,'alice');assert.equal(core[0].effect.type,'subject-lifecycle');assert.equal(core[0].effect.occurredAtMs,at);const before=sources(f);await f.deliver(f.event('exact-audit','account.disabled'));assert.deepEqual(sources(f),before);clean(f);
});
...
for(const type of ['account.disabled','email.revoked'])test('direct and alias V2 history stays independent of current V1 receipt and stored time '+type,async t=>{
 const f=await setup(t,'ON',false),bare=new Admin({DB:f.adapter,IDENTITY_WEBHOOK_SECRET:secret},f.clock),subjects=['alice','alias\n\uE000'];for(const [i,subject]of subjects.entries()){const old={version:2,id:'old:v2:'+i,sequence:i+1,issuer,subject,type:type==='account.disabled'?'account.resumed':'email.verified',occurredAt:at-30000-i,email:type==='email.revoked'?'alice@example.com':null};await bare.identityWebhook(await f.request(old));f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at-10000);}
 ...
 f.raw.prepare('INSERT INTO release_provider_revocations VALUES(?,?,?,?,?)').run(issuer,'alice',type,type==='email.revoked'?'alice@example.com':'',at-22222);f.advance(1000);await f.deliver(f.event('with-v2',type));const core=bodies(f).filter(b=>b.subject);assert.equal(core.length,2);for(const b of core){assert.equal(b.lifecycle.eventId,'old:v2:'+subjects.indexOf(b.subject));assert.equal(b.origin.eventId,'v1:with-v2');assert.equal(b.origin.storedRevocationAtMs,at-22222);assert.equal(b.effect.type,b.subject==='alice'?(type==='account.disabled'?'subject-lifecycle':'email-lifecycle'):'entity-head');}clean(f);
});

test('email revoked history is filtered before live membership/API sentinels',async t=>{
 const f=await setup(t);for(let n=0;n<600;n++){f.raw.prepare("INSERT INTO memberships VALUES(?,'org','alice','e1','member',?,?)").run('old:m'+n,at+900000,at-10);f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,revoked_at,permission) VALUES(?,'alice','m1','e1','api_key',?,?,?,'read')").run('old:api'+n,(n+5000).toString(16).padStart(64,'0'),at+900000,at-10);}
...
for(const recursive of ['ON','OFF'])for(const type of ['account.disabled','email.revoked'])for(const stage of ['tombstone','physical'])test('ignored original '+stage+' preserves central success and emits only representable evidence '+type+'; '+recursive,async t=>{
 const f=await setup(t,recursive,false);f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'alice','alice',at);if(stage==='tombstone')f.raw.exec("CREATE TRIGGER ignore_v1_tombstone BEFORE INSERT ON release_provider_revocations BEGIN SELECT RAISE(IGNORE); END");else if(type==='account.disabled')f.raw.exec("CREATE TRIGGER ignore_v1_disable BEFORE UPDATE ON accounts BEGIN SELECT RAISE(IGNORE); END");else f.raw.exec("CREATE TRIGGER ignore_v1_claim BEFORE UPDATE ON account_emails BEGIN SELECT RAISE(IGNORE); END");
 ...
 if(stage==='tombstone'){assert.ok(!core.some(b=>b.origin));assert.equal(type==='account.disabled'?f.raw.prepare("SELECT disabled_at n FROM accounts WHERE id='alice'").get().n:f.raw.prepare("SELECT revoked_at n FROM account_emails WHERE id='e1'").get().n,at);}else{assert.equal(core.find(b=>b.kind==='identity-transition-source').origin.storedRevocationAtMs,at);assert.equal(type==='account.disabled'?f.raw.prepare("SELECT disabled_at n FROM accounts WHERE id='alice'").get().n:f.raw.prepare("SELECT revoked_at n FROM account_emails WHERE id='e1'").get().n,null);}
...
for(const reason of ['live-members','live-api','email-spaces','old-members'])test('complete indexed dependent scope excludes overflow '+reason,async t=>{
 const f=await setup(t);for(let n=0;n<513;n++){
  if(reason==='live-members'){f.raw.prepare('INSERT INTO organizations(id) VALUES(?)').run('o:'+n);f.raw.prepare("INSERT INTO memberships VALUES(?,?,'alice','e1','member',?,NULL)").run('m:'+n,'o:'+n,at+900000);}
  if(reason==='live-api')f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,permission) VALUES(?,'alice','m1','e1','api_key',?,?,'read')").run('api:'+n,(n+1000).toString(16).padStart(64,'0'),at+900000);
  if(reason==='email-spaces')f.raw.prepare("INSERT INTO spaces VALUES(?,'Raw',NULL,'org','managed',?,'session:alice')").run('raw:'+n,at);
  if(reason==='old-members')f.raw.prepare("INSERT INTO memberships VALUES(?,'org','alice','e1','member',?,?)").run('old:m'+n,at+900000,at-1);
 }
...
for(const recursive of ['ON','OFF'])for(const type of ['account.disabled','email.revoked'])test('ignored final V1 attempt cleanup aborts before commit '+type+'; '+recursive,async t=>{
 ...
 f.raw.exec("CREATE TRIGGER ignore_v1_final_cleanup BEFORE DELETE ON release_seoul_provider_v1_attempts BEGIN SELECT RAISE(IGNORE); END");
