...
for(const deleted of [false,true])test((deleted?'trash':'live')+' memory pages use their creation-time index without sorting the Space',async()=>{
 const {db,token}=await fixture();try{
  ...
  for(const query of queries.filter(q=>q.sql.includes('ORDER BY r.created_at DESC,r.id'))){
   const plan=(await db.raw.prepare('EXPLAIN '+query.sql).all(...query.values)).map(row=>row['QUERY PLAN']).join('\n');
   assert.ok(!plan.includes('Sort'),plan);
  }
 }finally{db.close();}
});
...
test('export target lookup is indexed by Space and pages hydrate only their snapshot IDs',async()=>{
 const {db,token,other}=await fixture();try{
  ...
  for(let n=0;n<1000;n++)(await db.raw.prepare("INSERT INTO memory_audit_events(action,space_id,memory_id,revision,actor_credential_id,created_at) VALUES('memory_updated','s2',?,1,'session:bob',?)").run(foreign.id,at));
  ...
  for(const query of targetQueries){
   const plan=(await db.raw.prepare('EXPLAIN '+query.sql).all(...query.values)).map(row=>row['QUERY PLAN']).join('\n');
   assert.match(plan,/Index( Only)? Scan using release_audit_export/);assert.match(plan,/Index Cond:[^\n]*space_id/);assert.ok(query.count<=2);
  }
 }finally{db.close();}
});

test('share invitation pages are bounded, stable, and bound to the recipient account',async t=>{
...
test('invitation cursor advances across a bounded expired or unauthorized page',async t=>{
 t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;
 const {db,token,other}=await fixture();let now=at;try{
  (await db.raw.prepare('UPDATE credentials SET expires_at=?').run(at+7*86400000));
  ...
  now+=86400000;(await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(now));
  const visible=await transfers.share(token,'s1','bob@example.com');
  (await db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(now));
