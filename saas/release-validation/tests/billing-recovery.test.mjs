...
test('checkout recovers a customer failure after six minutes with a fresh Stripe expiry', async t => {
  ...
  assert.equal((await db.raw.prepare('SELECT checkout_attempted FROM release_checkout_requests').get()).checkout_attempted, 0);
  now += 360000;
  (await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(now));
  assert.equal((await billing.checkout(token, 's1', 'price_test', 'recovery')).url, 'https://checkout.stripe.com/c/pay/test');
  assert.equal(checkoutCalls, 1);
  const request = await db.raw.prepare('SELECT expires_at FROM release_checkout_requests').get();
...
test('concurrent checkout claims reuse the winning persisted expiry and Stripe parameters', async t => {
  const { db, token } = await fixture(); t.after(() => db.close());
  let now = at + 360000, claimed = false, competing;
  (await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(now));
  (await db.raw.prepare("UPDATE release_pools SET customer_id='cus_test' WHERE id='account:alice'").run());
  (await db.raw.prepare(`INSERT INTO release_checkout_requests(id,pool_id,price_id,operation_key,created_at,expires_at,checkout_attempted)
  ...
  db.prepare = sql => {
    ...
    if (sql.startsWith('UPDATE memory_control.checkout_requests SET expires_at=') || sql.startsWith('UPDATE release_checkout_requests SET expires_at=')) {
  ...
  assert.deepEqual({ ...(await db.raw.prepare('SELECT checkout_attempted,expires_at FROM release_checkout_requests').get()) },
...
test('lost checkout responses retain their attempt and identical parameters after six minutes', async t => {
  ...
  const original = await db.raw.prepare('SELECT checkout_attempted,expires_at,session_id FROM release_checkout_requests').get();
  ...
  assert.deepEqual(calls[0], calls[1]);
  assert.equal((await db.raw.prepare('SELECT expires_at FROM release_checkout_requests').get()).expires_at, original.expires_at);
...
test('attempted checkout parameters are frozen while unattempted claims stay mutable', async t => {
  const { db } = await fixture(); t.after(() => db.close());
  (await db.raw.prepare(`INSERT INTO release_checkout_requests(id,pool_id,price_id,operation_key,created_at,expires_at,session_id,checkout_url,checkout_attempted)
    VALUES('attempted','account:alice','price_test','op:attempted',?,?, 'cs_old','https://checkout.stripe.com/c/pay/old',1)`).run(at - 3600000, at - 1500000));
  for (const sql of ["UPDATE release_checkout_requests SET checkout_attempted=0 WHERE id='attempted'",
    "UPDATE release_checkout_requests SET expires_at=expires_at+1000 WHERE id='attempted'",
    "UPDATE release_checkout_requests SET price_id='price_other' WHERE id='attempted'",
    "UPDATE release_checkout_requests SET operation_key='other' WHERE id='attempted'"])
    await assert.rejects(async () => (await db.raw.exec(sql)), /memory_immutable_record/);
  (await db.raw.prepare(`INSERT INTO release_checkout_requests(id,pool_id,price_id,operation_key,created_at,expires_at,checkout_attempted)
    VALUES('pending','account:alice','price_test','op:pending',?,?,0)`).run(at, at + 2100000));
  (await db.raw.prepare("UPDATE release_checkout_requests SET expires_at=?,checkout_attempted=1 WHERE id='pending' AND checkout_attempted=0").run(at + 2400000));
  await assert.rejects(async () => (await db.raw.exec("UPDATE release_checkout_requests SET expires_at=expires_at+1000 WHERE id='pending'")), /memory_immutable_record/);
  // A session/url recorded at claim time may still be filled in on the row.
  (await db.raw.prepare("UPDATE release_checkout_requests SET session_id='cs_new',checkout_url='https://checkout.stripe.com/c/pay/new' WHERE id='pending'").run());
});
