...
for(const action of ['create','update','delete','restore','erase','supersede','search'])for(const expiry of ['credential','membership'])test(action+' commits nothing when '+expiry+' expires during operation lookup',async()=>{
 ...
 try {
  ...
  (await db.raw.prepare(expiry==='membership'?"UPDATE memberships SET expires_at=? WHERE id='m1'":"UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+1));
  const before=JSON.stringify((await db.raw.prepare('SELECT * FROM memories').all())),count=(await db.raw.prepare('SELECT count(*) n FROM release_operations').get()).n;
  ...
  assert.equal(JSON.stringify((await db.raw.prepare('SELECT * FROM memories').all())),before);
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_operations').get()).n,count,'A denied write must not add an operation or usage charge');
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_usage_events').get()).n,count);
 }finally{db.close();}
});
...
for(const replay of [false,true])test('erasure '+(replay?'replay':'write')+' requires recent proof after operation lookup',async()=>{
 ...
 try {
  ...
  (await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299999));
  const changed=advanceAfterRead(db,operationLookup,()=>{now+=2;});
  await assert.rejects(()=>store.erase(token,'s1',memory.id,2,memory.id,'erase'),error=>error.status===403);
  assert.equal(changed(),true);
  assert.equal((await db.raw.prepare('SELECT erased_at FROM memories').get()).erased_at!==null,replay);
 }finally{db.close();}
});

for(const action of ['retention','rebuild','export','share','revoke','accept'])test(action+' rejects expired authority immediately before its mutation',async t=>{
 if(action==='share'||action==='revoke'||action==='accept'){t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;}
 ...
 try {
  ...
  (await db.raw.exec("UPDATE release_jobs SET state='done'"));
  (await db.raw.prepare('UPDATE credentials SET expires_at=? WHERE id=?').run(at+1,'session:alice'));
  ...
  await assert.rejects(actions[action],error=>error.status===403);assert.equal(changed(),true);
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_space_policies').get()).n,0);
  ...
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE memory_id=?').get(memory.id)).state,'done');
 }finally{db.close();}
});
