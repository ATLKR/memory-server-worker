...
test('unconfigured vector deletes remain pending without burning retry attempts',async()=>{
 const {db,token}=await fixture();try{
  ...
  const row=(await db.raw.prepare("SELECT state,attempt,last_error FROM release_jobs WHERE kind='delete'").get());
...
test('unavailable rebuild and paused extraction reject before creating work',async()=>{
 const {db,token}=await fixture();try{
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_jobs').get()).n,0);
 }finally{db.close();}
});
...
for(const enabled of [undefined,'false','true'])test('scheduled provider work requires explicit enablement: '+enabled,async()=>{
 const {db,token}=await fixture();let calls=0,billingCalls=0;try{
  ...
  (await db.raw.prepare("INSERT INTO release_billing_events(id,subscription_id,available_at,created_at) VALUES('queued-billing','sub_synthetic',?,?)").run(at,at));
  ...
  assert.equal((await db.raw.prepare("SELECT last_success_at FROM release_heartbeats WHERE name='maintenance'").get()).last_success_at,at);
 }finally{db.close();}
});
...
test('maintenance bounds temporary cleanup, preserves live proofs and retained memories',async()=>{
 const {db,token}=await fixture();try{
  ...
  const insert=(await db.raw.prepare("INSERT INTO release_reauth_challenges(id,credential_id,email_id,token_digest,expires_at) VALUES(?,'session:alice','e1','digest',?)"));
  for(let n=0;n<105;n++)await insert.run('expired-'+n,at-1);
  await insert.run('current',at+1);
  (await db.raw.prepare("INSERT INTO email_challenges(id,account_id,address,domain,token_digest,expires_at) VALUES('identity-audit','alice','added@example.com','example.com',?,?)").run('d'.repeat(64),at-1));
  (await db.raw.prepare("INSERT INTO release_export_sessions VALUES('expired-export','alice','s1',0,?,?)").run(at-1,at-1000));
  (await db.raw.prepare("INSERT INTO release_mail_budget VALUES('alice','2026-09-07',20)").run());
  (await db.raw.prepare("INSERT INTO release_mail_budget VALUES('alice','2026-09-08',20)").run());
  const jobs=new Jobs(environment(db),()=>at);await jobs.maintain();
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_reauth_challenges WHERE expires_at<=?').get(at)).n,5);
  assert.ok((await db.raw.prepare("SELECT id FROM release_reauth_challenges WHERE id='current'").get()));
  assert.ok((await db.raw.prepare("SELECT id FROM email_challenges WHERE id='identity-audit'").get()));
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_export_sessions').get()).n,0);
  assert.deepEqual((await db.raw.prepare('SELECT day,quantity FROM release_mail_budget').all()).map(x=>({...x})),[{day:'2026-09-08',quantity:20}]);
  await jobs.maintain();assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_reauth_challenges WHERE expires_at<=?').get(at)).n,0);
  assert.equal((await db.raw.prepare('SELECT body FROM memories WHERE id=?').get(m.id)).body,'retained memory');
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM memory_versions').get()).n,1);
 }finally{db.close();}
});

test('configuration and ingest lists describe disabled automation and expiry truthfully',async()=>{
 const {db,token}=await fixture();try{
  (await db.raw.prepare("INSERT INTO release_jobs(id,space_id,revision,kind,state,available_at,created_at) VALUES('expired','s1',0,'ingest','done',?,?)").run(at,at));
  (await db.raw.prepare("INSERT INTO release_ingests(id,account_id,space_id,actor_credential_id,state,proposals,expires_at,created_at) VALUES('expired','alice','s1','session:alice','review','[]',?,?)").run(at-1,at-1000));
