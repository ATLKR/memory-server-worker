...
async function setup(t,release=true){const f=await fixture();t.after(()=>f.db.close());let now=at;
 (await f.db.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(AUTH_ISSUER,'r12','bob',at));
...
for(const release of [false,true])for(const version of ['2025-11-25','2026-07-28'])for(const change of ['revoke','expire'])test('final MCP '+(release?'release':'foundation')+' '+version+' suppresses memory after membership '+change,async t=>{
 const f=await setup(t,release),count=inject(f.db,async ()=>(await f.db.raw.prepare(change==='revoke'?"UPDATE memberships SET revoked_at=? WHERE id='m2'":"UPDATE memberships SET expires_at=? WHERE id='m2'").run(at)));
...
for(const kind of ['oauth','pat','session'])for(const change of ['credential','membership'])test(kind+' checks '+change+' expiry after the final snapshot without plaintext',async t=>{
 const f=await setup(t);let token=f.oauth,credentialId=(await f.db.raw.prepare("SELECT id FROM credentials WHERE id LIKE 'oauth:%'").get()).id;
 ...
 (await f.db.raw.prepare(change==='credential'?'UPDATE credentials SET expires_at=? WHERE id=?':'UPDATE memberships SET expires_at=? WHERE id=?').run(at+100,change==='credential'?credentialId:'m2'));
...
for(const kind of ['pat','session'])test('final '+kind+' response suppresses revocation during buffering',async t=>{
 ...
 const count=inject(f.db,async ()=>(await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m2'").run(at))),response=await f.call('memory_get',{spaceId:'so',memoryId:f.saved.id},token),text=await response.text();assert.equal(count(),1);assert.equal(response.status,200);assert.doesNotMatch(text,/CONFIDENTIAL-R12/);assert.equal(result(text).isError,true);
});
for(const grant of ['independent','share revoked','recipient revoked','grantor revoked','share expired','grantor expired'])test('final snapshot respects accepted-share alternatives: '+grant,async t=>{
 ...
 if(grant==='share expired')(await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:bob'").run(at+172800000));
 if(grant==='grantor expired')(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+100));
 if(grant.endsWith('revoked'))inject(f.db,async ()=>{(await f.db.raw.prepare(grant==='share revoked'?'UPDATE release_shares SET revoked_at=? WHERE id=?':grant==='recipient revoked'?'UPDATE account_emails SET revoked_at=? WHERE id=?':'UPDATE memberships SET revoked_at=? WHERE id=?').run(at,grant==='share revoked'?share.id:grant==='recipient revoked'?'e2':'m1'));f.setNow(at+200);});
...
for(const release of [false,true])test('Space list suppresses expired metadata in final '+(release?'release':'foundation')+' snapshot',async t=>{
 const f=await setup(t,release);(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m2'").run(at+100));inject(f.db,()=>f.setNow(at+200),true);
...
for(const name of ['memory_list','memory_search'])test('empty '+name+' still checks requested Space authority',async t=>{
 const f=await setup(t);await f.memory.remove(f.token,'so',f.saved.id,1,'remove');inject(f.db,async ()=>(await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m2'").run(at)));
...
for(const change of ['read lost','revision','erasure'])test('full write response becomes a receipt after '+change+' without claiming the committed write failed',async t=>{
 ...
 inject(f.db,async()=>{const memory=(await f.db.raw.prepare("SELECT id FROM memories WHERE space_id='s2'").get());if(change==='read lost')(await f.db.raw.prepare('INSERT INTO memory_identity.credential_policies(credential_id,capabilities,space_ids,verified_oauth) VALUES(?,?::jsonb,NULL,true) ON CONFLICT(credential_id) DO UPDATE SET capabilities=excluded.capabilities,verified_oauth=true').run(key.id,'["create"]'));else if(change==='revision')await f.memory.update(f.other,'s2',memory.id,{body:'later',expectedRevision:1},'later');else {await f.memory.remove(f.other,'s2',memory.id,1,'delete');await f.memory.erase(f.other,'s2',memory.id,2,memory.id,'erase');}});
...
test('write-only receipt survives erasure and keeps its committed revision',async t=>{
 ...
 inject(f.db,async()=>{const memory=(await f.db.raw.prepare("SELECT id FROM memories WHERE space_id='s2'").get());await f.memory.remove(f.other,'s2',memory.id,1,'delete');await f.memory.erase(f.other,'s2',memory.id,2,memory.id,'erase');});
...
test('superseding add receipt retains its separate update-capability requirement',async t=>{
 const f=await setup(t),key=await new Admin({DB:f.db},()=>at).issueKey(f.token,{label:'Writer',organizationId:'org',capabilities:['create','update'],spaceIds:['so'],expiresInDays:1});inject(f.db,async ()=>(await f.db.raw.prepare('INSERT INTO memory_identity.credential_policies(credential_id,capabilities,space_ids,verified_oauth) VALUES(?,?::jsonb,NULL,true) ON CONFLICT(credential_id) DO UPDATE SET capabilities=excluded.capabilities,verified_oauth=true').run(key.id,'["create"]')));
...
test('final response query binds bounded manifests and uses point lookups under foreign growth',async t=>{
 const f=await setup(t),prepare=f.db.prepare.bind(f.db);let query,values;
 ...
 const median=async ()=>{const times=[];for(let i=0;i<5;i++){const before=performance.now();(await f.db.raw.prepare(query).get(...values));times.push(performance.now()-before);}return times.sort((a,b)=>a-b)[2];},before=await median();
 (await f.db.raw.exec(`INSERT INTO memory_control.spaces(id,owner_account_id,deployment_id,data_policy,source_byte_limit,message_limit,name,security_mode,created_at_ms,actor_credential_id) SELECT 'foreign:'||x,'alice','memory-sg',(SELECT data_policy FROM memory_control.spaces WHERE id='s1'),67108864,100000,'Foreign','managed',${at},'session:alice' FROM generate_series(1,100000) x;`));
 const elapsed=await median(),plan=(await f.db.raw.prepare('EXPLAIN (COSTS OFF) '+query).all(...values)).map(row=>row['QUERY PLAN']);assert.equal(plan.some(line=>/Seq Scan on (\S+\.)?\w+ (s|r)\b/.test(line)),false,plan.join('\n'));assert.ok(plan.some(line=>/Index (Scan|Only Scan|Cond).*spaces/i.test(line)||/Index Cond.*s\.id/i.test(line)),plan.join('\n'));assert.ok(elapsed<before*8+5);assert.ok(values.length<30);t.diagnostic('final snapshot median ms: '+before.toFixed(3)+' -> '+elapsed.toFixed(3));
});
