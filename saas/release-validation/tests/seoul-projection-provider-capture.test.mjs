...
async function setup(t,recursive='ON',mapped=true){
 ...
 for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql','0028_seoul-projection-bootstrap-schema.sql','0029_seoul-projection-preparation-schema.sql','0030_seoul-projection-workspace-schema.sql'])f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8'));
 assert.equal(f.raw.prepare('SELECT version FROM release_meta').get().version,30);const schema=new URL('../../seoul-projection-provider-schema.sql',import.meta.url);assert.ok(existsSync(schema),'additive provider schema must exist');const migration=readFileSync(new URL('../../migrations/0031_seoul-projection-provider-schema.sql',import.meta.url),'utf8');assert.equal(migration,'-- Forward SaaS migration 31: seoul-projection-provider-schema.sql\n'+readFileSync(schema,'utf8'));f.raw.exec(migration);assert.equal(f.raw.prepare('SELECT version FROM release_meta').get().version,31);
 for(const file of ['0032_seoul-projection-command-schema.sql','0033_seoul-projection-provider-v1-schema.sql','0034_seoul-projection-target-schema.sql'])f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8'));
...
test('actual Admin V2 signature path forwards optional capture; expired signature and late proof create no source',async t=>{
 ...
 const original=f.capture.providerStatements.bind(f.capture);f.capture.providerStatements=(...args)=>{f.db.setClock(()=>at+1000000);return original(...args);};await assert.rejects(f.verified([f.event(3,'account.resumed')]),e=>e.status===401);assert.deepEqual(sources(f),before);clean(f);
});
...
for(const recursion of ['ON','OFF'])test('late second primitive failure rolls back whole verified operation and all grouping; recursion '+recursion,async t=>{
 ...
 f.raw.exec("CREATE TRIGGER fail_second_group BEFORE INSERT ON release_seoul_provider_groups WHEN NEW.ordinal=1 BEGIN SELECT RAISE(ABORT,'second primitive failure'); END");
...
test('terminal deletion never resumes and older negative preparation leaves the newer head intact',async t=>{
 const f=await setup(t);await f.deliver(f.event(1,'account.suspended'));const negative=sources(f).find(r=>r.stream_kind==='subject'&&r.stream_key===issuer+'\nalice');f.advance(2000);await f.deliver(f.event(2,'account.resumed'));const latest=f.raw.prepare("SELECT * FROM release_seoul_authority_heads WHERE stream_kind='subject' AND stream_key=?").get(issuer+'\nalice');
 const preparer=createSeoulProjectionPreparer(f.adapter,'durable-sql');assert.equal((await preparer.prepare(negative.revision)).status,'prepared');assert.equal((await preparer.prepare(negative.revision)).status,'already_prepared');const prepared=f.raw.prepare('SELECT payload_bytes FROM release_seoul_projection_events WHERE event_id=?').get(negative.event_id);assert.equal(decodeSeoulHeadEvent(new TextEncoder().encode(prepared.payload_bytes)).effect.state,'suspended');assert.deepEqual(f.raw.prepare("SELECT * FROM release_seoul_authority_heads WHERE stream_kind='subject' AND stream_key=?").get(issuer+'\nalice'),latest);
...
for(const reason of ['identities','product','policy-bytes','spaces','mailbox','foreign'])test('accepted provider transition remains minimal under complete account exclusion: '+reason,async t=>{
 const f=await setup(t);if(reason==='identities'||reason==='product')for(let n=0;n<(reason==='identities'?513:80);n++)f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'retained:'+n+'x'.repeat(30),'alice',at);
 if(reason==='policy-bytes')f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run(JSON.stringify(Array(30000).fill('read')),'api:alice');
 if(reason==='spaces')for(let n=0;n<513;n++)f.raw.prepare("INSERT INTO spaces VALUES(?,'Retained','alice',NULL,'managed',?,'session:alice')").run('raw:'+n,at);
 if(reason==='mailbox')f.raw.prepare("INSERT INTO account_emails VALUES('bad','alice',?,'example.com',?,NULL)").run('a\nb@example.com',at);
 if(reason==='foreign')f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://retained.example','opaque','alice',at);
 const event=f.event(1,'account.resumed');await f.deliver(event);assert.equal(f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='key:alice'").get().revoked_at,at);const core=bodies(f);assert.ok(core.some(b=>b.kind==='identity-transition-source'&&b.origin.eventId===event.id));assert.ok(core.every(b=>['identity-transition-source','subject-source-unrepresentable'].includes(b.kind)));assert.deepEqual(rows(f,'release_seoul_account_exclusions').map(r=>r.account_id),['alice']);assert.equal(rows(f,'release_seoul_dirty_spaces').length,0);clean(f);
});
...
test('two-event planner has bounded requests and indexed full-history and streaming Space probes',async t=>{
 ...
 const explain=piece=>{const q=request.statements.find(s=>s.sql.includes(piece));assert.ok(q,piece);return f.raw.prepare('EXPLAIN QUERY PLAN '+q.sql).all(...q.values).map(r=>r.detail);};
...
for(const reason of ['claims','memberships','credentials','huge-space-id','claim-product','policy-unsupported'])test('bounded retained superset excludes before normal JSON: '+reason,async t=>{
 ...
 if(reason==='claims')for(let n=0;n<513;n++)f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,?)').run('old:e'+n,'alice','old'+n+'@example.com','example.com',at-20,at-10);
 if(reason==='memberships')for(let n=0;n<513;n++)f.raw.prepare("INSERT INTO memberships VALUES(?,'org','alice','e1','member',9007199254740991,?)").run('old:m'+n,at-10);
 if(reason==='credentials')for(let n=0;n<513;n++)f.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,revoked_at,permission) VALUES(?,'alice','personal_key',?,?,?,'read')").run('old:k'+n,(n+100).toString(16).padStart(64,'0'),at+900000,at-10);
 if(reason==='huge-space-id')f.raw.prepare("INSERT INTO spaces VALUES(?,'Retained','alice',NULL,'managed',?,'session:alice')").run('x'.repeat(10000),at);
 if(reason==='claim-product'){for(let n=0;n<18;n++)f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'p'+n,'alice',at);for(let n=0;n<30;n++)f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,NULL)').run('e:'+n,'alice','a'+n+'@example.com','example.com',at);}
 if(reason==='policy-unsupported')f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run('["admin"]','api:alice');
 await f.verified([f.event(1,'account.resumed'),f.event(2,'email.verified')]);assert.ok(bodies(f).every(b=>['identity-transition-source','subject-source-unrepresentable'].includes(b.kind)));assert.equal(bodies(f).filter(b=>b.kind==='identity-transition-source').length,2);assert.equal(rows(f,'release_seoul_account_exclusions').length,1);assert.equal(rows(f,'release_seoul_dirty_spaces').length,0);assert.equal(f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='key:alice'").get().revoked_at,at);clean(f);
});
...
test('opaque JS-unit sorting and full canonical email key preserve every exact mapped alias',async t=>{
 ...
 for(const s of [opaque,'\ud800\udc00','\uE000'])f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,s,'alice',at);
 f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,NULL)').run('max:e','alice',mailbox,mailbox.split('@')[1],at);
...
test('unsupported original surrogate subject never becomes a fabricated normal or minimal identity',async t=>{
 const f=await setup(t,'ON',false),subject='opaque\uD800';f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at);await f.deliver(f.event(1,'account.resumed',subject));assert.equal(f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='key:alice'").get().revoked_at,at);
...
test('reverse JWT order uses real intermediate state and grouping history rejects update/delete/replacement',async t=>{
 ...
 const groups=rows(f,'release_seoul_provider_groups');assert.equal(groups.length,2);assert.throws(()=>f.raw.exec("UPDATE release_seoul_provider_groups SET ordinal=ordinal"),/immutable/);assert.throws(()=>f.raw.exec('DELETE FROM release_seoul_provider_groups'),/retained/);const g=groups[0];assert.throws(()=>f.raw.prepare('INSERT OR REPLACE INTO release_seoul_provider_groups VALUES(?,?,?,?)').run(g.outer_operation_id,g.ordinal,g.sub_attempt_id,g.publisher_event_id),/fresh accepted/);assert.deepEqual(rows(f,'release_seoul_provider_groups'),groups);clean(f);
});

test('late source failure and database-expired HMAC preserve original rollback and error mapping',async t=>{
 const f=await setup(t);f.raw.exec("CREATE TRIGGER late_provider_source BEFORE INSERT ON release_seoul_authority_changes WHEN NEW.stream_kind='subject' BEGIN SELECT RAISE(ABORT,'late provider source'); END");await assert.rejects(f.deliver(f.event(1,'account.resumed')),/late provider source/);assert.equal(rows(f,'release_webhook_events').length,0);assert.equal(rows(f,'release_seoul_provider_groups').length,0);assert.equal(f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='key:alice'").get().revoked_at,null);clean(f);
 f.db.setClock(()=>at+300001);await assert.rejects(f.deliver(f.event(2,'account.resumed')),e=>e.status===401);assert.equal(rows(f,'release_identity_lifecycle_events').length,0);clean(f);
});
...
test('new stale receipt in a two-triplet operation does not claim an accepted-advancement grouping',async t=>{
 ...
 const groups=rows(f,'release_seoul_provider_groups');assert.equal(groups.length,oldGroups.length+1);assert.equal(groups.at(-1).ordinal,1);assert.equal(groups.at(-1).publisher_event_id,'publisher:21');assert.ok(f.raw.prepare('SELECT id FROM release_identity_lifecycle_events WHERE id=?').get('publisher:19'));assert.ok(!bodies(f).some(b=>b.origin?.eventId==='publisher:19'));clean(f);
});
