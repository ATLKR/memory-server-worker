...
for (const expiry of ['credential', 'membership', 'proof']) test('queued SQL rejects expired ' + expiry + ' with bound application time unchanged', async () => {
  ...
  try {
    ...
    if (expiry === 'proof') (await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at - 300000 + 1));
    const space = expiry === 'membership' ? 'so' : 's1';
    (await db.raw.exec('CREATE TABLE queued_admissions(id TEXT PRIMARY KEY)'));
    ...
    assert.equal((await db.raw.prepare('SELECT count(*) AS n FROM queued_admissions').get()).n, 0);
  } finally { db.close(); }
});
...
for (const expiry of ['credential', 'membership']) test('SCIM-key revocation cannot cross queued ' + expiry + ' expiry', async () => {
  ...
  try {
    ...
    (await db.raw.prepare(expiry === 'credential'
      ? "UPDATE credentials SET expires_at=? WHERE id='session:alice'"
      : "UPDATE memberships SET expires_at=? WHERE id='m1'").run(at + 1));
    const observed = queueBeforeRun(db, sql => sql.startsWith('UPDATE memory_identity.scim_keys SET revoked_at='), () => { now = at + 2; });
    ...
    assert.equal((await db.raw.prepare('SELECT revoked_at FROM release_scim_keys WHERE id=?').get(key.id)).revoked_at, null);
  } finally { db.close(); }
});
...
test('OAuth policy registration cannot materialize an already expired credential after queuing', async () => {
  ...
  try {
    ...
    (await db.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,permission) VALUES('oauth:fixture','alice','session',?,?,'read')").run(await digest(token), at + 1));
    const observed = queueBeforeRun(db, sql => sql.startsWith('INSERT INTO memory_identity.credential_policies'), () => { now = at + 2; });
    ...
    assert.equal(observed(), true);
    assert.equal((await db.raw.prepare("SELECT count(*) AS n FROM release_credential_policies WHERE credential_id='oauth:fixture'").get()).n, 0);
  } finally { db.close(); }
});
...
test('manual retry leaves an ingest dead when its source deadline passes in the SQL queue', async () => {
  ...
  const { db, token } = await fixture({ clock: () => now });
  try {
    (await db.raw.prepare("INSERT INTO release_jobs(id,space_id,revision,kind,state,attempt,available_at,created_at) VALUES('queued-retry','s1',0,'ingest','dead',5,?,?)").run(at, at));
    (await db.raw.prepare("INSERT INTO release_ingests(id,account_id,space_id,actor_credential_id,ciphertext,proposals,state,expires_at,created_at) VALUES('queued-retry','alice','s1','session:alice','synthetic-source','[]','queued',?,?)").run(at + 1, at));
    const observed = queueBeforeRun(db, sql => sql.startsWith("UPDATE memory_jobs.release_jobs SET state='pending',attempt=0"), () => { now = at + 2; });
    ...
    assert.deepEqual({ ...(await db.raw.prepare("SELECT state,attempt FROM release_jobs WHERE id='queued-retry'").get()) }, { state: 'dead', attempt: 5 });
  } finally { db.close(); }
});
...
test('SQL clock advances after binding and conservatively retains an application clock ahead of D1', async () => {
  ...
  try {
    ...
    assert.equal((await db.raw.prepare("SELECT memory_control.now_ms() AS value").get()).value, at + 200);
  } finally { db.close(); }
});
