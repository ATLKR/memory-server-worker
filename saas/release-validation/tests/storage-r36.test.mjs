...
test('restore reconstructs retained trash in two empty recovered HOT databases before success', async t => {
    ...
    const original = (await f.db.raw.prepare('SELECT payload_id FROM memories WHERE id=?').get(memory.id)).payload_id;
    ...
    assert.equal((await f.db.raw.prepare('SELECT payload_id FROM memories WHERE id=?').get(memory.id)).payload_id, original);
...
test('restore projection failure or lost admission never publishes the recovered head: ' + boundary, async t => {
    ...
    const payloads = new PayloadStore(f.env, f.clock), store = new MemoryStore(f.db, f.clock, payloads), retained = (await f.db.raw.prepare('SELECT * FROM memories WHERE id=?').get(memory.id));
    ...
    payloads.stage = async (...args) => {
        ...
        await stage(...args);
        if (boundary === 'revocation-after-stage') { touched = true; (await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at)); }
    };
    if (boundary === 'credential-expiry') {
        (await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at + 5));
    ...
    const current = (await f.db.raw.prepare('SELECT revision,deleted_at,erased_at FROM memories WHERE id=?').get(memory.id));
...
test('update-only scoped credential restores the projection without gaining read capability', async t => {
    ...
    await f.store.remove(f.token, 's1', memory.id, 1, 'delete'); f.recoverHot();
    (await f.db.raw.exec("SET session_replication_role='replica'; UPDATE release_credential_policies SET capabilities='[\"update\"]' WHERE credential_id='key:alice'; SET session_replication_role='origin'"));
...
for (const route of ['list', 'export']) test('an erased raw first page still advances to the survivor: ' + route, async t => {
    ...
    const ordered = (await f.db.raw.prepare('SELECT id FROM memories ORDER BY id').all()), vanished = ordered[0].id, survivor = ordered[1].id;
    const transfers = new Transfers(f.db, f.clock, f.payloads), snapshot = await transfers.startExport(f.token, 's1');
    const read = f.payloads.read.bind(f.payloads); let fired = false;
...
for (const route of ['list', 'export']) test('erasure omission cannot bypass final authority on an empty page: ' + route, async t => {
    ...
    f.payloads.read = async (ctx, ref) => { if (!erased) { erased = true; await f.store.remove(f.token, 's1', memory.id, 1, 'delete');
        ...
        (await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at)); } return read(ctx, ref); };
...
test('changed retention policy prevents projection publication and retains restore_expired classification', async t => {
    ...
    await f.store.remove(f.token, 's1', memory.id, 1, 'delete'); f.recoverHot();
    (await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at + 40 * 86400000)); f.advance(2 * 86400000);
    ...
    payloads.read = async (...args) => { const value = await read(...args); changed = true;
        (await f.db.raw.prepare("INSERT INTO release_space_policies(space_id,retention_days) VALUES('s1',1) ON CONFLICT(space_id) DO UPDATE SET retention_days=1").run()); return value; };
    await assert.rejects(() => store.restore(f.token, 's1', memory.id, 2, 'restore'), error => error.code === 'restore_expired');
    assert.equal(changed, true); assert.equal(f.hotCount(), 0); assert.equal((await f.db.raw.prepare('SELECT revision FROM memories WHERE id=?').get(memory.id)).revision, 2);
});
