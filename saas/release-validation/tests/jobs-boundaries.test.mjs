...
for(const boundary of ['before-embedding','before-upsert'])for(const owner of ['account','organization'])test('disabled '+owner+' prevents new provider work '+boundary,async()=>{
 ...
 try {
  ...
  const disable=async ()=>(await db.raw.exec(owner==='account'?"UPDATE accounts SET disabled_at=1 WHERE id='alice'":"UPDATE organizations SET disabled_at=1 WHERE id='org'"));
  ...
  if(boundary==='before-embedding')disable();
   ...
   db.prepare= sql=>{const statement=prepare(sql);if(sql.startsWith('INSERT INTO memory_search.vector_refs')){const run=statement.run.bind(statement);statement.run=async()=>{const result=await run();await disable();return result;};}return statement;};
  }
...
for(const change of ['update','delete','erase','lease-expired','lease-replaced'])test('new vector upsert is fenced after '+change+' during reference persistence',async()=>{
 ...
 try {
  ...
  const prepare=db.prepare.bind(db);
  db.prepare= sql=>{const statement=prepare(sql);if(sql.startsWith('INSERT INTO memory_search.vector_refs')){const run=statement.run.bind(statement);statement.run=async()=>{
   const result=await run();if(!changed){changed=true;
    ...
    else if(change==='lease-replaced')(await db.raw.prepare('UPDATE release_jobs SET lease_token=? WHERE memory_id=?').run('replacement-owner',memory.id));
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_vector_refs WHERE memory_id=?').get(memory.id)).n,1,'Identifier-only reference remains available for cleanup');
  if(change==='lease-expired'){
   assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE memory_id=?').get(memory.id)).state,'leased','An expired owner cannot release its lease');
...
for(const change of ['logout','expiry','membership-revocation'])test('canonical organization indexing survives writer '+change,async()=>{
 ...
 try {
  ...
  if(change==='logout')(await db.raw.exec("UPDATE credentials SET revoked_at=1 WHERE id='session:alice'"));
  else if(change==='expiry')(await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at));
  else (await db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m1'").run(at));
  const env={DB:db,AI:{async run(){return{data:[Array(1024).fill(.1)]};}},MEMORY_INDEX:{async upsert(){upserts++;},async deleteByIds(){},async getByIds(){return[];}}};
