...
async function finish(jobs,db,advance,id) {
 for(let n=0;n<10;n++){
  if((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(id)).state==='done')return;
  advance(60000);await jobs.drain(5);
 }
 assert.fail('Bounded job failed to finish across scheduled invocations');
}

test('erasure checkpoints bounded pages and daily sweeps restart with retained identifiers',async t=>{
 ...
 try {
  ...
  for(let revision=1;revision<=250;revision++){
   ...
   for(let chunk=0;chunk<10;chunk++){
    ...
    (await db.raw.prepare('INSERT INTO release_vector_refs VALUES(?,?,?)').run(memory.id,vectorId,revision));p.vectors.add(vectorId);
   }
  }
  await store.remove(token,'s1',memory.id,250,'delete');(await db.raw.exec("UPDATE release_jobs SET state='done'"));
  await store.erase(token,'s1',memory.id,251,memory.id,'erase');
  const jobId=memory.id+':252';await p.jobs.drain(5);
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(jobId)).state,'pending','A large cleanup must yield before the scheduled deadline');
  assert.ok(now-at<520000);assert.equal((await db.raw.prepare('SELECT vector_erased_at FROM release_erasure_ledger').get()).vector_erased_at,null);
  assert.equal((await db.raw.prepare('SELECT attempt FROM release_jobs WHERE id=?').get(jobId)).attempt,0,'Useful continuation must not consume a failure');
  assert.ok((await db.raw.prepare('SELECT cleanup_cursor FROM release_jobs WHERE id=?').get(jobId)).cleanup_cursor);
  // Bounded resumed pages must still delete every retained identifier exactly
  // once across scheduled invocations.
...
test('upsert resumes completed chunks and rebuild restarts their progress',async()=>{
 ...
 try {
  ...
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(jobId)).state,'pending');
  assert.equal((await db.raw.prepare('SELECT next_chunk FROM release_jobs WHERE id=?').get(jobId)).next_chunk,7);
  ...
  assert.equal((await db.raw.prepare('SELECT next_chunk FROM release_jobs WHERE id=?').get(jobId)).next_chunk,0);
...
test('fast cleanup has a provider-call cap and retains earlier failure attempts when yielding',async()=>{
 ...
 try {
  ...
  await store.remove(token,'s1',memory.id,1,'delete');(await db.raw.exec("UPDATE release_jobs SET state='done'"));
  await store.erase(token,'s1',memory.id,2,memory.id,'erase');const jobId=memory.id+':3';
  for(let n=0;n<1100;n++)(await db.raw.prepare('INSERT INTO release_vector_refs VALUES(?,?,1)').run(memory.id,'vector-'+String(n).padStart(4,'0')));
  ...
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(jobId)).state,'pending');
  assert.equal((await db.raw.prepare('SELECT attempt FROM release_jobs WHERE id=?').get(jobId)).attempt,2);
 }finally{db.close();}
});

for(const failure of ['provider','replaced-lease'])test('cleanup checkpoint survives '+failure+' after an earlier confirmed page',async t=>{
 const {db,token}=await fixture();let now=at;
 try {
  const store=new MemoryStore(db,()=>at),memory=await store.create(token,'s1',{body:'private'},'create');
  await store.remove(token,'s1',memory.id,1,'delete');(await db.raw.exec("UPDATE release_jobs SET state='done'"));
  await store.erase(token,'s1',memory.id,2,memory.id,'erase');const jobId=memory.id+':3';
  for(let n=0;n<250;n++)(await db.raw.prepare('INSERT INTO release_vector_refs VALUES(?,?,1)').run(memory.id,'vector-'+String(n).padStart(4,'0')));
  ...
  const jobs=new Jobs({DB:db,MEMORY_INDEX:{async deleteByIds(ids){deleted.push(ids[0]);},async getByIds(){
   if(++pages===2&&fail){
    ...
    (await db.raw.prepare("UPDATE release_jobs SET lease_token='replacement-worker' WHERE id=?").run(jobId));
   }return[];
  }}},()=>now);
  await jobs.drain(1);
  const row=(await db.raw.prepare('SELECT state,attempt,cleanup_cursor,lease_token FROM release_jobs WHERE id=?').get(jobId));
