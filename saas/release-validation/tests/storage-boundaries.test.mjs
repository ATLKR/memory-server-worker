...
for(const change of ['credential','expiry','account','membership','share'])test('Space listing withholds metadata after '+change+' authority loss',async t=>{
 if(change==='share'){t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;}
 ...
 try {
  ...
  const changed=afterRows(db,'SELECT s.id,s.name',async ()=>{
   if(change==='credential')(await db.raw.exec("UPDATE credentials SET revoked_at=1 WHERE id='session:alice'"));
   if(change==='expiry')(await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at));
   if(change==='account')(await db.raw.exec("UPDATE accounts SET disabled_at=1 WHERE id='alice'"));
   if(change==='membership')(await db.raw.exec("UPDATE memberships SET revoked_at=1 WHERE id='m1'"));
   if(change==='share')(await db.raw.prepare('UPDATE release_shares SET revoked_at=1 WHERE id=?').run(shared.id));
  });
...
for(const boundary of ['before-embedding','before-index'])for(const change of ['credential','account','organization'])test('search prevents provider work after '+change+' loss '+boundary,async()=>{
 ...
 try {
  const mutate=async ()=>(await db.raw.exec(change==='credential'?"UPDATE credentials SET revoked_at=1 WHERE id='session:alice'":change==='account'?"UPDATE accounts SET disabled_at=1 WHERE id='alice'":"UPDATE organizations SET disabled_at=1 WHERE id='org'"));
...
for(const state of ['cancelled','expired','review','approved'])for(const boundary of ['before-claim','during-provider'])test('terminal '+state+' ingest stops automatic retries '+boundary,async()=>{
 ...
 try {
  const terminal=async ()=>{if(state==='expired')now=at+86400001;else (await db.raw.prepare('UPDATE release_ingests SET state=?,ciphertext=NULL WHERE id=?').run(state,ingestId));};
  ...
  assert.deepEqual({...(await db.raw.prepare('SELECT state,attempt,last_error FROM release_jobs WHERE id=?').get(ingestId))},{state:'done',attempt:boundary==='before-claim'?0:1,last_error:null});
 }finally{db.close();}
});

for(const change of ['share','grantor-membership'])test('invitations withhold source Space metadata after '+change+' loss',async t=>{
 ...
 try {
  ...
  const changed=afterRows(db,'SELECT sh.id,sh.space_id AS spaceId',async ()=>{
   if(change==='share')(await db.raw.prepare('UPDATE release_shares SET revoked_at=1 WHERE id=?').run(share.id));
   else (await db.raw.exec("UPDATE memberships SET revoked_at=1 WHERE id='m1'"));
  });
...
test('sharing to a disabled recipient returns the expected unavailable response',async()=>{
 ...
 try {
  (await db.raw.exec("UPDATE accounts SET disabled_at=1 WHERE id='bob'"));
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_shares').get()).n,0);
 }finally{db.close();}
});
