...
test('a completed checkout blocks a second subscription after local expiry and before webhook processing', async t => {
  ...
  assert.equal((await f.db.raw.prepare("SELECT count(*) AS n FROM release_billing_events WHERE subscription_id='sub_paid'").get()).n, 1);
  await f.billing.drain();
  assert.equal((await f.db.raw.prepare("SELECT subscription_id FROM release_pools WHERE id='account:alice'").get()).subscription_id, 'sub_paid');
...
test('replacement requires verified terminal provider state: '+status+'/'+subscriptionStatus,async t=>{
  ...
  const closed=(await f.db.raw.prepare('SELECT state,subscription_id FROM release_checkout_closures').get());
  ...
  for(const sql of ['UPDATE release_checkout_closures SET checked_at=checked_at+1','DELETE FROM release_checkout_closures'])
    await assert.rejects(async()=>(await f.db.raw.exec(sql)),/immutable/);
});
...
for(const state of ['open','unrecognized'])test('provider '+state+' never frees a locally expired checkout',async t=>{
  ...
  assert.equal(f.creations(),1);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_checkout_closures').get()).n,0);
});

test('unknown lost checkout attempts stay blocked after their local expiry',async t=>{
  const f=await setup(t);
  (await f.db.raw.prepare('UPDATE release_checkout_requests SET session_id=NULL,checkout_url=NULL').run());
...
test('replacement rechecks authorization after awaiting provider closure evidence',async t=>{
  ...
  f.env.fetch=async(url,init)=>{const result=await fetcher(url,init);if(String(url).endsWith('/checkout/sessions/cs_1'))(await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(f.now));return result;};
...
test('replacement insertion checks a concurrent unresolved attempt after closure reads',async t=>{
  ...
  f.db.prepare= sql=>{const statement=prepare(sql);if(sql.startsWith('INSERT INTO memory_control.checkout_requests')){const run=statement.run.bind(statement);statement.run=async()=>{if(!injected){injected=true;(await f.db.raw.prepare(`INSERT INTO release_checkout_requests(id,pool_id,price_id,operation_key,created_at,expires_at)
...
test('historical checkout reconciliation is bounded and continues on the next retry',async t=>{
  const f=await setup(t,'expired'),fetcher=f.env.fetch;
  for(let i=2;i<=7;i++)(await f.db.raw.prepare(`INSERT INTO release_checkout_requests(id,pool_id,price_id,operation_key,created_at,expires_at,session_id)
  ...
  assert.equal(reads,7);assert.equal(f.creations(),2);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_checkout_closures').get()).n,7);
});
...
test('concurrent replacements create at most one new checkout',async t=>{
  ...
  f.db.prepare= sql=>{const statement=prepare(sql);if(sql.startsWith('INSERT INTO memory_control.checkout_requests')){const run=statement.run.bind(statement);statement.run=async()=>{if(!winner){winner=Promise.resolve();winner=await f.billing.checkout(f.token,'s1','price_test','replacement-b');}return run();};}return statement;};
