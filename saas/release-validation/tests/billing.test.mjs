...
for(const phase of ['cached','provider'])for(const change of ['membership','recent proof'])test('checkout rechecks '+change+' before returning a '+phase+' URL',async t=>{
 ...
 const invalidate=async ()=>{if(change==='membership')(await db.raw.prepare('UPDATE memberships SET revoked_at=? WHERE id=?').run(now,'m1'));else now+=300001;};
 const env={DB:db,BACKGROUND_JOBS_ENABLED:'true',STRIPE_SECRET_KEY:'synthetic',STRIPE_WEBHOOK_SECRET:'w'.repeat(64),STRIPE_API_VERSION:'synthetic',BILLING_PRICES_JSON:JSON.stringify({price_test:{plan:'team',monthlyUnits:10000,storageBytes:1048576000}}),fetch:async(url)=>{
  ...
  if(armed&&phase==='provider')await invalidate();
 ...
 if(phase==='cached'){
  ...
  db.prepare= sql=>{const statement=prepare(sql);if(sql.includes('FROM memory_control.checkout_requests WHERE pool_id=?')){const first=statement.first.bind(statement);statement.first=async()=>{const result=await first();await invalidate();return result;};}return statement;};
 }
 armed=true;
...
for(const [name,changes] of [
 ...
 if(enabled){
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) AS n FROM release_checkout_requests').get()).n,0);
  assert.equal((await db.raw.prepare("SELECT customer_id FROM release_pools WHERE id='account:alice'").get()).customer_id,null);
  (await db.raw.prepare("UPDATE release_pools SET customer_id='cus_test' WHERE id='account:alice'").run());
...
for(const enabled of [undefined,'false'])test('billing checkout and portal reject disabled processing before provider calls: '+enabled,async t=>{
 ...
 (await db.raw.prepare('UPDATE release_pools SET customer_id=? WHERE id=?').run('cus_test','account:alice'));
 ...
 assert.equal((await billing.webhook(request())).status,200);
 assert.equal((await db.raw.prepare("SELECT count(*) AS n FROM release_billing_events WHERE state='pending'").get()).n,1);
});
for(const name of ['checkout-idempotent','bad-plan','bad-webhook','current-subscription','cancelled-subscription','duplicate-event','lease-fencing'])test('billing '+name,async()=>{
 ...
 if(name==='bad-webhook'){await assert.rejects(()=>billing.webhook(event()),e=>e.status===401);return;}await billing.webhook(event());if(name==='duplicate-event')await billing.webhook(event());if(name==='cancelled-subscription')status='canceled';await billing.drain();const pool=(await db.raw.prepare("SELECT * FROM release_pools WHERE id='account:alice'").get());
 if(name==='lease-fencing'){assert.equal(pool.plan,'free');return;}assert.equal(pool.plan,name==='cancelled-subscription'?'free':'team');if(name==='duplicate-event')assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_billing_events').get()).n,1);
 }finally{db.close();}
});
test('billing periodic reconciliation repairs missing subscription webhooks',async()=>{
 const {db}=await fixture();try{
  (await db.raw.exec(`UPDATE release_pools SET customer_id='cus_test',subscription_id='sub_test',plan='team',updated_at=${at-7200000} WHERE id='account:alice'`));
  ...
  const b=new Billing(env,()=>at);assert.equal(typeof b.reconcile,'function');await b.reconcile();await b.reconcile();assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_billing_events WHERE state='pending'").get()).n,1);
  await b.drain();assert.equal((await db.raw.prepare("SELECT plan FROM release_pools WHERE id='account:alice'").get()).plan,'free');
 }finally{db.close();}
});
