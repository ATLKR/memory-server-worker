...
for(const name of ['encrypted-source','extract-review','approval-retry','fabricated-evidence','revoked-actor','expiry'])test('ingest '+name,async()=>{
 ...
 if(name==='encrypted-source'){const row=(await db.raw.prepare('SELECT ciphertext FROM release_ingests').get());assert.ok(!row.ciphertext.includes('연구'));assert.equal((await pipeline.submit(token,'s1',input,'ingest-one')).id,started.id);return;}
 if(name==='revoked-actor')(await db.raw.exec("UPDATE credentials SET revoked_at="+at+" WHERE id='session:alice'"));
 if(name==='expiry')time=at+86400001;
 await jobs.drain(1);
 if(['fabricated-evidence','revoked-actor','expiry'].includes(name)){assert.notEqual((await db.raw.prepare('SELECT state FROM release_ingests').get()).state,'review');assert.equal((await db.raw.prepare('SELECT count(*) n FROM memories').get()).n,0);return;}
 const result=await pipeline.get(token,'s1',started.id);assert.equal(result.state,'review');assert.equal((await db.raw.prepare('SELECT count(*) n FROM memories').get()).n,0);
 const approved=await pipeline.approve(token,'s1',started.id,[0],'approval-one');assert.equal(approved.memories.length,1);assert.equal((await db.raw.prepare('SELECT ciphertext FROM release_ingests').get()).ciphertext,null);
 if(name==='approval-retry'){await pipeline.approve(token,'s1',started.id,[0],'approval-one');assert.equal((await db.raw.prepare('SELECT count(*) n FROM memories').get()).n,1);await assert.rejects(()=>pipeline.approve(token,'s1',started.id,[],'approval-two'),e=>e.status===409);}
 }finally{db.close();}
});
...
for(const phase of ['queued','source-read','provider']) test('organization ingest revocation is fenced at '+phase,async()=>{
 ...
 try {
  ...
  (await db.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,permission) VALUES('org-ingest-key','alice','m1','e1','api_key',?,?,'write')").run(await digest(key),at+900000));
  (await db.raw.exec("INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES('org-ingest-key','[\"read\",\"create\"]','[\"so\"]')"));
  const revoke=async ()=>(await db.raw.prepare('UPDATE memberships SET revoked_at=? WHERE id=?').run(at,'m1'));
  ...
  const row=(await db.raw.prepare('SELECT ciphertext,proposals,state FROM release_ingests WHERE id=?').get(submitted.id));
  assert.equal(row.proposals,null);assert.notEqual(row.state,'review');assert.ok(!row.ciphertext.includes('private source'));
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM memories').get()).n,0);
...
test('expired extraction quotes are hidden even when cleanup cron is delayed',async()=>{
 const {db,token}=await fixture();try{(await db.raw.exec(`UPDATE credentials SET expires_at=${at+172800000} WHERE id='session:alice'`));(await db.raw.prepare("INSERT INTO release_jobs(id,space_id,revision,kind,state,available_at,created_at) VALUES('expired-ingest','s1',0,'ingest','done',?,?)").run(at,at));(await db.raw.prepare("INSERT INTO release_ingests(id,account_id,space_id,actor_credential_id,proposals,state,expires_at,created_at) VALUES('expired-ingest','alice','s1','session:alice',?,'review',?,?)").run(JSON.stringify([{body:'private',quote:'expired source',sourceMessageId:'u1',kind:'fact'}]),at+86400000,at));
