...
async function after(db,match,callback,method='run'){const prepare=db.prepare.bind(db);db.prepare= sql=>{const s=prepare(sql),original=s[method].bind(s);s[method]=async()=>{const result=await original();if(match(sql))await callback();return result;};return s;};}
...
for(const kind of ['reauth','email_link'])for(const change of ['recipient','proof','expiry'])test('mail rejects obsolete '+kind+' '+change+' after budget reservation',async t=>{
 ...
 after(f.db,sql=>sql.includes('INSERT INTO memory_ops.mail_budget'),async ()=>{
  if(change==='expiry')now=at+600001;
  else if(change==='proof')(await f.db.raw.exec(kind==='reauth'?'UPDATE memory_identity.reauth_challenges SET used_at=1':'UPDATE memory_identity.email_challenges SET invalidated_at=1'));
  else if(kind==='reauth')(await f.db.raw.exec("UPDATE memory_identity.account_emails SET revoked_at=1 WHERE id='e1'"));
  else (await f.db.raw.exec("INSERT INTO memory_identity.external_email_blocks(account_id,address,created_at) VALUES('alice','new@example.com',1)"));
 });
...
for(const permission of [['write'],{toString:'write'},null,1,true])test('PAT rejects non-string permission '+JSON.stringify(permission),async t=>{
 const f=await fixture();t.after(()=>f.db.close());const before=(await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n;
 ...
 assert.equal(response.status,400);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n,before);
});
for(const input of [{Operations:[{op:'replace',path:'active',value:false}]},{schemas:[patchSchema],Operations:[{op:['replace'],path:'active',value:false}]},{schemas:[patchSchema],Operations:[{op:{toString:'replace'},path:'active',value:false}]}])test('SCIM rejects malformed envelope before mutation '+JSON.stringify(input),async t=>{
 ...
 assert.equal(response.status,400);assert.equal((await f.db.raw.prepare("SELECT revoked_at FROM memberships WHERE id='m2'").get()).revoked_at,null);
});
...
for(const [path,data,table] of [['/v1/keys',{label:'null caps',capabilities:null,expiresInDays:1},'credentials'],['/v1/spaces/s1/shares',{email:'bob@example.com',days:null},'release_shares'],['/v1/spaces/s1/memories',{body:'null operation identifier',operationId:null},'memories']])test('explicit null is not an omitted optional input: '+path,async t=>{
 const f=await fixture();t.after(()=>f.db.close());const before=(await f.db.raw.prepare('SELECT count(*) n FROM '+table).get()).n,response=await createRelease({DB:f.db},{clock:()=>at}).route(request(path,'POST',data),f.token);
 assert.equal(response.status,400);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM '+table).get()).n,before);
});
for(const kind of ['reauth','email_link'])for(const boundary of ['session','proof','recent'])test('mail compares '+kind+' '+boundary+' expiry after its final query',async t=>{
 const f=await fixture();t.after(()=>f.db.close());let now=at,sent=0;
 after(f.db,sql=>sql.includes('INSERT INTO memory_ops.mail_budget'),async ()=>{if(boundary==='session')(await f.db.raw.prepare("UPDATE memory_identity.credentials SET expires_at=? WHERE id='session:alice'").run(at+100));if(boundary==='recent')(await f.db.raw.prepare("UPDATE memory_identity.credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));});
 after(f.db,sql=>sql.includes('least(c.expires_at,c.membership_expires_at,p.expires_at)'),()=>{now=at+(boundary==='proof'?600001:200);},'first');
 const env={DB:f.db,MAIL_FROM:'memory@example.com',EMAIL:{send:async()=>{sent++;return {messageId:'test'};}}},release=createRelease(env,{clock:()=>now,identity:new IdentityService(f.db,()=>now)});
 const response=await release.route(request(kind==='reauth'?'/v1/account/reauth':'/v1/account/emails','POST',kind==='reauth'?{emailId:'e1'}:{email:'new@example.com'}),f.token);
...
test('email-link destination cannot become claimed while reserving delivery budget',async t=>{
 const f=await fixture();t.after(()=>f.db.close());let sent=0;after(f.db,sql=>sql.includes('INSERT INTO memory_ops.mail_budget'),async ()=>(await f.db.raw.prepare('INSERT INTO memory_identity.account_emails VALUES(?,?,?,?,?,NULL)').run('taken','bob','new@example.com','example.com',at)));
...
test('domain revocation during the mail budget invalidates the proof without dispatch or cleanup failure',async t=>{
 ...
 (await f.db.raw.prepare('INSERT INTO memory_identity.domains VALUES(?,?,?,?,NULL)').run('domain','org','example.com',at+86400000));(await f.db.raw.exec("INSERT INTO memory_identity.domain_managers VALUES('domain','m1',NULL)"));
 after(f.db,sql=>sql.includes('INSERT INTO memory_ops.mail_budget'),async ()=>(await f.db.raw.prepare("INSERT INTO memory_identity.revocations(id,kind,actor_credential_id,domain_id,address,created_at) VALUES('blocked','domain','session:alice','domain','new@example.com',?)").run(at)));
 const env={DB:f.db,MAIL_FROM:'memory@example.com',EMAIL:{send:async()=>{sent++;return {messageId:'test'};}}},release=createRelease(env,{clock:()=>at,identity:new IdentityService(f.db,()=>at)});
 const response=await release.route(request('/v1/account/emails','POST',{email:'new@example.com'}),f.token);assert.equal(response.status,403);assert.equal(sent,0);assert.equal((await f.db.raw.prepare('SELECT invalidated_at FROM memory_identity.email_challenges').get()).invalidated_at,at);
});
for(const name of [['_memory-verification.example.com.'],{toString:'_memory-verification.example.com.'}])test('DNS proof answer requires a string owner name '+JSON.stringify(name),async t=>{
 ...
 await assert.rejects(()=>admin.verifyDomain(f.token,challenge.id),e=>e.status===409&&e.code==='dns_proof_missing');assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM domains').get()).n,0);
});
