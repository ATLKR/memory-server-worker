...
for(const data of [
 ...
 const f=await setup(t),key=await new Admin({DB:f.db},()=>at).issueScimKey(f.token,'org'),response=await f.request('/scim/v2/org/Users/m2',data,key.token,'PATCH');assert.equal(response.status,400);assert.equal((await f.db.raw.prepare("SELECT revoked_at FROM memberships WHERE id='m2'").get()).revoked_at,null);
});
for(const email of ['not-an-email','.start@example.com','two..dots@example.com','local@-example.com','local@example..com','é@example.com','x'.repeat(65)+'@example.com'])for(const path of ['/v1/organizations/org/invites','/v1/account/emails','/v1/domains/domain/emails/revoke','/v1/spaces/s1/shares'])test('malformed email gives400 '+path+' '+email,async t=>{
 const f=await setup(t),response=await f.request(path,{email,...(path.endsWith('/invites')?{role:'member'}:{})});assert.equal(response.status,400);assert.equal(f.sent(),0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM email_challenges').get()).n,0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM workspace_invitations').get()).n,0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM revocations').get()).n,0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_shares').get()).n,0);
});
test('valid address normalization keeps plus/dot distinctions and authority/proof failures remain403',async t=>{
 ...
 const invite=await f.request('/v1/organizations/org/invites',{email:' New.Tag+keep@EXAMPLE.COM ',role:'member'});assert.equal(invite.status,201);assert.equal((await f.db.raw.prepare('SELECT address FROM workspace_invitations').get()).address,'new.tag+keep@example.com');
 assert.equal((await f.request('/v1/organizations/org/invites',{email:'valid@example.com',role:'member'},f.other)).status,403);
 (await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-300001));assert.equal((await f.request('/v1/account/emails',{email:'valid@example.com'})).status,403);assert.equal(f.sent(),0);
});
test('SCIM accepts escaped case-equivalent attribute names without changing their values',async t=>{
 ...
 assert.equal((await f.request('/scim/v2/org/Users/m2',data,key.token,'PATCH')).status,204);assert.equal((await f.db.raw.prepare("SELECT revoked_at FROM memberships WHERE id='m2'").get()).revoked_at,at);
});
test('case-equivalent SCIM requests still enforce owner protection',async t=>{
 const f=await setup(t),key=await new Admin({DB:f.db},()=>at).issueScimKey(f.token,'org');assert.equal((await f.request('/scim/v2/org/Users/m1',{SCHEMAS:[schema],OPERATIONS:[{OP:'replace',VALUE:{ACTIVE:false}}]},key.token,'PATCH')).status,403);assert.equal((await f.db.raw.prepare("SELECT revoked_at FROM memberships WHERE id='m1'").get()).revoked_at,null);
});
test('share-route email validation preserves normalization and current authority/proof checks',async t=>{
 ...
 const f=await setup(t);const response=await f.request('/v1/spaces/s1/shares',{email:' BOB@EXAMPLE.COM '});assert.equal(response.status,201);assert.equal((await f.db.raw.prepare('SELECT recipient_email_id FROM release_shares').get()).recipient_email_id,'e2');
 ...
 (await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-300001));assert.equal((await f.request('/v1/spaces/s1/shares',{email:'bob@example.com'})).status,403);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_shares').get()).n,1);
});
for(const email of ['.start@example.com','local@example..com','x'.repeat(65)+'@example.com'])test('signed email-revocation payload uses the same mailbox validation: '+email,async t=>{
 ...
 await assert.rejects(async()=>admin.identityWebhook(new Request(PUBLIC_ORIGIN+'/webhooks/identity',{method:'POST',body:raw,headers:{'x-memory-timestamp':timestamp,'x-memory-signature':await hmac(secret,timestamp+'.'+raw)}})),e=>e.status===400&&e.code==='invalid_email');assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_webhook_events').get()).n,0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_provider_revocations').get()).n,0);
});
