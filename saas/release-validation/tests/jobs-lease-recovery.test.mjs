...
async function enqueue(db,id,kind='upsert') {
 (await db.raw.prepare('INSERT INTO release_jobs(id,space_id,revision,kind,available_at,created_at) VALUES(?,\'s1\',0,?,?,?)').run(id,kind,at,at));
 if(kind==='ingest')(await db.raw.prepare("INSERT INTO release_ingests(id,account_id,space_id,actor_credential_id,ciphertext,expires_at,created_at) VALUES(?,'alice','s1','session:alice','encrypted',?,?)").run(id,at+86400000,at));
}
...
test('invalid extraction key does not block independent work: '+JSON.stringify(invalidKey),async()=>{
 ...
 try {
  ...
  assert.deepEqual({...(await db.raw.prepare("SELECT state,attempt,lease_token FROM release_jobs WHERE id='a-ingest'").get())},
...
for(const kind of ['upsert','delete','ingest'])test(kind+' lease recovery stops after five abandoned attempts',async()=>{
 ...
 try {
  ...
  const failed=(await db.raw.prepare('SELECT state,attempt,last_error,lease_token,lease_until FROM release_jobs WHERE id=?').get('abandoned'));
  ...
  (await db.raw.prepare("UPDATE release_jobs SET state='pending',attempt=0,available_at=?,last_error=NULL WHERE id='abandoned'").run(now));
...
test('exhausted lease cleanup is bounded and does not block later eligible work',async()=>{
 ...
 try {
  ...
  (await db.raw.prepare("UPDATE release_jobs SET state='leased',attempt=5,lease_token='lost-worker',lease_until=?").run(at));
  enqueue(db,'ready');
  const jobs=worker(db,()=>at);
  assert.equal((await jobs.claim()).id,'ready');
  assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE state='dead'").get()).n,100);
  assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE state='leased' AND attempt=5").get()).n,5);
  assert.equal(await jobs.claim(),null);
  assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE state='dead'").get()).n,105);
 }finally{db.close();}
});
...
for(const state of ['cancelled','review','approved','expired'])test('terminal '+state+' ingest wins over exhausted lease failure',async()=>{
 ...
 try {
  ...
  (await db.raw.prepare("UPDATE release_jobs SET state='leased',attempt=5,lease_token='lost-worker',lease_until=? WHERE id='terminal'").run(at));
  (await db.raw.prepare('UPDATE release_ingests SET state=? WHERE id=?').run(state,'terminal'));
  assert.equal(await worker(db,()=>at).claim(),null);
  assert.deepEqual({...(await db.raw.prepare("SELECT state,attempt,last_error,lease_token,lease_until FROM release_jobs WHERE id='terminal'").get())},
