...
for(const state of ['cancelled','missing','approved','other-account','revoked-before-update'])test('ingest cancellation returns a truthful authorized result for '+state,async()=>{
 ...
 try{
  ...
  if(state==='revoked-before-update'){
   ...
   db.prepare= sql=>{const statement=prepare(sql);if(sql.startsWith('UPDATE memory_content.release_ingests SET ciphertext=NULL')){const run=statement.run.bind(statement);statement.run=async()=>{(await db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at));return run();};}return statement;};
  }
  ...
  const current=(await db.raw.prepare('SELECT state,ciphertext FROM release_ingests WHERE id=?').get(started.id));
  assert.equal(current.state,state==='approved'?'approved':state==='cancelled'?'cancelled':'queued');
  assert.equal((await db.raw.prepare('SELECT count(*) AS n FROM memories').get()).n,state==='approved'?1:0);
 }finally{db.close();}
});
...
test('exhausted ingestion exposes failed status and manual retry returns it to queued',async()=>{
 ...
 try {
  ...
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(started.id)).state,'dead');
...
for(const terminal of ['cancelled','expired'])test('failed ingestion can become '+terminal+' without exposing retained source',async()=>{
 ...
 try {
  ...
  if(terminal==='cancelled'){
   ...
   assert.equal((await db.raw.prepare('SELECT ciphertext FROM release_ingests WHERE id=?').get(started.id)).ciphertext,null);
  }else {
   (await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+172800000));now=at+86400001;
  }
...
for(const state of ['queued','review','failed','approved','cancelled','expired'])test('create-only ingest submission replay reports current '+state+' without source access',async()=>{
 ...
 try {
  (await db.raw.exec("SET session_replication_role='replica'; UPDATE release_credential_policies SET capabilities='[\"create\"]' WHERE credential_id='key:alice'; SET session_replication_role='origin'"));
  (await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id IN ('session:alice','key:alice')").run(at+172800000));
  ...
  const units=(await db.raw.prepare('SELECT sum(units) n FROM release_usage_events').get()).n;
  assert.deepEqual(await ingest.submit(key,'s1',input,'submit'),{id:started.id,state,replayed:true});
  assert.equal((await db.raw.prepare('SELECT sum(units) n FROM release_usage_events').get()).n,units);
...
for(const kind of ['upsert','delete'])test('manual retry still accepts a dead '+kind+' indexing job',async()=>{
 ...
 try {
  (await db.raw.prepare('INSERT INTO release_jobs(id,space_id,revision,kind,state,attempt,available_at,created_at) VALUES(?,?,?,?,?,?,?,?)').run('index-job','s1',1,kind,'dead',5,at,at));
  ...
  assert.deepEqual({...(await db.raw.prepare("SELECT state,attempt FROM release_jobs WHERE id='index-job'").get())},{state:'pending',attempt:0});
 }finally{db.close();}
});
...
for(const terminal of ['cancelled','expired','missing-source','cancelled-during-retry'])test('dead ingestion retry rejects '+terminal+' source atomically',async()=>{
 const {db,token}=await fixture();let now=at;
 try {
  (await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+172800000));
  const env={DB:db,PUBLIC_ORIGIN:'https://memory.example.com',REQUEST_LIMITER:{async limit(){return{success:true};}},BACKGROUND_JOBS_ENABLED:'true',PAYLOAD_KEY:Buffer.alloc(32,3).toString('base64url'),AI:{async run(){throw Error('offline');}}};
  const ingest=new Ingest(env,()=>now),jobs=new Jobs(env,()=>now),release=createRelease(env,{clock:()=>now});jobs.ingest=job=>ingest.process(job);
  const started=await ingest.submit(token,'s1',{messages:[{id:'msg1',role:'user',content:'Private source'}]},'submit');
  for(let n=0;n<5;n++){await jobs.drain(1);now+=100000;}
  const cancel=async ()=>(await db.raw.prepare("UPDATE release_ingests SET state='cancelled',ciphertext=NULL,proposals=NULL WHERE id=?").run(started.id));
  if(terminal==='cancelled')await cancel();
  if(terminal==='expired')now=at+86400001;
  if(terminal==='missing-source')(await db.raw.prepare('UPDATE release_ingests SET ciphertext=NULL WHERE id=?').run(started.id));
  if(terminal==='cancelled-during-retry'){
   const prepare=db.prepare.bind(db);
   db.prepare=sql=>{const statement=prepare(sql);if(sql.startsWith("UPDATE memory_jobs.release_jobs SET state='pending',attempt=0")){const run=statement.run.bind(statement);statement.run=async()=>{await cancel();return run();};}return statement;};
  }
  (await db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(now));
  ...
  assert.deepEqual({...(await db.raw.prepare('SELECT state,attempt FROM release_jobs WHERE id=?').get(started.id))},{state:'dead',attempt:5});
 }finally{db.close();}
});
