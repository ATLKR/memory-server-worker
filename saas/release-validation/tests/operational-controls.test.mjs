...
const totals = async db => (await db.raw.prepare('SELECT coalesce(sum(calls),0) calls,coalesce(sum(reserved_microusd),0) reserved FROM release_provider_budgets').get());
function afterReservation(db, action) {
  const prepare = db.prepare.bind(db); let calls = 0;
  db.prepare =  sql => {
    const s = prepare(sql);
    if (sql.startsWith('INSERT INTO memory_ops.provider_budgets')) {
...
test('invite gate uses verified canonical mailbox hashes without alias folding', async t => {
  ...
  assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM provider_identities').get()).n, 0, 'Admission itself creates no identity or access');
});
test('exact mapped active identities are grandfathered without trusting a changed token email', async t => {
  const f = await setup(t);
  (await f.db.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(AUTH_ISSUER, 'existing', 'alice', at));
...
test('a roster entry never admits a disabled mapped account or revives it', async t => {
  ...
  (await f.db.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(p.issuer, p.subject, 'alice', at));
  ...
  assert.equal((await f.db.raw.prepare("SELECT disabled_at FROM accounts WHERE id='alice'").get()).disabled_at, at);
});
...
test('signed OAuth signup admission runs before account, Space, or credential creation', async t => {
  ...
  const before = await Promise.all(['accounts', 'spaces', 'credentials', 'provider_identities'].map(async table => (await f.db.raw.prepare('SELECT count(*) n FROM ' + table).get()).n));
  assert.equal((await request()).status, 401);
  assert.deepEqual(await Promise.all(['accounts', 'spaces', 'credentials', 'provider_identities'].map(async table => (await f.db.raw.prepare('SELECT count(*) n FROM ' + table).get()).n)), before);
  env.ENROLLMENT_EMAIL_HASHES_JSON = JSON.stringify([await digest(p.email)]);
  assert.equal((await request()).status, 200);
  assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM provider_identities WHERE subject=?').get(p.subject)).n, 1);
  env.ENROLLMENT_EMAIL_HASHES_JSON = '[]';
  assert.equal((await request()).status, 200, 'Existing admission is not an ongoing ACL');
  (await f.db.raw.prepare('UPDATE accounts SET disabled_at=? WHERE id=(SELECT account_id FROM provider_identities WHERE subject=?)').run(at, p.subject));
  assert.equal((await request()).status, 401);
});
test('the primary sign-in trigger also catches account disablement after admission', async t => {
  ...
  (await f.db.raw.prepare("UPDATE accounts SET disabled_at=? WHERE id='alice'").run(at));
  const count = (await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n;
  await assert.rejects(() => new WorkspaceService(f.db, f.clock).signIn(p));
  assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM credentials').get()).n, count);
});
test('provider reservations atomically share one cap across kinds and concurrent callers', async t => {
  ...
  assert.deepEqual({ ...(await totals(f.db)) }, { calls: 2, reserved: 30200 });
});
test('provider month changes at the UTC execution-time boundary and retains old reservations', async t => {
  ...
  assert.deepEqual((await f.db.raw.prepare('SELECT month,reserved_microusd amount FROM release_provider_budgets ORDER BY month').all()).map(r => ({ ...r })),
...
for (const budget of [undefined, '', '-1', '1.0', '001', 'NaN', '1000000000', [200], 200])
  test('GA rejects malformed or missing provider budget: ' + JSON.stringify(budget), async t => {
    ...
    assert.equal((await totals(f.db)).calls, 0);
  });
...
test('provider retries reserve again after an uncertain failure and never refund it', async t => {
  ...
  assert.equal(calls, 2); assert.deepEqual({ ...(await totals(f.db)) }, { calls: 2, reserved: 400 });
});
test('embedding byte bound rejects multibyte oversize before reservation or provider dispatch', async t => {
  ...
  assert.equal(calls, 1); assert.equal((await totals(f.db)).reserved, 200);
});
...
test('search rechecks revocation after the budget await and retries do not buy another embedding', async t => {
  ...
  const count = afterReservation(f.db, async () => (await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at)));
  ...
  assert.equal(calls, 1); assert.equal((await totals(f.db)).calls, 2);
});
test('index jobs recheck the current source after the budget await before disclosing plaintext', async t => {
  ...
  assert.equal(count(), 1); assert.equal(calls, 0); assert.equal((await totals(f.db)).reserved, 200);
});
test('ingestion rechecks current authority after the budget await and preserves the reservation', async t => {
  ...
  const count = afterReservation(f.db, async () => (await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m1'").run(at)));
  await assert.rejects(() => ingest.process(job), rejected('ingest_authority_expired'));
  assert.equal(count(), 1); assert.equal(calls, 0); assert.equal((await totals(f.db)).reserved, 30000);
});
test('extraction bounds the source and output before buying provider work', async t => {
  ...
  assert.equal((await totals(f.db)).reserved, 30000);
});
