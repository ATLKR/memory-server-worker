...
async function afterFirst(db,match,change){const prepare=db.prepare.bind(db);db.prepare= sql=>{const s=prepare(sql);if(match(sql)){const first=s.first.bind(s);s.first=async()=>{const result=await first();await change(result);return result;};}return s;};}
...
for(const expiry of ['credential','membership','lease','ingest'])test('ingest discloses no source when '+expiry+' expires during the final authorized read',async()=>{
 ...
 try{
  ...
  if(expiry==='credential')(await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+1));
  if(expiry==='membership')(await db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+1));
  if(expiry==='lease')(await db.raw.prepare('UPDATE release_jobs SET lease_until=? WHERE id=?').run(at+1,request.id));
  if(expiry==='ingest')(await db.raw.prepare('UPDATE release_ingests SET expires_at=? WHERE id=?').run(at+1,request.id));
...
for(const stage of ['embedding','upsert'])test('indexing starts no '+stage+' after the final eligibility read crosses lease expiry',async()=>{
 ...
 try{
  ...
  afterFirst(db,sql=>sql.startsWith('SELECT EXISTS('),async ()=>{if(stage==='embedding'||calls.includes('embedding'))now=(await db.raw.prepare('SELECT lease_until FROM release_jobs WHERE id=?').get(job.id)).lease_until+1;});
  await assert.rejects(()=>jobs.index(job),/lease_lost/);assert.deepEqual(calls,stage==='embedding'?[]:['embedding']);
  assert.equal((await db.raw.prepare('SELECT revision FROM memories WHERE id=?').get(memory.id)).revision,1);
 }finally{db.close();}
});
...
for(const stage of ['delete','confirm'])test('vector cleanup starts no '+stage+' call after lease expiry',async()=>{
 ...
 try{
  ...
  (await db.raw.prepare("UPDATE release_jobs SET state='done' WHERE revision=1").run());(await db.raw.prepare('INSERT INTO release_vector_refs VALUES(?,?,1)').run(memory.id,'old-vector'));
  const env={DB:db,MEMORY_INDEX:{async deleteByIds(){calls.push('delete');if(stage==='confirm')now=(await db.raw.prepare('SELECT lease_until FROM release_jobs WHERE id=?').get(memory.id+':2')).lease_until+1;},async getByIds(){calls.push('confirm');return[];}}};
  const jobs=new Jobs(env,()=>now),job=await jobs.claim();
  if(stage==='delete'){let renewals=0;const prepare=db.prepare.bind(db);db.prepare= sql=>{const s=prepare(sql);if(sql.startsWith('UPDATE memory_jobs.release_jobs SET lease_until=greatest(')){const run=s.run.bind(s);s.run=async()=>{const result=await run();if(++renewals===2)now=(await db.raw.prepare('SELECT lease_until FROM release_jobs WHERE id=?').get(job.id)).lease_until+1;return result;};}return s;};}
...
for(const expiry of ['credential','membership'])test('Space listing filters '+expiry+' expiry crossed during its final snapshot',async()=>{
 ...
 try{
  ...
  afterFirst(db,sql=>sql.includes('"spaceIds"'),()=>{now=at+2;});
...
for(const expiry of ['credential','grantor','share'])test('invitation listing filters '+expiry+' expiry crossed during its final snapshot',async t=>{
 if(expiry!=='credential'){t.skip('cross-border share federation is deferred (0011); share INSERTs are denied');return;}
 const {db,token,other}=await fixture();let now=at;
 try{
  const transfers=new Transfers(db,()=>now);
  (await db.raw.prepare('UPDATE credentials SET expires_at=?').run(at+2*86400000));
  (await db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:bob'").run(at+1));
  afterFirst(db,sql=>sql.includes('"shareIds"'),()=>{now=at+2;});
  await assert.rejects(()=>transfers.invitations(other),error=>error.status===403);
 }finally{db.close();}
});
