...
test('sharded update resolves a winner between receipt and revision reads: ' + variant, async t => {
        ...
        if (sql.includes('WHERE r.id=? AND s.id=? AND r.revision=?')) {
            ...
            statement.first = async () => {
                if (!fired) {
                    ...
                    if (variant === 'revoked') (await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at));
                }
                return first();
            };
        }
    ...
    assert.equal((await f.db.raw.prepare("SELECT count(*) n FROM release_operations WHERE action='update'").get()).n, 1);
    assert.equal((await f.db.raw.prepare('SELECT revision FROM memories WHERE id=?').get(memory.id)).revision, 2);
    assert.equal((await f.db.raw.prepare("SELECT count(*) n FROM release_payload_intents WHERE action='update'").get()).n, 1);
});
