...
test('daily soft-delete cleanup removes an upsert that finished after deletion',async()=>{
 ...
 try {
  ...
  assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE state='done'").get()).n,2);
  now+=3*86400000;await jobs.maintain();await jobs.drain(5);assert.equal(vectors.size,0);
  const current=(await db.raw.prepare('SELECT body,deleted_at,erased_at FROM memories').get());
  ...
  assert.equal((await db.raw.prepare('SELECT count(*) n FROM release_vector_refs').get()).n,1);
...
test('soft-delete sweep waits a day from confirmed completion and preserves active leases',async()=>{
 ...
 try {
  ...
  now=at+86400000+4000;await jobs.maintain();assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(jobId)).state,'done');
  now+=1001;await jobs.maintain();assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(jobId)).state,'pending');
  const lease=await jobs.claim();(await db.raw.prepare('UPDATE release_jobs SET cleanup_cursor=? WHERE id=?').run('confirmed-page',jobId));
  now+=86400000;await jobs.maintain();
  const row=(await db.raw.prepare('SELECT state,lease_token,cleanup_cursor FROM release_jobs WHERE id=?').get(jobId));
...
test('soft-delete sweep is bounded and excludes memories that were restored',async()=>{
 ...
 try {
  ...
  const firstCount=(await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE kind='delete' AND state='pending'").get()).n;
  assert.ok(firstCount>=99&&firstCount<=100,'The raw page contains at most one restored memory');
  assert.equal((await db.raw.prepare('SELECT state FROM release_jobs WHERE id=?').get(memories[0].id+':2')).state,'done');
  await jobs.maintain();assert.equal((await db.raw.prepare("SELECT count(*) n FROM release_jobs WHERE kind='delete' AND state='pending'").get()).n,105);
 }finally{db.close();}
});
