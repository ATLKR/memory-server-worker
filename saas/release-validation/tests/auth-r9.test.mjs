...
async function afterQuery(db,match,callback,method='first'){const wrap=d=>{const prepare=d.prepare.bind(d);d.prepare=sql=>{const statement=prepare(sql),read=statement[method].bind(statement);statement[method]=async()=>{const result=await read();if(match(sql))await callback();return result;};return statement;};};wrap(db);if(typeof db.withSession==='function'){const withSession=db.withSession.bind(db);db.withSession=c=>{const d=withSession(c);wrap(d);return d;};}}
...
for(const boundary of ['credential','membership'])for(const operation of ['get','create readback','update readback','Space readback','Spaces','list','search'])test('foundation '+operation+' checks final '+boundary+' expiry',async t=>{
 ...
 if(boundary==='credential')(await f.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='s-alice'").run(NOW+100));else (await f.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m-work'").run(NOW+100));
 afterQuery(f.db,sql=>sql.includes('SELECT ')&&sql.includes('active_credentials'),async()=>{now=NOW+200;await f.setNow(NOW+200);},['Spaces','list','search'].includes(operation)?'all':'first');
...
for(const boundary of ['credential','membership'])test('billing usage checks final '+boundary+' expiry',async t=>{
 ...
 (await f.db.raw.prepare(boundary==='credential'?"UPDATE credentials SET expires_at=? WHERE id='session:alice'":"UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+100));
 afterQuery(f.db,sql=>sql.includes('AS "grantExpiresAt"'),()=>{now=at+200;});
...
for(const boundary of ['session revocation','membership revocation','recent proof'])test('portal rechecks '+boundary+' after customer lookup before provider',async t=>{
 let now=at,calls=0;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());
 (await f.db.raw.prepare("UPDATE release_pools SET customer_id='cus_test' WHERE id='org:org'").run());
 if(boundary==='recent proof')(await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));
 afterQuery(f.db,sql=>sql.includes('p.customer_id AS "customerId"'),async ()=>{if(boundary==='session revocation')(await f.db.raw.prepare("UPDATE credentials SET revoked_at=? WHERE id='session:alice'").run(at));else if(boundary==='membership revocation')(await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m1'").run(at));else now=at+200;});
...
for(const operation of ['checkout','customer','portal'])test(operation+' checks recent proof after final Space authority read before provider',async t=>{
 let now=at,calls=0,armed=false;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());
 if(operation!=='customer')(await f.db.raw.prepare("UPDATE release_pools SET customer_id='cus_test' WHERE id='account:alice'").run());
 (await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));
 afterQuery(f.db,sql=>operation==='checkout'?sql.includes('checkout_attempted AS "checkoutAttempted"'):operation==='portal'?sql.includes('p.customer_id AS "customerId"'):sql.includes('FROM memory_control.checkout_requests WHERE pool_id=?'),()=>{armed=true;});
...
for(const location of ['member data','final caller read'])for(const detail of [false,true])test('SCIM active recalculates target expiry at '+location+'; detail='+detail,async t=>{
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const admin=new Admin({DB:f.db},()=>now),key=await admin.issueScimKey(f.token,'org');
 (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m2'").run(at+100));let dataRead=false;
...
for(const elapsed of [200,600])test('billing usage preserves independent shares only while their grantor authority remains live: '+elapsed,async t=>{
 ...
 let now=at;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const transfers=new Transfers(f.db,()=>now),share=await transfers.share(f.token,'so','bob@example.com',1);await transfers.accept(f.other,share.id);
 (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m2'").run(at+100));(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+500));
...
for(const boundary of ['HMAC','body hash','receipt read'])test('billing webhook rejects timestamp expiry during '+boundary,async t=>{
 ...
 assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_billing_events').get()).n,0);assert.equal((await f.db.raw.prepare('SELECT count(*) n FROM release_webhook_events').get()).n,0);
});
for(const boundary of ['lease claim','job read'])test('billing reconciliation stops before provider when lease expires during '+boundary,async t=>{
 let now=at,calls=0;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());(await f.db.raw.prepare('INSERT INTO release_billing_events(id,subscription_id,available_at,created_at) VALUES(?,?,?,?)').run('evt_pending','sub_test',at,at));
 afterQuery(f.db,sql=>boundary==='lease claim'?sql.includes('UPDATE memory_ops.billing_lock SET token='):sql.includes('AS "subscriptionId"')&&sql.includes('memory_ops.billing_events'),()=>{now=at+120001;});
 await new Billing(billingEnv(f.db,async()=>{calls++;return Response.json({id:'sub_test'});}),()=>now).drain(1);assert.equal(calls,0);assert.equal((await f.db.raw.prepare("SELECT attempt FROM release_billing_events WHERE id='evt_pending'").get()).attempt,0);
});
for(const boundary of ['session','recent proof'])for(const location of ['challenge write','final snapshot'])test('foundation email delivery rechecks '+boundary+' at '+location,async t=>{
 const f=await createFixture();t.after(()=>f.close());let now=NOW,sent=0;if(boundary==='session')(await f.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='s-alice'").run(NOW+100));else (await f.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='s-alice'").run(NOW-299900));
 afterQuery(f.db,sql=>location==='challenge write'?sql.includes('INSERT INTO memory_identity.email_challenges'):sql.includes('FROM memory_identity.email_challenges p JOIN memory_identity.active_credentials c'),async()=>{now=NOW+200;await f.setNow(NOW+200);},location==='challenge write'?'run':'first');
 ...
 assert.ok((await f.raw.prepare('SELECT invalidated_at FROM email_challenges').get()).invalidated_at);
});
for(const boundary of ['session','membership','recent proof','challenge'])test('domain DNS lookup waits for current '+boundary+' authority',async t=>{
 let now=at,calls=0;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());const admin=new Admin({DB:f.db,fetch:async()=>{calls++;return Response.json({Status:0,Answer:[]});}},()=>now),challenge=await admin.beginDomain(f.token,'org','example.com');
 let challengeId=challenge.id;
 if(boundary==='session')(await f.db.raw.prepare("UPDATE credentials SET expires_at=? WHERE id='session:alice'").run(at+100));
 if(boundary==='membership')(await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at+100));
 if(boundary==='recent proof')(await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));
 // expires_at is immutable on a recorded challenge, so an already-expired
 // challenge is seeded as issued long enough ago to have lapsed.
 if(boundary==='challenge'){challengeId='expired-challenge';(await f.db.raw.prepare('INSERT INTO release_domain_challenges(id,organization_id,actor_account_id,domain,proof,expires_at) VALUES(?,?,?,?,?,?)').run(challengeId,'org','alice','example.net','memory-verification=expired',at+100));}
 afterQuery(f.db,sql=>sql.includes('SELECT organization_id AS "organizationId",domain,proof'),()=>{now=at+200;});
 await assert.rejects(()=>admin.verifyDomain(f.token,challengeId));assert.equal(calls,0);
});

test('email-link delivery keeps its recent-proof requirement through the mail budget await',async t=>{
 let now=at,sent=0;const f=await fixture({clock:()=>now});t.after(()=>f.db.close());(await f.db.raw.prepare("UPDATE credentials SET reauthenticated_at=? WHERE id='session:alice'").run(at-299900));
 afterQuery(f.db,sql=>sql.includes('INSERT INTO memory_ops.mail_budget'),()=>{now=at+200;},'run');
 ...
 assert.equal(response.status,403);assert.equal(sent,0);assert.ok((await f.db.raw.prepare('SELECT invalidated_at FROM email_challenges').get()).invalidated_at);
});
