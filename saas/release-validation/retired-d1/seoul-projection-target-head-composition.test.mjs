...
async function central(t,recursive){
 ...
 for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql','0028_seoul-projection-bootstrap-schema.sql','0029_seoul-projection-preparation-schema.sql','0030_seoul-projection-workspace-schema.sql','0031_seoul-projection-provider-schema.sql','0032_seoul-projection-command-schema.sql','0033_seoul-projection-provider-v1-schema.sql','0034_seoul-projection-target-schema.sql','0035_seoul-projection-target-manifest-schema.sql'])(await f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8')));
 assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,35);assert.deepEqual((await f.raw.prepare('PRAGMA foreign_key_check').all()),[]);(await f.raw.exec('PRAGMA recursive_triggers='+recursive));
...
for(const recursive of ['ON','OFF'])for(const reversed of [false,true])test(`persisted target writer/preparer/HMAC/SQL receipts preserve identity negatives; reversed=${reversed}, recursion ${recursive}`,async t=>{
 ...
 (await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(?,'credential','unrelated:credential',?,?,?)").run(uuid(100),uuid(101),JSON.stringify(negative),at));
 ...
 const centralBefore=(await f.raw.prepare('SELECT * FROM release_seoul_authority_changes ORDER BY revision').all());for(const [i,command]of commands.entries())assert.deepEqual(await f.writer.setTarget(command),{status:'already_committed',receipt:receipts[i]});assert.deepEqual((await f.raw.prepare('SELECT * FROM release_seoul_authority_changes ORDER BY revision').all()),centralBefore);assert.deepEqual((await f.raw.prepare('SELECT * FROM release_seoul_target_attempt').all()),[]);assert.deepEqual((await f.raw.prepare('SELECT * FROM release_seoul_target_preparation_stage').all()),[]);assert.deepEqual((await f.raw.prepare('PRAGMA foreign_key_check').all()),[]);
});
