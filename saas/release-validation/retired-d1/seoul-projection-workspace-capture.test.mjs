...
for(const recursive of ['ON','OFF'])for(const command of ['invite','member','issue','revoke'])test('captured dirty revision survives later Workspace '+command+'; '+recursive,async t=>{
 const f=await setup(t,recursive);api(f,'api:after-capture');const invite=command==='invite'?await invitation(f):null,space=command==='issue'?'s1':'so',prior=(await f.raw.prepare('SELECT max(revision) n FROM release_seoul_authority_changes').get()).n;
 ...
 const actual=(await f.raw.prepare('SELECT * FROM release_seoul_dirty_spaces WHERE space_id=?').get(space)),latest=Math.max(...sources(f).map(r=>r.revision));assert.ok(latest>prior);assert.equal(actual.dirty_revision,latest);assert.equal(actual.captured_revision,prior);clean(f);
});
...
async function clean(f){for(const table of stage)assert.equal(rows(f,table).length,0,table);for(const r of f.calls)if(r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_workspace_attempts'))){assert.ok(r.statements.length<=100);assert.ok(Buffer.byteLength(JSON.stringify(r))<=1048576);}assert.equal((await f.raw.prepare('SELECT state FROM release_seoul_projection_state').get()).state,'backfill-required');}
async function setup(t,recursive='ON',mapBob=true){
 const f=await fixture();t.after(()=>f.db.close());f.raw=f.db.raw;
 for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql','0028_seoul-projection-bootstrap-schema.sql','0029_seoul-projection-preparation-schema.sql'])(await f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8')));
 assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,29);const schema=new URL('../../seoul-projection-workspace-schema.sql',import.meta.url);assert.ok(existsSync(schema),'new additive workspace schema must exist');
 const migration=readFileSync(new URL('../../migrations/0030_seoul-projection-workspace-schema.sql',import.meta.url),'utf8');assert.equal(migration,'-- Forward SaaS migration 30: seoul-projection-workspace-schema.sql\n'+readFileSync(schema,'utf8'));(await f.raw.exec(migration));assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,30);(await f.raw.exec('PRAGMA recursive_triggers='+recursive));
 ...
 for(const account of mapBob?['alice','bob']:['alice'])(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,account,account,at));
...
async function api(f,id,n=1,policy=true){(await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,permission) VALUES(?,'bob','m2','e2','api_key',?,?,'read')").run(id,n.toString(16).padStart(64,'0'),at+900000));if(policy)(await f.raw.prepare("INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES(?,'[\"read\"]',NULL)").run(id));}
async function invitation(f){(await f.raw.exec("UPDATE memberships SET revoked_at="+(at-1)+" WHERE id='m2'"));return f.workspace.createInvite(f.token,'org',{email:'bob@example.com',role:'member'});}
async function assertOrigin(f,command,receiptTable){const core=bodies(f);assert.ok(core.length);for(const body of core){assert.equal(body.origin.commandType,command);assert.ok((await f.raw.prepare('SELECT id FROM '+receiptTable+' WHERE id=?').get(body.origin.receiptId)));}return core;}
...
for(const recursive of ['ON','OFF'])test('another account admin revokes exact member and all API children, never personal key; recursion '+recursive,async t=>{
 ...
 const core=assertOrigin(f,'membership-revoke','workspace_membership_revocations');assert.deepEqual(core.map(b=>b.id).sort(),['api:one','api:two','m2']);assert.ok(core.every(b=>b.accountId==='bob'&&b.revokedAtMs===at&&b.effect.occurredAtMs===at));assert.equal((await f.raw.prepare('SELECT revoked_at FROM credentials WHERE id=?').get(personal.id)).revoked_at,null);assert.deepEqual(rows(f,'release_seoul_dirty_spaces').map(r=>r.space_id),['so']);clean(f);
...
for(const recursive of ['ON','OFF'])for(const org of [false,true])test('legacy Workspace '+(org?'API':'personal')+' issue retains policy null, receipt and PAT response; recursion '+recursive,async t=>{
 const f=await setup(t,recursive);const key=await f.workspace.issueKey(f.token,{label:'Legacy',permission:'read',expiresInDays:2,...(org?{organizationId:'org'}:{})});assert.deepEqual(Object.keys(key),['id','token','expiresAt']);assert.equal(key.expiresAt,at+2*86400000);const [body]=assertOrigin(f,'workspace-key-issue','workspace_key_issuances');assert.equal(sources(f).length,1);assert.equal(body.id,key.id);assert.equal(body.origin.receiptId,key.id);assert.equal(body.credentialKind,org?'api_key':'personal_key');assert.equal(body.policy,null);assert.equal((await f.raw.prepare('SELECT count(*) n FROM release_credential_policies WHERE credential_id=?').get(key.id)).n,0);assert.ok(!JSON.stringify(sources(f)).includes(key.token));assert.deepEqual(rows(f,'release_seoul_dirty_spaces').map(r=>r.space_id),[org?'so':'s1']);clean(f);
});
...
for(const recursive of ['ON','OFF'])for(const command of ['invite','member','issue','revoke'])test('late '+command+' failure rolls back original command, cascades, audit and capture; recursion '+recursive,async t=>{
 ...
 (await f.raw.exec("CREATE TRIGGER workspace_capture_late BEFORE DELETE ON release_seoul_workspace_attempts BEGIN SELECT RAISE(ABORT,'workspace capture late'); END"));
...
for(const reason of ['foreign','unmapped','credentials','bytes','spaces','space-id'])test('valid target revoke preserves central success with complete exclusion for '+reason,async t=>{
 const f=await setup(t,'ON',reason!=='unmapped');api(f,'api:target');if(reason==='foreign')(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://foreign.example','foreign','bob',at));
 if(reason==='credentials')for(let n=2;n<=513;n++)api(f,'api:'+n,n);
 if(reason==='bytes')(await f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run(JSON.stringify(Array(30000).fill('read')),'api:target'));
...
test('raw exact-organization Space and membership-key discovery use bounded indexed plans',async t=>{
 ...
 const probe=request.statements.find(s=>s.sql.includes('SET space_count='));const plan=(await f.raw.prepare('EXPLAIN QUERY PLAN '+probe.sql).all(...probe.values)).map(r=>r.detail);assert.ok(plan.some(r=>r.includes('release_spaces_organization_created')),JSON.stringify(plan));assert.ok(!plan.some(r=>r.includes('MERGE (UNION)')||r.includes('UNION USING TEMP')||r.includes('ORDER BY')),JSON.stringify(plan));
 const keys=request.statements.find(s=>s.sql.includes('SET credential_count=')),keyPlan=(await f.raw.prepare('EXPLAIN QUERY PLAN '+keys.sql).all(...keys.values)).map(r=>r.detail);assert.ok(keyPlan.some(r=>r.includes('release_seoul_workspace_live_credentials')&&r.includes('revoked_at=?')),JSON.stringify(keyPlan));clean(f);
});

test('new staging never reuses an unexpected retained attempt after a zero-row scope',async t=>{
 const f=await setup(t);(await f.raw.exec("INSERT INTO release_seoul_workspace_attempts(id,command_kind,receipt_id,entity_id) VALUES('00000000-0000-4000-8000-000000000001','workspace-key-revoke','stale','missing')"));const before=rows(f,'release_seoul_workspace_attempts');await assert.rejects(f.workspace.revokeKey(f.token,'missing'),WorkspaceError);assert.deepEqual(rows(f,'release_seoul_workspace_attempts'),before);assert.equal(sources(f).length,0);
});

test('actual ordered 0030 preserves populated 0029 tables, objects and source history',async t=>{
 const f=await fixture();t.after(()=>f.db.close());f.raw=f.db.raw;for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql','0028_seoul-projection-bootstrap-schema.sql','0029_seoul-projection-preparation-schema.sql'])(await f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8')));
 (await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES('retained','credential','old','00000000-0000-4000-8000-000000000001','{}',?)").run(at));(await f.raw.prepare("INSERT INTO release_seoul_account_exclusions VALUES('bob','unmapped','00000000-0000-4000-8000-000000000002',0,0,?)").run(at));
 const snapshot=async table=>(await f.raw.prepare('SELECT * FROM '+table).all()).map(r=>JSON.stringify(r)).sort();const objects=(await f.raw.prepare("SELECT name,type,sql FROM sqlite_master WHERE name NOT LIKE 'sqlite_%' ORDER BY name").all());const before=new Map(objects.filter(o=>o.type==='table'&&o.name!=='release_meta').map(o=>[o.name,snapshot(o.name)]));
 (await f.raw.exec(readFileSync(new URL('../../migrations/0030_seoul-projection-workspace-schema.sql',import.meta.url),'utf8')));for(const [table,data] of before)assert.deepEqual(snapshot(table),data,table);for(const object of objects)assert.deepEqual((await f.raw.prepare('SELECT name,type,sql FROM sqlite_master WHERE name=?').get(object.name)),object);assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,30);assert.deepEqual((await f.raw.prepare('PRAGMA foreign_key_check').all()),[]);
});
...
test('unsupported target policy and retained mailbox never poison accepted key revocation',async t=>{
 for(const reason of ['policy-count','policy-id','mailbox']){
  const f=await setup(t);api(f,'api:target');if(reason==='policy-count')(await f.raw.prepare('UPDATE release_credential_policies SET space_ids=? WHERE credential_id=?').run(JSON.stringify(Array.from({length:51},(_,i)=>'s:'+i)),'api:target'));if(reason==='policy-id')(await f.raw.prepare('UPDATE release_credential_policies SET space_ids=? WHERE credential_id=?').run(JSON.stringify(['s'.repeat(129)]),'api:target'));
  if(reason==='mailbox'){(await f.raw.prepare("INSERT INTO account_emails VALUES('retained:e','bob',?,'example.com',?,NULL)").run('a\nb@example.com',at));(await f.raw.exec("INSERT INTO organizations(id) VALUES('retained:org'); INSERT INTO memberships VALUES('retained:m','retained:org','bob','retained:e','member',9007199254740991,NULL)"));(await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,permission) VALUES('retained:k','bob','retained:m','retained:e','api_key',?,?,'read')").run('f'.repeat(64),at+900000));}
  const target=reason==='mailbox'?'retained:k':'api:target';await f.workspace.revokeKey(f.other,target);assert.equal((await f.raw.prepare('SELECT revoked_at FROM credentials WHERE id=?').get(target)).revoked_at,at);assert.deepEqual(rows(f,'release_seoul_account_exclusions').map(r=>r.account_id),['bob']);assert.deepEqual(bodies(f).map(b=>b.kind),['subject-source-unrepresentable']);clean(f);
 }
});

test('large revoked child history is neither recaptured nor counted as effective fanout',async t=>{
 const f=await setup(t);for(let n=1;n<=600;n++)api(f,'old:'+n,n,false);(await f.raw.exec("UPDATE credentials SET revoked_at="+(at-1)+" WHERE membership_id='m2'"));api(f,'api:live',601);
 await f.workspace.revokeMembership(f.token,'org','m2');assert.deepEqual(bodies(f).map(b=>b.id).sort(),['api:live','m2']);assert.equal(rows(f,'release_seoul_account_exclusions').length,0);assert.equal((await f.raw.prepare("SELECT count(*) n FROM credentials WHERE membership_id='m2' AND revoked_at=?").get(at-1)).n,600);clean(f);
});
...
for(const command of ['invite','issue','revoke'])test('unsupported mapping exclusion uses the actual '+command+' target and preserves response',async t=>{
 const f=await setup(t);const target=command==='issue'?'alice':'bob';(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://retained.example','opaque',target,at));
 ...
 if(command==='issue'){const key=await f.workspace.issueKey(f.token,{label:'Retained',permission:'read',expiresInDays:1});assert.ok((await f.raw.prepare('SELECT id FROM credentials WHERE id=?').get(key.id)));}
 if(command==='revoke'){api(f,'api:target');await f.workspace.revokeKey(f.token,'api:target');assert.equal((await f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='api:target'").get()).revoked_at,at);}
