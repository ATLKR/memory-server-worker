...
for(const recursive of ['ON','OFF'])test('captured dirty revision survives later bootstrap claim creation; '+recursive,async t=>{
 const f=await setup(t,recursive);(await f.raw.prepare("INSERT INTO account_emails VALUES('captured:email','alice','after-capture@example.com','example.com',?,NULL)").run(at-2));(await f.raw.prepare("INSERT INTO memberships VALUES('captured:member','org','alice','captured:email','member',?,?)").run(at+10000,at-1));(await f.raw.prepare("UPDATE account_emails SET revoked_at=? WHERE id='captured:email'").run(at-1));
 (await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES('captured:seed','target','so',?,'{}',?)").run(crypto.randomUUID(),at));const prior=(await f.raw.prepare('SELECT max(revision) n FROM release_seoul_authority_changes').get()).n;
 (await f.raw.prepare('INSERT INTO release_seoul_dirty_spaces(space_id,dirty_revision,captured_revision) VALUES(?,?,?)').run('so',prior,prior));
 const result=await f.workspace.signIn(principal('alice',{emailVerified:true,email:'after-capture@example.com'}));assert.equal(result.accountId,'alice');const actual=(await f.raw.prepare("SELECT * FROM release_seoul_dirty_spaces WHERE space_id='so'").get()),latest=Math.max(...sources(f).map(r=>r.revision));assert.ok(latest>prior);assert.equal(actual.dirty_revision,latest);assert.equal(actual.captured_revision,prior);clean(f);
});
...
async function setup(t,recursive='ON'){
 ...
 for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql'])(await f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8')));
 assert.ok(existsSync(schema),'additive bootstrap staging schema must exist');const migration=readFileSync(new URL('../../migrations/0028_seoul-projection-bootstrap-schema.sql',import.meta.url),'utf8');assert.equal(migration,'-- Forward SaaS migration 28: seoul-projection-bootstrap-schema.sql\n'+readFileSync(schema,'utf8'));(await f.raw.exec(migration));assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,28);(await f.raw.exec('PRAGMA recursive_triggers='+recursive));
 const calls=[];const storage={sql:{exec(sql,...values){const statement=(await f.raw.prepare(sql)),result=statement.all(...values),readonly=statement.columns().length>0&&/^(SELECT|WITH|PRAGMA)\b/i.test(sql.trim());return {rowsWritten:readonly?0:Number((await f.raw.prepare('SELECT changes() n').get()).n),toArray:()=>result,[Symbol.iterator]:()=>result[Symbol.iterator]()};}},transactionSync(fn){(await f.raw.exec('BEGIN IMMEDIATE'));try{const r=fn();(await f.raw.exec('COMMIT'));return r;}catch(e){(await f.raw.exec('ROLLBACK'));throw e;}}};
 const engine=new SqlDatabaseEngine(storage,{objectName:'sql:staging:control:1'});engine.initialize();(await f.raw.prepare('INSERT INTO durable_sql_state VALUES(1,?,?,?,?,?,?,?)').run('staging','control','control',1,'ready','a'.repeat(64),'b'.repeat(64)));
 ...
 (await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'alice','alice',at));
 return f;
}
...
for(const recursive of ['ON','OFF'])for(const mode of ['absent','blocked','already-claimed','created'])test('first sign-in captures exact bootstrap and optional email '+mode+'; recursion '+recursive,async t=>{
 ...
 const subject=core.find(r=>r.kind==='subject-source');assert.equal(subject.accountId,result.accountId);assert.deepEqual(subject.providerIdentities,[{issuer,subject:'new\nsubject',accountId:result.accountId,createdAtMs:at}]);assert.deepEqual(subject.effect,{type:'entity-head',disposition:'changed'});assert.equal(subject.origin.commandType,'workspace-sign-in');assert.ok((await f.raw.prepare('SELECT id FROM workspace_sign_ins WHERE id=?').get(subject.origin.receiptId)));
 ...
 const request=f.calls.find(r=>r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_bootstrap_attempts')));assert.ok(request.statements.length<=100);assert.ok(Buffer.byteLength(JSON.stringify(request))<=1048576);assert.equal((await f.raw.prepare('SELECT state FROM release_seoul_projection_state').get()).state,'backfill-required');
});
...
for(const recursive of ['ON','OFF'])test('retained linked subjects use one complete JS UTF16-sorted set and all claim email keys; recursion '+recursive,async t=>{
 const f=await setup(t,recursive),subjects=['alice','a\nopaque','\uE000','😀'];for(const subject of subjects.slice(1))(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at));
 (await f.raw.exec('CREATE TABLE bootstrap_observed(before_bytes TEXT,after_bytes TEXT); CREATE TRIGGER bootstrap_observe BEFORE DELETE ON release_seoul_bootstrap_scope WHEN OLD.stream_kind=\'subject\' BEGIN INSERT INTO bootstrap_observed VALUES(OLD.before_bytes,OLD.after_bytes); END'));
...
test('retained lifecycle state copied by a new mapping has generic command effect',async t=>{
 const f=await setup(t);(await f.raw.prepare("INSERT INTO release_webhook_events VALUES('identity','retained-resume',?,?)").run('c'.repeat(64),at));(await f.raw.prepare("INSERT INTO release_identity_lifecycle_events VALUES('retained-resume',?,'resumed-subject',1,'account.resumed','',?,?,?,?)").run(issuer,at-1,at,at,'c'.repeat(64)));
...
test('complete identity array repetition overflow excludes a successful claim without partial normal sources',async t=>{
 const f=await setup(t);const identities=['alice',...Array.from({length:79},(_,n)=>'linked:'+String(n).padStart(3,'0')+'x'.repeat(20))];for(const subject of identities.slice(1))(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at));assert.ok(Buffer.byteLength(JSON.stringify(identities.map(subject=>({issuer,subject,accountId:'alice',createdAtMs:at}))))<131072);
 const result=await f.workspace.signIn(principal('alice',{emailVerified:true,email:'overflow@example.com'}));assert.equal(result.accountId,'alice');assert.ok((await f.raw.prepare("SELECT id FROM account_emails WHERE address='overflow@example.com'").get()));assert.deepEqual(bodies(f).map(r=>r.kind),['subject-source-unrepresentable']);assert.equal(rows(f,'release_seoul_account_exclusions').length,1);assert.equal(rows(f,'release_seoul_dirty_spaces').length,0);clean(f);
...
test('existing foreign candidate Space cannot be classified as newly created bootstrap authority',async t=>{
 const f=await setup(t);const original=f.capture.workspaceBootstrap.bind(f.capture);let candidate;f.capture.workspaceBootstrap=async (database,scope,...args)=>{candidate=scope.spaceId;(await f.raw.prepare("INSERT INTO spaces VALUES(?,?,'bob',NULL,'managed',?,'session:bob')").run(candidate,'Foreign candidate',at));return original(database,scope,...args);};
...
test('raw Space discovery streams its bounded union and deduplicates retained claim dependencies',async t=>{
 const f=await setup(t);for(let n=0;n<2;n++){(await f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,NULL)').run('historical-email:'+n,'alice','old@example.com','example.com',at-100));(await f.raw.prepare('INSERT INTO memberships VALUES(?,?,?,?,?,?,?)').run('historical-member:'+n,'org','alice','historical-email:'+n,'member',at+10000,at-50));(await f.raw.prepare('UPDATE account_emails SET revoked_at=? WHERE id=?').run(at-50,'historical-email:'+n));}
 (await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES('setup-target','target','so','00000000-0000-4000-8000-000000000001','{}',?)").run(at));(await f.raw.exec("INSERT INTO release_seoul_targets SELECT 'so',1,max(revision) FROM release_seoul_authority_changes"));
 ...
 const request=f.calls.find(r=>r.statements.some(s=>s.sql.includes('INSERT INTO release_seoul_bootstrap_attempts')));const probe=request.statements.find(s=>s.sql.includes('SET space_count='));const plan=(await f.raw.prepare('EXPLAIN QUERY PLAN '+probe.sql).all(...probe.values)).map(r=>r.detail);assert.ok(!plan.some(line=>line.includes('UNION USING TEMP B-TREE')||line.includes('MERGE (UNION)')||line.includes('ORDER BY')),JSON.stringify(plan));assert.ok(plan.some(line=>line.includes('spaces_account')||line.includes('release_spaces_account_created')),JSON.stringify(plan));
});
...
test('unsupported retained account and foreign first mapping preserve central bootstrap without partial source state',async t=>{
 const f=await setup(t);(await f.raw.prepare('INSERT INTO accounts(id) VALUES(?)').run('retained account'));(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'retained','retained account',at));
 ...
 const foreign=await f.workspace.signIn(principal('foreign',{issuer:'https://retained.example'}));assert.ok((await f.raw.prepare('SELECT id FROM accounts WHERE id=?').get(foreign.accountId)));assert.equal(rows(f,'release_seoul_account_exclusions').length,2);assert.equal(sources(f).length,0);clean(f);
});
...
for(const oversized of [false,true])test('raw unselected Space '+(oversized?'ID bytes':'count')+' excludes successful bootstrap before staging',async t=>{
 const f=await setup(t);const count=oversized?1:513;for(let n=0;n<count;n++)(await f.raw.prepare("INSERT INTO spaces VALUES(?,'Retained','alice',NULL,'managed',?,'session:alice')").run(oversized?'s'.repeat(129):'raw:'+n,at));
 const result=await f.workspace.signIn(principal('alice',{emailVerified:true,email:'space-bound@example.com'}));assert.equal(result.accountId,'alice');assert.ok((await f.raw.prepare("SELECT id FROM account_emails WHERE address='space-bound@example.com'").get()));assert.deepEqual(bodies(f).map(r=>r.kind),['subject-source-unrepresentable']);assert.equal(rows(f,'release_seoul_account_exclusions')[0].reason,oversized?'unsupported-state':'fanout-count');assert.equal(rows(f,'release_seoul_dirty_spaces').length,0);clean(f);
});

test('additive migration preserves 0027 source/exclusion rows and refuses unexpected staging without deletion',async t=>{
 const f=await fixture();t.after(()=>f.db.close());f.raw=f.db.raw;for(const file of ['0026_seoul-projection-schema.sql','0027_seoul-projection-capture-schema.sql'])(await f.raw.exec(readFileSync(new URL('../../migrations/'+file,import.meta.url),'utf8')));
 (await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES('retained-source','credential','retained','00000000-0000-4000-8000-000000000002','{}',?)").run(at));(await f.raw.prepare("INSERT INTO release_seoul_account_exclusions VALUES('alice','unmapped','00000000-0000-4000-8000-000000000003',0,0,?)").run(at));const beforeSources=sources(f),beforeExclusions=rows(f,'release_seoul_account_exclusions'),priorTable=(await f.raw.prepare("SELECT sql FROM sqlite_master WHERE name='release_seoul_capture_attempts'").get()).sql;
 (await f.raw.exec(readFileSync(new URL('../../migrations/0028_seoul-projection-bootstrap-schema.sql',import.meta.url),'utf8')));assert.deepEqual(sources(f),beforeSources);assert.deepEqual(rows(f,'release_seoul_account_exclusions'),beforeExclusions);assert.equal((await f.raw.prepare("SELECT sql FROM sqlite_master WHERE name='release_seoul_capture_attempts'").get()).sql,priorTable);assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,28);
 (await f.raw.exec("INSERT INTO release_seoul_bootstrap_attempts(id,command_kind,receipt_id,email_id,space_id) VALUES('00000000-0000-4000-8000-000000000004','workspace-sign-in','receipt','email','space')"));const retained=rows(f,'release_seoul_bootstrap_attempts');await assert.rejects(async()=>(await f.raw.exec("INSERT INTO release_seoul_bootstrap_attempts(id,command_kind,receipt_id,email_id,space_id) VALUES('00000000-0000-4000-8000-000000000005','workspace-sign-in','receipt2','email2','space2')")),/staging is not empty/);assert.deepEqual(rows(f,'release_seoul_bootstrap_attempts'),retained);assert.deepEqual(sources(f),beforeSources);assert.deepEqual((await f.raw.prepare('PRAGMA foreign_key_check').all()),[]);
});
