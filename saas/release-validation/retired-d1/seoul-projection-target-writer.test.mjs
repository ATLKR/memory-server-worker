...
async function setup(t,recursive='ON',install=true,mode='durable-sql'){
 ...
 assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,33);(await f.raw.exec('PRAGMA recursive_triggers='+recursive));f.calls=[];f.batches=0;f.reads=0;
 if(!install)return f;
 assert.ok(existsSync(schema),'target schema file exists');assert.equal(readFileSync(migration,'utf8'),'-- Forward SaaS migration 34: seoul-projection-target-schema.sql\n'+readFileSync(schema,'utf8'));(await f.raw.exec(readFileSync(migration,'utf8')));assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,34);
 (await f.raw.exec(readFileSync(new URL('../../migrations/0035_seoul-projection-target-manifest-schema.sql',import.meta.url),'utf8')));assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,35);
...
async function clean(f){assert.deepEqual(rows(f,'release_seoul_target_attempt'),[]);assert.deepEqual((await f.raw.prepare('PRAGMA foreign_key_check').all()),[]);assert.equal((await f.raw.prepare('SELECT state FROM release_seoul_projection_state').get()).state,'backfill-required');}
...
test('additive0034 preserves populated0033 rows and every previous schema object',async t=>{
 ...
 assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,34);clean(f);
});
...
for(const recursive of ['ON','OFF'])test('retained operation always precedes historical replay and rejects same/foreign replacements; '+recursive,async t=>{
 ...
 await assert.rejects(async()=>(await f.raw.exec('UPDATE release_seoul_target_attempt SET eligible=eligible')),/immutable/);assert.deepEqual(snapshot(f),before);
});
...
for(const recursive of ['ON','OFF'])test('changed dirty INSERT preserves actual captured advancement after its witness; '+recursive,async t=>{
 const f=await setup(t,recursive);await f.writer.setTarget(command(1));(await f.raw.exec("CREATE TRIGGER advance_capture AFTER INSERT ON release_seoul_target_attempt WHEN NEW.command_id='"+uuid(2)+"' BEGIN UPDATE release_seoul_dirty_spaces SET captured_revision=dirty_revision WHERE space_id='s1'; END"));const result=await f.writer.setTarget(command(2,1,false));assert.equal(result.status,'committed');assert.deepEqual(rows(f,'release_seoul_dirty_spaces'),[{space_id:'s1',dirty_revision:2,captured_revision:1}]);clean(f);
});
...
for(const recursive of ['ON','OFF'])for(const mode of ['unchanged','ineligible'])test(`nonmutating ${mode} completion rejects an in-batch dirty alteration; ${recursive}`,async t=>{
 ...
 (await f.raw.exec("CREATE TRIGGER unexpected_capture AFTER INSERT ON release_seoul_target_attempt WHEN NEW.command_id='"+uuid(2)+"' BEGIN UPDATE release_seoul_dirty_spaces SET captured_revision=dirty_revision WHERE space_id='s1'; END"));assert.deepEqual(await f.writer.setTarget(command(2,1,true)),{status:'uncertain'});assert.deepEqual(snapshot(f),before);clean(f);
});
...
for(const recursive of ['ON','OFF'])test('ineligible orphan race preserves actual command source and rejects an extra scoped source; '+recursive,async t=>{
 const f=await setup(t,recursive);let retained;f.beforeBatch=async ()=>{f.beforeBatch=undefined;(await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(?,'target','s2',?,'{}',?)").run(uuid(1),uuid(90),at));retained=snapshot(f);};assert.deepEqual(await f.writer.setTarget(command(1)),{status:'target_state_invalid'});assert.deepEqual(snapshot(f),retained);clean(f);
 const other=await setup(t,recursive);let before;other.beforeBatch=async ()=>{other.beforeBatch=undefined;(await other.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(?,'target','s2',?,'{}',?)").run(uuid(1),uuid(90),at));before=snapshot(other);};(await other.raw.exec("CREATE TRIGGER unexpected_scoped_source AFTER INSERT ON release_seoul_target_attempt BEGIN INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(NEW.command_id,'target',NEW.space_id,'"+uuid(91)+"','{}',NEW.decided_at); END"));assert.deepEqual(await other.writer.setTarget(command(1)),{status:'uncertain'});assert.deepEqual(snapshot(other),before);clean(other);
});
...
test('wrong prepared hashes fail current planning while immutable historical replay remains valid',async t=>{
 const f=await setup(t);const first=await f.writer.setTarget(command(1)),source=sourceByEvent(f,first.receipt.resultingEventId),candidate=await prepareSeoulHeadCandidate(source);(await f.raw.exec('BEGIN IMMEDIATE'));(await f.raw.prepare('INSERT INTO release_seoul_prepared_sources VALUES(?,?,?,?)').run(1,source.event_id,'f'.repeat(64),'e'.repeat(64)));(await f.raw.prepare("INSERT INTO release_seoul_projection_events(event_id,source_revision,event_kind,stream_kind,stream_key,source_sha256,space_id,snapshot_seq,issued_at,expires_at,payload_bytes,payload_sha256) VALUES(?,1,'head','target','s1',?,NULL,NULL,NULL,NULL,?,?)").run(source.event_id,'f'.repeat(64),candidate.eventText,'e'.repeat(64)));(await f.raw.prepare('INSERT INTO release_seoul_projection_deliveries(event_id) VALUES(?)').run(source.event_id));(await f.raw.prepare('UPDATE release_seoul_authority_heads SET payload_sha256=? WHERE revision=1').run('f'.repeat(64)));(await f.raw.exec('COMMIT'));const before=snapshot(f);assert.deepEqual(await f.writer.setTarget(command(2,1,false)),{status:'target_state_invalid'});assert.deepEqual(await f.writer.setTarget(command(1)),{status:'already_committed',receipt:first.receipt});assert.deepEqual(snapshot(f),before);
});
...
for(const recursive of ['ON','OFF'])test('receipt occupied UUID, immutable input and exact staged-source guards reject direct writes; '+recursive,async t=>{
 ...
 await assert.rejects(async()=>(await f.raw.prepare('INSERT OR REPLACE INTO release_seoul_target_receipts VALUES(?,?,?,?,?,?,?,?,?)').run(...values)),/occupied/);await assert.rejects(async()=>(await f.raw.exec('UPDATE release_seoul_target_receipts SET selected=selected')),/immutable/);await assert.rejects(async()=>(await f.raw.exec('DELETE FROM release_seoul_target_receipts')),/retained/);
 await assert.rejects(async()=>(await f.raw.prepare('INSERT INTO release_seoul_target_receipts VALUES(?,?,?,?,?,?,?,?,?)').run(uuid(2),r.space_id,r.resulting_revision,0,r.operator_reference,'unchanged',r.resulting_revision,r.resulting_event_id,at)),/staged source/);assert.deepEqual(snapshot(f),before);
});

test('native-D1-shaped adapter and maximum input keep bounded indexed ten-statement writes',async t=>{
 const f=await setup(t,'OFF',true,'native-d1'),space='s'.repeat(128);(await f.raw.prepare("INSERT INTO spaces SELECT ?,'Maximum Space',account_id,organization_id,security_mode,created_at,actor_credential_id FROM spaces WHERE id='s1'").run(space));const result=await f.writer.setTarget({...command(1,null,true,space),operatorReference:'o'.repeat(256)});assert.equal(result.status,'committed');const request=f.calls.find(r=>r.statements.length===10);assert.ok(request);assert.ok(request.statements.every(s=>s.values.length<=100&&Buffer.byteLength(s.sql)<=100000));assert.ok(Buffer.byteLength(JSON.stringify({statements:request.statements}))+4096<=1048576);assert.ok(Buffer.byteLength(rows(f,'release_seoul_authority_changes')[0].record_bytes)<1024);
 for(let i=0;i<150;i++)(await f.raw.prepare("INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(?,'target',?,?,'{}',?)").run(uuid(1000+i),'unrelated:'+i,uuid(2000+i),at));
 const before=f.calls.length;await f.writer.setTarget({...command(2,1,true,space),operatorReference:'o'.repeat(256)});const removed=await f.writer.setTarget({...command(3,1,false,space),operatorReference:'o'.repeat(256)});assert.equal(removed.status,'committed');assert.equal((await f.writer.setTarget({...command(4,removed.receipt.resultingRevision,false,space),operatorReference:'o'.repeat(256)})).status,'committed');const relevant=f.calls.slice(before);for(const execution of relevant)for(const statement of execution.statements){const plan=(await f.raw.prepare('EXPLAIN QUERY PLAN '+statement.sql).all(...statement.values)).map(r=>r.detail).join('\n');assert.doesNotMatch(plan,/SCAN (?:c|original|release_seoul_authority_changes|release_seoul_target_receipts)(?:\s|$)/,'writer joins bind indexed identities');}clean(f);
