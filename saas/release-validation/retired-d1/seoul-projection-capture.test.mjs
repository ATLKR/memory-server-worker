...
for(const recursive of ['ON','OFF'])for(const command of ['issue','unlink'])test('captured dirty revision survives later first-capture '+command+'; '+recursive,async t=>{
 const f=await setup(t,{recursive}),space=command==='issue'?'s1':'so',prior=(await f.raw.prepare('SELECT max(revision) n FROM release_seoul_authority_changes').get()).n;
 ...
 const actual=(await f.raw.prepare('SELECT * FROM release_seoul_dirty_spaces WHERE space_id=?').get(space)),latest=Math.max(...captured(f).map(r=>r.revision));assert.ok(latest>prior);assert.equal(actual.dirty_revision,latest);assert.equal(actual.captured_revision,prior);clean(f);
});
const list=async (f,table)=>(await f.raw.prepare('SELECT * FROM '+table+' ORDER BY rowid').all());
...
async function select(f,spaceId,n){(await f.raw.prepare('INSERT INTO release_seoul_authority_changes(source_command_id,stream_kind,stream_key,event_id,record_bytes,created_at) VALUES(?,?,?,?,?,?)').run('target:'+spaceId,'target',spaceId,uuid(n),'{}',at));const revision=(await f.raw.prepare('SELECT max(revision) revision FROM release_seoul_authority_changes').get()).revision;(await f.raw.prepare('INSERT INTO release_seoul_targets VALUES(?,?,?)').run(spaceId,1,revision));}
async function setup(t,{recursive='ON',subjects=['zeta','alpha\nopaque'],selected=['s1','s2','so']}={}){
 ...
 (await f.raw.exec(readFileSync(new URL('../../migrations/0026_seoul-projection-schema.sql',import.meta.url),'utf8')));
 assert.ok(existsSync(source),'inactive source-capture schema must exist');const migration=readFileSync(new URL('../../migrations/0027_seoul-projection-capture-schema.sql',import.meta.url),'utf8');assert.equal(migration,'-- Forward SaaS migration 27: seoul-projection-capture-schema.sql\n'+readFileSync(source,'utf8'));(await f.raw.exec(migration));assert.equal((await f.raw.prepare('SELECT version FROM release_meta').get()).version,27);
 for(const subject of subjects)(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,subject,'alice',at));
 (await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run(issuer,'bob','bob',at));
 selected.forEach((s,i)=>select(f,s,700+i));(await f.raw.exec('PRAGMA recursive_triggers='+recursive));
 const calls=[];
...
for(const recursive of ['ON','OFF'])test('scoped key capture stores final canonical policy atomically with no raw tokens; recursion '+recursive,async t=>{
 ...
 assert.equal(body.tokenDigest,(await f.raw.prepare('SELECT token_digest FROM credentials WHERE id=?').get(key.id)).token_digest);
 ...
 assert.equal((await f.raw.prepare('SELECT state FROM release_seoul_projection_state').get()).state,'backfill-required');
});
...
for(const recursive of ['ON','OFF'])test('self unlink captures exact two-subject email and member/PAT cascades without personal effects; recursion '+recursive,async t=>{
 ...
 const oldKey='old-revoked-key';(await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,revoked_at) VALUES(?,'alice','m1','e1','api_key',?,?,?)").run(oldKey,'d'.repeat(64),at+10000,at-1));
 (await f.raw.prepare('INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES(?,?,NULL)').run(oldKey,'["read","create"]'));
 (await f.raw.exec(`CREATE TRIGGER capture_test_policy_order AFTER INSERT ON revocations BEGIN UPDATE release_credential_policies SET capabilities='["create","read"]' WHERE credential_id='old-revoked-key'; END`));
 ...
 assert.equal((await f.raw.prepare("SELECT revoked_at FROM credentials WHERE id='key:alice'").get()).revoked_at,null);assert.deepEqual(list(f,'release_seoul_dirty_spaces').map(r=>r.space_id),['so']);
...
test('denied key issuance and unauthorized existing-email scope create no source, exclusion or dirty work',async t=>{
 ...
 const prior=f.capture.issueKeyStatements.bind(f.capture);f.capture.issueKeyStatements=async (...args)=>{const statements=prior(...args);(await f.db.setClock(()=>at+1000000));return statements;};
...
test('late capture SQL failure rolls back real key and email commands, source heads, dirty work and staging',async t=>{
 ...
 (await f.raw.exec("CREATE TRIGGER capture_late_failure BEFORE DELETE ON release_seoul_capture_attempts BEGIN SELECT RAISE(ABORT,'capture_late_failure'); END"));
...
for(const mode of ['foreign','unmapped','identity-bytes','fanout'])test('valid unlink preserves central acceptance with explicit bounded exclusion for '+mode,async t=>{
 ...
 const f=await setup(t,{subjects});if(mode==='foreign')(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://retained.example','foreign','alice',at));
 if(mode==='identity-bytes')assert.ok(Buffer.byteLength(JSON.stringify(subjects))>131072);
 if(mode==='fanout')for(let n=0;n<513;n++)(await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at,revoked_at) VALUES(?,'alice','m1','e1','api_key',?,?,?)").run('historic:'+n,n.toString(16).padStart(64,'0'),at+1000,at-1));
 await f.identity.unlinkEmail(f.token,'e1');assert.equal((await f.raw.prepare("SELECT revoked_at FROM account_emails WHERE id='e1'").get()).revoked_at,at);
...
test('foreign retained mapping excludes a successful new key without returning its token in projection state',async t=>{
 const f=await setup(t);(await f.raw.prepare('INSERT INTO provider_identities VALUES(?,?,?,?)').run('https://retained.example','foreign','alice',at));
 const key=await f.admin.issueKey(f.token,{label:'still accepted',capabilities:['read'],expiresInDays:1});assert.ok((await f.raw.prepare('SELECT id FROM credentials WHERE id=?').get(key.id)));
...
for(const capabilities of ['[{"retained":"object"}]','["unknown-capability"]'])test('malformed retained policy excludes the full accepted cascade before canonical JSON: '+capabilities,async t=>{
 const f=await setup(t);const key=await new Admin({DB:f.adapter},()=>at).issueKey(f.token,{label:'retained',organizationId:'org',capabilities:['read'],expiresInDays:1});(await f.raw.prepare('UPDATE release_credential_policies SET capabilities=? WHERE credential_id=?').run(capabilities,key.id));
 await f.identity.unlinkEmail(f.token,'e1');assert.equal((await f.raw.prepare('SELECT revoked_at FROM credentials WHERE id=?').get(key.id)).revoked_at,at);assert.deepEqual(bodies(f).map(r=>r.kind),['subject-source-unrepresentable']);assert.equal(list(f,'release_seoul_account_exclusions')[0].reason,'unsupported-state');assert.equal(list(f,'release_seoul_dirty_spaces').length,0);clean(f);
});

test('a missing retained PAT policy stays explicitly null and never becomes an implicit broad policy',async t=>{
 const f=await setup(t);(await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at) VALUES('missing-policy','alice','m1','e1','api_key',?,?)").run('f'.repeat(64),at+1000));
...
test('raw candidate Space overflow excludes successful central mutation even when almost all candidates are unselected',async t=>{
 const f=await setup(t);for(let n=0;n<513;n++)(await f.raw.prepare("INSERT INTO spaces VALUES(?,?,'alice',NULL,'managed',?,'session:alice')").run('unselected:'+n,'unselected',at));
 const key=await f.admin.issueKey(f.token,{label:'accepted beyond projection scope',capabilities:['read'],expiresInDays:1});assert.ok((await f.raw.prepare('SELECT id FROM credentials WHERE id=?').get(key.id)));assert.equal(list(f,'release_seoul_account_exclusions')[0]?.reason,'fanout-count');assert.deepEqual(bodies(f).map(r=>r.kind),['subject-source-unrepresentable']);assert.equal(list(f,'release_seoul_dirty_spaces').length,0);clean(f);
});

test('oversized retained candidate Space ID excludes before copying even when the Space is unselected',async t=>{
 const f=await setup(t);(await f.raw.prepare("INSERT INTO spaces VALUES(?,?,'alice',NULL,'managed',?,'session:alice')").run('unselected:'+ 'x'.repeat(131072),'retained',at));
 const key=await f.admin.issueKey(f.token,{label:'valid central command',capabilities:['read'],expiresInDays:1});assert.ok((await f.raw.prepare('SELECT id FROM credentials WHERE id=?').get(key.id)));assert.equal(list(f,'release_seoul_account_exclusions')[0]?.reason,'unsupported-state');assert.deepEqual(bodies(f).map(r=>r.kind),['subject-source-unrepresentable']);assert.equal(list(f,'release_seoul_dirty_spaces').length,0);clean(f);
});
...
for(const recursive of ['ON','OFF'])for(const mode of ['newline','2306-byte-key'])test('retained mailbox '+mode+' preserves accepted unlink via exact exclusion; recursion '+recursive,async t=>{
 ...
 (await f.raw.prepare('INSERT INTO account_emails VALUES(?,?,?,?,?,NULL)').run('retained-email','alice',address,'example.com',at));
 (await f.raw.exec("INSERT INTO organizations(id) VALUES('retained-org'); INSERT INTO memberships VALUES('retained-member','retained-org','alice','retained-email','member',9007199254740991,NULL)"));
 (await f.raw.prepare("INSERT INTO credentials(id,account_id,membership_id,email_id,kind,token_digest,expires_at) VALUES('retained-key','alice','retained-member','retained-email','api_key',?,?)").run('e'.repeat(64),at+10000));
 await f.identity.unlinkEmail(f.token,'retained-email');for(const [table,id] of [['account_emails','retained-email'],['memberships','retained-member'],['credentials','retained-key']])assert.equal((await f.raw.prepare('SELECT revoked_at FROM '+table+' WHERE id=?').get(id)).revoked_at,at);
