...
The local decision tool works offline. Remote content calls require a server implementing `memory-routing-v1`, with public `/.well-known/memory-routing` metadata declaring a ready compatible target before content or credentials are sent. General runtime and company-consent endpoints are implemented, but provider routes default disabled and the deployed service must not be presumed ready. The plugin does not create consent records. A configured URL is not evidence of readiness. The plugin fails closed when metadata is missing or incompatible; it does not provision a Seoul database or migrate data. Explicit content-free clear/usage has the maintenance exception described below. See the [SaaS routing design](../../docs/agent-routing.ko.md) for the current product contract.
...
- `memory_usage`: `{ "routing": { "version": 1, "classification": "general" } }`. Read current general Space usage; counts are not actual provider billing.
- `memory_clear_space`: `{ "routing": { "version": 1, "classification": "general" }, "operationId": "stable-clear-id", "scope": "all-general-memory-in-space" }`. Destructive: clears **all general Agent Memory in the configured Space**, including other members' messages. Requires authorization for that entire scope. It does not clear medical profiles or legacy API records. Logical hiding and provider acknowledgement are distinct from verified physical purge. Never interpret an uncertain response as permission to generate a new clear ID.

The new routed calls use `x-memory-routing: 1`. Explicit general clear/usage can attempt content-free maintenance when compatible discovery reports `ready:false`; storage/region matching and server authorization still apply. Ingest/search continue to require `ready:true`. See [general runtime and rollout limits](../../docs/general-runtime.ko.md).

Before every general ingest/search the plugin sends a separate authenticated, metadata-only Space check. A different Space making global discovery ready cannot override the selected Space's Seoul lock or ACL. `issuedAtMs` and `expiresAtMs` yield a maximum 60-second server lifetime; the client starts that lifetime at its own monotonic request-start time, including all transit time, without assuming synchronized PC/server clocks. Medical transfers use their separate current consent receipt. No check is cached for the next call. Direct remote MCP supports initialization and tool listing and exposes `memory_route_check`; invoke it before sending general messages or queries. Use this regional plugin for medical content and consent handling.
