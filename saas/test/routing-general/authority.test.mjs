...
async function orgKey(f, { actor = 'alice', capabilities = ['read', 'create', 'delete'], spaces = ['so'] } = {}) {
  ...
  (await f.db.raw.prepare('INSERT INTO release_credential_policies(credential_id,capabilities,space_ids) VALUES(?,?,?)'))
...
async function shortOrgShare(f, expiresAt) {
  // The real schema validates this initial grant and keeps its expiry immutable.
  (await f.db.raw.prepare(`INSERT INTO release_shares(id,space_id,recipient_email_id,creator_credential_id,expires_at,created_at,creator_membership_id,creator_email_id)
...
test('personal PAT has exact Space and capability restrictions even when the account owns another Space', async t => {
  ...
  (await f.db.raw.prepare(`INSERT INTO spaces(id,owner_account_id,organization_id,deployment_id,data_policy,source_byte_limit,message_limit,name,security_mode,created_at_ms,actor_credential_id)
    VALUES('s3','alice',NULL,'memory-sg',jsonb_build_object('policyVersion',1,'residency','sg','profile','standard','processingBoundary','approved-processors','dataClass','general','classificationStatus','declared','sensitivityTags','[]'::jsonb,'placementEpoch',1),67108864,100000,'Second personal','managed',?,'session:alice')`).run(at));
...
test('organization PAT returns owning organization and minimum membership expiry', async t => {
  ...
  (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m1'").run(at + 30000));
...
test('organization members may read usage and search but only owners or admins may ingest and clear', async t => {
  ...
  (await f.db.raw.prepare("UPDATE memberships SET role='admin' WHERE id='m2'").run());
...
test('read-only interactive sessions cannot ingest or clear a personally owned Space', async t => {
  const f = await setup(t);
  (await f.db.raw.prepare("UPDATE credentials SET permission='read' WHERE id='session:alice'").run());
...
test('materialized OAuth sessions require their verified capability and exact Space policy', async t => {
  ...
  (await f.db.raw.prepare("INSERT INTO credentials(id,account_id,kind,token_digest,expires_at,permission) VALUES('oauth:actor','alice','session',?,?,'write')"))
    .run(await digest(token), at + 120000);
  (await f.db.raw.prepare("INSERT INTO release_credential_policies(credential_id,capabilities,space_ids,verified_oauth) VALUES('oauth:actor','[\"read\"]','[\"so\"]',true)").run());
...
for (const mode of ['credential', 'membership', 'email', 'actor-account', 'owner-organization']) test('organization authority immediately rejects revoked or disabled ' + mode, async t => {
  ...
  (await f.db.raw.prepare(sql).run(at));
...
test('accepted shares preserve actual owner and read-only rights after recipient organization membership ends', async t => {
  ...
  (await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m2'").run(at));
...
test('a longer accepted share remains usable when a shorter recipient membership expires', async t => {
  ...
  (await f.db.raw.prepare("UPDATE memberships SET expires_at=? WHERE id='m2'").run(at + 1000));
...
for (const mode of ['unaccepted', 'revoked', 'expired', 'grantor-membership', 'recipient-email']) test('outside read share rejects ' + mode, async t => {
  ...
  (await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m2'").run(at));
  if (mode === 'revoked') (await f.db.raw.prepare('UPDATE release_shares SET revoked_at=? WHERE id=?').run(at, share.id));
  if (mode === 'expired') f.setNow(at + 30000);
  if (mode === 'grantor-membership') (await f.db.raw.prepare("UPDATE memberships SET revoked_at=? WHERE id='m1'").run(at));
  if (mode === 'recipient-email') (await f.db.raw.prepare("UPDATE account_emails SET revoked_at=? WHERE id='e2'").run(at));
...
test('a suspended owner blocks personal shares even while their account is not terminally disabled', async t => {
  ...
  const share = await transfers.share(f.token, 's1', 'bob@example.com'); await transfers.accept(f.other, share.id);
  (await f.db.raw.prepare('INSERT INTO provider_identities(issuer,subject,account_id,created_at) VALUES(?,?,?,?)').run(issuer, 'alice-central', 'alice', at));
