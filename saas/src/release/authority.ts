...
export function shareGrantorAuthority(share: 'sh' | 'release_shares' = 'sh'): string {
      ...
      WHERE grantor.id=${share}.creator_credential_id AND (
        ...
        OR (s.organization_id IS NOT NULL AND EXISTS(SELECT 1 FROM active_memberships grantor_member
          WHERE grantor_member.id=${share}.creator_membership_id AND grantor_member.email_id=${share}.creator_email_id
            ...
            AND grantor_member.expires_at>? AND grantor_member.role IN ('owner','admin')))))`;
}
...
export function authority(action: Capability): string {
  ...
  AND ((s.organization_id IS NULL AND s.account_id=c.account_id AND c.kind IN ('session','personal_key'))
   OR (s.organization_id IS NOT NULL AND EXISTS(SELECT 1 FROM active_memberships m WHERE m.account_id=c.account_id
      AND m.organization_id=s.organization_id AND m.expires_at>?
   ...
   OR (${cap === 'read' ? '1' : '0'} AND c.kind IN ('session','personal_key') AND EXISTS(
      SELECT 1 FROM release_shares sh JOIN account_emails e ON e.id=sh.recipient_email_id
      WHERE sh.space_id=s.id AND e.account_id=c.account_id AND e.revoked_at IS NULL
       AND sh.accepted_at IS NOT NULL AND sh.revoked_at IS NULL AND sh.expires_at>?
...
export async function requireSpace(db: Database, token: string, spaceId: string, action: Capability, at: number): Promise<Actor> {
    const hash = await tokenHash(token);
    const result = await one<Actor>(db, `SELECT c.id,c.account_id AS accountId,c.kind,c.reauthenticated_at AS reauthenticatedAt FROM spaces s CROSS JOIN active_credentials c WHERE s.id=? AND ${authority(action)}`, [id(spaceId), ...params(hash, at, action)]);
    if (!result)
        return fail(403, 'access_denied');
    return result;
}
export async function interactive(db: Database, token: string, at: number, recent = false): Promise<Actor> {
    const hash = await tokenHash(token);
    const result = await one<Actor>(db, `SELECT c.id,c.account_id AS accountId,c.kind,c.reauthenticated_at AS reauthenticatedAt FROM active_credentials c WHERE c.token_digest=? AND c.expires_at>? AND c.membership_expires_at>? AND ${INTERACTIVE} AND c.permission='write' ${recent ? 'AND c.reauthenticated_at BETWEEN ? AND ?' : ''}`, [hash, at, at, ...(recent ? [at - 300000, at] : [])]);
    if (!result)
        return fail(403, recent ? 'recent_reauthentication_required' : 'interactive_session_required');
    return result;
}
export function recentSql(): string { return `${INTERACTIVE} AND c.permission='write' AND c.reauthenticated_at BETWEEN ? AND ?`; }
