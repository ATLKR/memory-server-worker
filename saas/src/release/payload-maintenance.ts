...
const table = (queue: Queue) => 'memory_ops.payload_' + queue;
const done = (queue: Queue) => queue === 'purges' ? 'purged_at' : 'retired_at';
const unreferenced = (alias: string, history: boolean) => `NOT EXISTS(SELECT 1 FROM memory_content.memories WHERE payload_id=${alias}.payload_id)${history ? ` AND NOT EXISTS(SELECT 1 FROM memory_content.memory_versions WHERE payload_id=${alias}.payload_id)` : ''}`;
...
export class PayloadMaintenance {
    ...
    private async collect(end: number): Promise<number> {
        ...
        const candidates = await within(rows<{ id: string }>(this.db, `SELECT id FROM memory_content.payload_intents
        ...
        const eligible = `i.id IN (SELECT value FROM jsonb_array_elements_text(?::jsonb)) AND i.published_at IS NULL AND i.collection_started_at IS NULL AND i.expires_at<=${sqlNow()}`;
        const result = await within(this.db.batch([
            stmt(this.db, `INSERT INTO memory_ops.payload_purges(payload_id,space_id,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,created_at)
              SELECT p.id,i.space_id,p.memory_id,p.payload_shard_id,p.payload_object_key,p.payload_sha256,p.payload_bytes,${sqlNow()}
              FROM memory_content.payload_intents i JOIN memory_content.payload_stages p ON p.intent_id=i.id WHERE ${eligible} AND p.state IN ('staging','ready')
                AND NOT EXISTS(SELECT 1 FROM memory_content.memories WHERE payload_id=p.id) AND NOT EXISTS(SELECT 1 FROM memory_content.memory_versions WHERE payload_id=p.id)
                AND NOT EXISTS(SELECT 1 FROM memory_ops.payload_purges old WHERE old.payload_id=p.id) RETURNING payload_id`, [at, ids, at]),
            stmt(this.db, `UPDATE memory_content.payload_stages SET state='purge_pending' WHERE state IN ('staging','ready') AND EXISTS(SELECT 1 FROM memory_ops.payload_purges q WHERE q.payload_id=payload_stages.id)
              AND intent_id IN (SELECT i.id FROM memory_content.payload_intents i WHERE ${eligible})`, [ids, at]),
            stmt(this.db, `UPDATE memory_content.payload_intents SET collection_started_at=${sqlNow()} WHERE id IN (SELECT value FROM jsonb_array_elements_text(?::jsonb))
              AND published_at IS NULL AND collection_started_at IS NULL AND expires_at<=${sqlNow()}
              AND NOT EXISTS(SELECT 1 FROM memory_content.payload_stages p WHERE p.intent_id=payload_intents.id AND
                (p.state NOT IN ('purge_pending','purged') OR NOT EXISTS(SELECT 1 FROM memory_ops.payload_purges q WHERE q.payload_id=p.id)))`, [at, ids, at]),
        ]), end);
    ...
    private async pending(queue: Queue, end: number): Promise<CleanupRow[]> {
        return within(rows<CleanupRow>(this.db, `SELECT * FROM ${table(queue)}
    ...
    private async item(queue: Queue, candidate: CleanupRow, result: PayloadMaintenanceResult, end: number): Promise<void> {
        ...
        const statements = [];
        if (queue === 'purges') statements.push(stmt(this.db, `UPDATE memory_content.payload_stages SET state='purge_pending'
          WHERE id=? AND state IN ('staging','ready','published') AND EXISTS(SELECT 1 FROM ${name} q WHERE q.payload_id=payload_stages.id
        ...
        statements.push(stmt(this.db, `UPDATE ${name} SET attempts=attempts+1,available_at=${sqlNow()}+60000
          WHERE payload_id=? AND ${completed} IS NULL AND available_at<=${sqlNow()} AND ${unreferenced(name, queue === 'purges')}
            ${queue === 'purges' ? "AND EXISTS(SELECT 1 FROM memory_content.payload_stages p WHERE p.id=memory_ops.payload_purges.payload_id AND p.state='purge_pending')" : ''} RETURNING *`, [at, candidate.payload_id, at]));
        ...
        if (queue === 'purges') acknowledge.push(stmt(this.db, `UPDATE memory_content.payload_stages SET state='purged'
          WHERE id=? AND state='purge_pending' AND EXISTS(SELECT 1 FROM memory_ops.payload_purges q WHERE q.payload_id=payload_stages.id AND q.purged_at IS NOT NULL AND q.attempts=? AND ${unreferenced('q', true)})`, [row.payload_id, row.attempts]));
