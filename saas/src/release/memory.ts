...
export const columns = `r.id,r.space_id AS "spaceId",r.body,r.source,r.revision,r.created_at AS "createdAt",r.updated_at AS "updatedAt",r.deleted_at AS "deletedAt",r.event_time AS "eventTime",r.kind,r.provenance::text AS provenance,r.supersedes_id AS "supersedesMemoryId",r.erased_at AS "erasedAt",
...
export function erasureStatements(db: Database, memoryId: string, at: number, actorId: string, condition = 'TRUE', values: Value[] = []): Statement[] {
    return [
        stmt(db, `INSERT INTO memory_ops.erasure_permits SELECT r.id,?,${sqlNow()} FROM memory_content.memories r WHERE r.id=? AND r.deleted_at IS NOT NULL AND r.erased_at IS NULL AND (${condition})`, [actorId, at, memoryId, ...values]),
        stmt(db, `INSERT INTO memory_ops.payload_purges(payload_id,space_id,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,created_at)
          SELECT p.id,i.space_id,p.memory_id,p.payload_shard_id,p.payload_object_key,p.payload_sha256,p.payload_bytes,${sqlNow()}
          FROM memory_content.payload_stages p JOIN memory_content.payload_intents i ON i.id=p.intent_id
            JOIN memory_content.memories target ON target.id=p.memory_id AND target.space_id=i.space_id WHERE p.memory_id=?
            AND EXISTS(SELECT 1 FROM memory_ops.erasure_permits WHERE memory_id=?)
            AND NOT EXISTS(SELECT 1 FROM memory_ops.payload_purges old WHERE old.payload_id=p.id)`, [at, memoryId, memoryId]),
        stmt(db, `DELETE FROM memory_content.memory_versions WHERE memory_id=? AND EXISTS(SELECT 1 FROM memory_ops.erasure_permits WHERE memory_id=?)`, [memoryId, memoryId]),
        stmt(db, `UPDATE memory_content.memories SET body='[erased]',source=NULL,provenance='{}'::jsonb,event_time=NULL,payload_id=NULL,payload_shard_id=NULL,payload_object_key=NULL,payload_sha256=NULL,payload_bytes=NULL,logical_bytes=NULL,erased_at=${sqlNow()},updated_at=greatest(updated_at,${sqlNow()}),deleted_at=greatest(updated_at,${sqlNow()}),revision=revision+1,actor_credential_id=? WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.erasure_permits WHERE memory_id=?)`, [at, at, at, actorId, memoryId, memoryId]),
        stmt(db, `INSERT INTO memory_ops.erasure_ledger(memory_id,space_id,erased_at) SELECT id,space_id,erased_at FROM memory_content.memories WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.erasure_permits WHERE memory_id=?)`, [memoryId, memoryId]),
        stmt(db, `INSERT INTO memory_ops.release_events(action,actor_credential_id,resource_id,created_at) SELECT 'memory_erased',actor_credential_id,memory_id,${sqlNow()} FROM memory_ops.erasure_permits WHERE memory_id=?`, [at, memoryId]),
        stmt(db, `DELETE FROM memory_ops.erasure_permits WHERE memory_id=?`, [memoryId])
    ];
}
export class MemoryStore {
    ...
    control: Database;
    constructor(db: Database, clock: () => number = Date.now, payloads?: PayloadBackend, control?: Database) { this.db = db; this.clock = clock; this.payloads = payloads; this.control = control ?? db; }
    /** Cross-tier quota: the pool catalog is control-plane, usage counters are
     * regional. The service boundary pre-check replaces the retired in-database
     * budget trigger; concurrent commits can overrun by one operation each. */
    private async checkQuota(spaceId: string, units: number, bytes: number): Promise<void> {
        if (units <= 0 && bytes <= 0) return;
        const pool = await one<{ id: string }>(this.db, `SELECT pool_id AS id FROM memory_ops.space_pools WHERE space_id=?`, [spaceId]);
        if (!pool) return;
        const limits = await one<{ monthlyUnits: number; storageLimitBytes: number; state: string }>(this.control,
            `SELECT monthly_units AS "monthlyUnits",storage_limit_bytes AS "storageLimitBytes",state FROM memory_control.pools WHERE id=?`, [pool.id]);
        if (!limits) return;
        if (units > 0) {
    ...
    preparedCreate(operationId: string, item: PreparedPayloadItem, input: CreateInput): Statement {
        ...
        return stmt(this.db, `INSERT INTO memory_content.memories(id,space_id,body,source,revision,created_at,updated_at,actor_credential_id,kind,provenance,event_time,supersedes_id,payload_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,logical_bytes)
          SELECT ?,space_id,'[external]',NULL,1,created_at,created_at,actor_credential_id,?,'{}'::jsonb,?,?,?,?,?,?,?,? FROM memory_ops.release_operations WHERE id=?`,
            ...
            const part = await Promise.all(values.slice(offset, offset + 4).map(async row => {
                ...
                try {
                    ...
                    if (input.omitErased && error instanceof ReleaseError && error.code === 'payload_purged'
                        && await one(this.db, `/* payload-erasure-race */ SELECT id FROM memory_content.memories WHERE id=? AND space_id=? AND erased_at IS NOT NULL`, [row.id, row.spaceId]))
                        return null;
                    throw error;
                }
    ...
    async archiveOne(memoryId: string, revision: number, target: 'current' | 'history' = 'current'): Promise<boolean> {
        ...
        if (!this.payloads?.enabled) fail(503, 'payload_storage_unavailable');
        const table = target === 'current' ? 'memory_content.memories' : 'memory_content.memory_versions', key = target === 'current' ? 'id' : 'memory_id';
        ...
        const source = await one<MemoryRow & { accountId: string }>(this.db, `SELECT ${sourceColumns},author.account_id AS "accountId"
          FROM ${table} r JOIN memory_identity.runtime_credentials author ON author.id=r.actor_credential_id
          WHERE r.${key}=? AND r.revision=? AND r.payload_id IS NULL
            AND EXISTS(SELECT 1 FROM memory_content.memories head WHERE head.id=r.${key} AND head.erased_at IS NULL)`, [memoryId, revision]);
        ...
        const find = () => one<{ id: string; requestHash: string; expiresAt: number }>(this.db,
            'SELECT id,request_hash AS "requestHash",expires_at AS "expiresAt" FROM memory_content.payload_intents WHERE account_id=? AND space_id=? AND client_key=?', [source.accountId, source.spaceId, archiveKey]);
        ...
        if (!intent) {
            ...
            await batch(this.db, [
                stmt(this.db, `INSERT INTO memory_content.payload_intents(id,account_id,space_id,client_key,request_hash,action,memory_id,expected_revision,item_count,reserved_bytes,created_at,expires_at)
                  SELECT ?,?,r.space_id,?,?,'archive',r.${key},r.revision,1,?,${sqlNow()},${sqlNow()}+86400000 FROM ${table} r
                  WHERE r.${key}=? AND r.revision=? AND r.payload_id IS NULL AND r.body=? AND r.source IS NOT DISTINCT FROM ? AND r.provenance=?::jsonb
                    AND EXISTS(SELECT 1 FROM memory_content.memories head WHERE head.id=r.${key} AND head.erased_at IS NULL)
                    AND NOT EXISTS(SELECT 1 FROM memory_content.payload_intents old WHERE old.account_id=? AND old.space_id=r.space_id AND old.client_key=?)`,
                ...
                stmt(this.db, `INSERT INTO memory_content.payload_stages(id,intent_id,ordinal,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,logical_bytes,created_at)
                  SELECT ?,id,0,?,?,?,?,?,?,created_at FROM memory_content.payload_intents WHERE id=?`,
        ...
        const stage = await one<{ id: string; shardId: string; objectKey: string; sha256: string; bytes: number; state: string }>(this.db,
            'SELECT id,payload_shard_id AS "shardId",payload_object_key AS "objectKey",payload_sha256 AS sha256,payload_bytes AS bytes,state FROM memory_content.payload_stages WHERE intent_id=? AND ordinal=0', [intent.id]);
        ...
        if (stage.state === 'staging') {
            ...
            await this.db.prepare(`UPDATE memory_content.payload_stages SET state='ready' WHERE id=? AND state='staging'
              AND EXISTS(SELECT 1 FROM memory_content.payload_intents WHERE id=payload_stages.intent_id AND expires_at>${sqlNow()})`).bind(ref.id, this.clock()).run();
        }
        const at = this.clock();
        await batch(this.db, [
            stmt(this.db, `INSERT INTO memory_content.payload_archive_permits(payload_id,memory_id,revision,target,created_at)
              SELECT ?,r.${key},r.revision,?,${sqlNow()} FROM ${table} r
              WHERE r.${key}=? AND r.revision=? AND r.payload_id IS NULL AND r.body=? AND r.source IS NOT DISTINCT FROM ? AND r.provenance=?::jsonb
                AND EXISTS(SELECT 1 FROM memory_content.memories head WHERE head.id=r.${key} AND head.erased_at IS NULL)
                AND EXISTS(SELECT 1 FROM memory_content.payload_stages p JOIN memory_content.payload_intents i ON i.id=p.intent_id WHERE p.id=? AND p.state='ready' AND i.expires_at>${sqlNow()})`,
                [ref.id, target, at, memoryId, revision, source.body, source.source, source.provenance, ref.id, at]),
            stmt(this.db, `UPDATE ${table} SET body='[external]',source=NULL,provenance='{}'::jsonb,payload_id=?,payload_shard_id=?,payload_object_key=?,payload_sha256=?,payload_bytes=?,logical_bytes=?
              WHERE ${key}=? AND revision=? AND EXISTS(SELECT 1 FROM memory_content.payload_archive_permits WHERE payload_id=? AND target=?)`,
                [ref.id, ref.shardId, ref.objectKey, ref.sha256, ref.bytes, bytes, memoryId, revision, ref.id, target]),
            stmt(this.db, `UPDATE memory_content.payload_stages SET state='published' WHERE id=? AND state='ready' AND EXISTS(SELECT 1 FROM memory_content.payload_archives WHERE payload_id=?)`, [ref.id, ref.id]),
            stmt(this.db, `UPDATE memory_content.payload_intents SET published_at=${sqlNow()} WHERE id=? AND published_at IS NULL AND EXISTS(SELECT 1 FROM memory_content.payload_archives WHERE payload_id=?)`, [at, intent.id, ref.id]),
            ...
            ...(target === 'history' ? [stmt(this.db, `INSERT INTO memory_ops.payload_retirements(payload_id,space_id,memory_id,payload_shard_id,payload_object_key,payload_sha256,payload_bytes,created_at)
              SELECT p.id,i.space_id,p.memory_id,p.payload_shard_id,p.payload_object_key,p.payload_sha256,p.payload_bytes,${sqlNow()}
              FROM memory_content.payload_stages p JOIN memory_content.payload_intents i ON i.id=p.intent_id
              WHERE p.id=? AND p.state='published' AND EXISTS(SELECT 1 FROM memory_content.payload_archives a WHERE a.payload_id=p.id AND a.target='history')
                AND NOT EXISTS(SELECT 1 FROM memory_ops.payload_retirements old WHERE old.payload_id=p.id)`, [at, ref.id])] : [])
            // The archive permit is permanent authority history; the
            // payload_archives row already records this consumption.
        ]);
        return !!await one(this.db, 'SELECT payload_id FROM memory_content.payload_archives WHERE payload_id=?', [ref.id]);
    }
    async confirmRead(hash: string, spaceId: string, targets: Pick<Memory, 'id' | 'revision'>[], input: { deleted?: boolean; currentFact?: boolean } = {}): Promise<Map<string, { restoreUntil: number | null; restoreExpired: boolean }>> {
        ...
        const current = await one<{ expiresAt: number; checkedAt: number; memoryIds: string }>(this.db, `/* rest-memory-disclosure */
            WITH authorized AS MATERIALIZED (
                SELECT s.id AS "spaceId",least(c.expires_at,c.membership_expires_at,${accessExpiry('read')}) AS "expiresAt",
                    coalesce((SELECT retention_days FROM memory_ops.space_policies WHERE space_id=s.id),30) AS "retentionDays"
                ...
                SELECT r.id,r.deleted_at+a."retentionDays"*86400000 AS "restoreUntil" FROM jsonb_array_elements(?::jsonb) target
                    JOIN memory_content.memories r ON r.id=target.value->>'id'
                    AND r.revision=(target.value->>'revision')::bigint CROSS JOIN authorized a
                WHERE r.space_id=a."spaceId" AND r.erased_at IS NULL
                    ${input.deleted === undefined ? '' : `AND r.deleted_at IS ${input.deleted ? 'NOT ' : ''}NULL`}
                    ${input.currentFact ? 'AND NOT EXISTS(SELECT 1 FROM memory_content.memories successor WHERE successor.supersedes_id=r.id)' : ''}
            ) SELECT "expiresAt",${SQL_NOW_MS} AS "checkedAt",coalesce((SELECT jsonb_agg(jsonb_build_object('id',id,'restoreUntil',"restoreUntil")) FROM allowed)::text,'[]') AS "memoryIds" FROM authorized`,
    ...
    async get(token: string, spaceId: string, memoryId: string, includeDeleted = false): Promise<Memory> {
        ...
        const row = await one<Omit<Memory, 'provenance'> & {
            provenance: string;
        }>(this.db, `SELECT ${columns} FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id CROSS JOIN memory_identity.active_credentials c WHERE s.id=? AND r.id=? AND r.erased_at IS NULL ${includeDeleted ? '' : 'AND r.deleted_at IS NULL'} AND ${authority('read')}`, [id(spaceId), id(memoryId), ...params(hash, at, 'read')]);
        if (!row) {
    ...
    async commit(token: string, spaceId: string, action: string, cap: Capability, key: string, input: unknown, memoryId: string | null, expectedRevision: number | null, units: number, bytes: number, build: (op: string, actorId: string, at: number) => Statement[], recent = false, additionalCap?: Capability, prepared?: PreparedPayloads): Promise<{
        ...
        const lookup = () => one<{
            ...
            revision: number | null;
        }>(this.db, `SELECT id,request_hash AS "requestHash",memory_id AS "memoryId",committed_revision AS revision FROM memory_ops.release_operations WHERE account_id=? AND space_id=? AND client_key=?`, [actor.accountId, spaceId, key]);
        let existing = await lookup();
        if (!existing && prepared?.committed) fail(503, 'operation_receipt_unavailable');
        if (!existing) {
            await this.checkQuota(spaceId, units, bytes);
      ...
      ${prepared?.intentId ? `AND EXISTS(SELECT 1 FROM memory_content.payload_intents i WHERE i.id=? AND i.account_id=c.account_id AND i.space_id=s.id AND i.client_key=? AND i.request_hash=? AND i.expires_at>${sqlNow()} AND i.published_at IS NULL AND i.item_count=(SELECT count(*) FROM memory_content.payload_stages p WHERE p.intent_id=i.id AND p.state='ready'))` : ''}`, [op, key, requestHash, action, memoryId, expectedRevision, expectedRevision === null ? 1 : expectedRevision + 1, at, at, units, spaceId, ...params(hash, at, cap), ...(additionalCap ? params(hash, at, additionalCap) : []), ...(recent ? [at - 300000, at] : []), ...(prepared?.intentId ? [prepared.intentId, key, requestHash, at] : [])]);
            try {
                ...
                if (!existing) {
                    ...
                    let constraintText = '';
                    for (let cursor: unknown = error; cursor instanceof Error && constraintText.length < 8192; cursor = (cursor as { cause?: unknown }).cause)
                        constraintText += ' ' + cursor.message;
                    if (action === 'create' && error instanceof Error && (constraintText.includes('release_one_successor') || constraintText.includes('memories.supersedes_id')))
                        ...
                        const current = await one<{ expiresAt: number; checkedAt: number; restoreUntil: number | null }>(this.db, `/* restore-denial */
                            WITH authorized AS MATERIALIZED (
                                SELECT s.id AS "spaceId",least(c.expires_at,c.membership_expires_at,${accessExpiry('update')}) AS "expiresAt",
                                    coalesce((SELECT retention_days FROM memory_ops.space_policies WHERE space_id=s.id),30) AS "retentionDays"
                                FROM memory_control.spaces s CROSS JOIN memory_identity.active_credentials c WHERE s.id=? AND ${authority('update')}
                            ) SELECT "expiresAt",${SQL_NOW_MS} AS "checkedAt",(SELECT r.deleted_at+a."retentionDays"*86400000 FROM memory_content.memories r
    ...
    async create(token: string, spaceId: string, input: CreateInput, key: string): Promise<WriteResult> {
        ...
        const bytes = prepared?.items[0]?.logicalBytes ??
            (enc.encode(value.body).length + (value.source === null ? 0 : enc.encode(value.source).length) + enc.encode(canonical(value.provenance)).length);
        // Generated IDs must not participate in the idempotency request digest.
        const result = await this.commit(token, spaceId, 'create', 'create', key, value, null, null, 1, bytes, (op, actor, at) => [
            stmt(this.db, `UPDATE memory_ops.release_operations SET memory_id=? WHERE id=?`, [memoryId, op]),
            prepared?.items[0] ? this.preparedCreate(op, prepared.items[0], value) :
                stmt(this.db, `INSERT INTO memory_content.memories(id,space_id,body,source,revision,created_at,updated_at,actor_credential_id,kind,provenance,event_time,supersedes_id)
    SELECT ?,space_id,?,?,1,created_at,created_at,actor_credential_id,?,?::jsonb,?,? FROM memory_ops.release_operations WHERE id=?`, [memoryId, value.body, value.source, value.kind, canonical(value.provenance), value.eventTime, value.supersedesMemoryId, op])
    ...
    async update(token: string, spaceId: string, memoryId: string, input: {
        ...
        if (this.payloads?.enabled) {
            ...
            const lookup = () => one(this.db, 'SELECT id FROM memory_ops.release_operations WHERE account_id=? AND space_id=? AND client_key=?', [actor.accountId, spaceId, key]);
            ...
            if (!existing) {
                ...
                const old = await one<MemoryRow>(this.db, `SELECT ${columns} FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id CROSS JOIN memory_identity.active_credentials c
        ...
        const updateBytes = prepared?.items[0]?.logicalBytes ??
            (enc.encode(input.body).length + (source === undefined || source === null ? 0 : enc.encode(source).length) + 128);
        const result = await this.commit(token, spaceId, 'update', 'update', key, requestInput, memoryId, input.expectedRevision, 1, updateBytes, (op, actor, at) => {
            const item = prepared?.items[0];
            return [item ? stmt(this.db, `UPDATE memory_content.memories SET body='[external]',source=NULL,provenance='{}'::jsonb,payload_id=?,payload_shard_id=?,payload_object_key=?,payload_sha256=?,payload_bytes=?,logical_bytes=?,revision=revision+1,updated_at=greatest(updated_at,(SELECT created_at FROM memory_ops.release_operations WHERE id=?)),actor_credential_id=? WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.release_operations WHERE id=?)`,
                [item.ref.id, item.ref.shardId, item.ref.objectKey, item.ref.sha256, item.ref.bytes, item.logicalBytes, op, actor, memoryId, op]) :
                stmt(this.db, `UPDATE memory_content.memories SET body=?,source=CASE WHEN ? THEN ? ELSE source END,revision=revision+1,updated_at=greatest(updated_at,(SELECT created_at FROM memory_ops.release_operations WHERE id=?)),actor_credential_id=? WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.release_operations WHERE id=?)`, [input.body, source === undefined ? 0 : 1, source ?? null, op, actor, memoryId, op])];
    ...
    async remove(token: string, spaceId: string, memoryId: string, expectedRevision: number, key: string): Promise<void> {
        ...
        await this.commit(token, spaceId, 'delete', 'delete', key, {}, memoryId, expectedRevision, 0, 0, (op, actor, at) => [
            stmt(this.db, `UPDATE memory_content.memories SET deleted_at=greatest(updated_at,(SELECT created_at FROM memory_ops.release_operations WHERE id=?)),updated_at=greatest(updated_at,(SELECT created_at FROM memory_ops.release_operations WHERE id=?)),revision=revision+1,actor_credential_id=? WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.release_operations WHERE id=?)`, [op, op, actor, memoryId, op])
        ]);
    }
    async restore(token: string, spaceId: string, memoryId: string, expectedRevision: number, key: string): Promise<WriteResult> {
        ...
        if (this.payloads?.enabled) {
            ...
            if (!existing) {
                const at = this.clock();
                const retained = await one<MemoryRow>(this.db, `SELECT ${columns} FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id CROSS JOIN memory_identity.active_credentials c
                ...
                if (retained) {
                    try {
                        ...
                        const current = await one<{ expiresAt: number; restoreUntil: number; checkedAt: number }>(this.db, `/* restore-projection */
                            SELECT least(c.expires_at,c.membership_expires_at,${accessExpiry('update')}) AS "expiresAt",${SQL_NOW_MS} AS "checkedAt",
                                r.deleted_at+coalesce((SELECT retention_days FROM memory_ops.space_policies WHERE space_id=s.id),30)*86400000 AS "restoreUntil"
        ...
        const result = await this.commit(token, spaceId, 'restore', 'update', key, {}, memoryId, expectedRevision, 1, 0, (op, actor, at) => [
            stmt(this.db, `UPDATE memory_content.memories SET deleted_at=NULL,updated_at=greatest(updated_at,(SELECT created_at FROM memory_ops.release_operations WHERE id=?)),revision=revision+1,actor_credential_id=? WHERE id=? AND EXISTS(SELECT 1 FROM memory_ops.release_operations WHERE id=?)`, [op, actor, memoryId, op])
        ]);
        try {
            return { ...await this.get(token, spaceId, memoryId), committedRevision: result.revision, replayed: result.replayed };
        }
    ...
    async erase(token: string, spaceId: string, memoryId: string, expectedRevision: number, confirmation: string, key: string): Promise<{
        ...
        await this.commit(token, spaceId, 'erase', 'delete', key, { confirmation }, memoryId, expectedRevision, 0, 0, (op, actor, at) => erasureStatements(this.db, memoryId, at, actor, 'EXISTS(SELECT 1 FROM memory_ops.release_operations WHERE id=?)', [op]), true);
    ...
    async spaces(token: string, input: {
        ...
        const caller = await one<{ id: string; kind: string; spaceIds: string | null }>(this.db,
            `SELECT c.id,c.kind,p.space_ids::text AS "spaceIds" FROM memory_identity.active_credentials c LEFT JOIN memory_identity.credential_policies p ON p.credential_id=c.id
        ...
        const pageAfter = (alias: string) => `${alias}.created_at_ms>=? AND (${alias}.created_at_ms>? OR (${alias}.created_at_ms=? AND ${alias}.id>?))`;
        const bounds = [time, time, time, after, limit + 1];
        const metadata = 'SELECT s.id,s.name,s.organization_id AS "organizationId",s.security_mode AS "securityMode",s.created_at_ms AS "createdAt"';
        ...
        const candidateSql = scoped ? `${metadata} FROM jsonb_array_elements_text(?::jsonb) selected JOIN memory_control.spaces s ON s.id=selected
                WHERE ${pageAfter('s')} ORDER BY s.created_at_ms,s.id LIMIT ?`
            : `${metadata} FROM (
                SELECT * FROM (SELECT owned.id,owned.created_at_ms AS "createdAt" FROM memory_identity.active_credentials actor JOIN memory_control.spaces owned ON owned.owner_account_id=actor.account_id
                    WHERE actor.token_digest=? AND actor.kind IN ('session','personal_key') AND ${pageAfter('owned')}
                    ORDER BY owned.created_at_ms,owned.id LIMIT ?)
                UNION SELECT * FROM (SELECT managed.id,managed.created_at_ms AS "createdAt" FROM memory_identity.active_credentials actor JOIN memory_identity.active_memberships member ON member.account_id=actor.account_id
                    ...
                    WHERE actor.token_digest=?
                        ...
                        AND ${pageAfter('managed')} ORDER BY managed.created_at_ms,managed.id LIMIT ?)
                UNION SELECT * FROM (SELECT DISTINCT shared.id,shared.created_at_ms AS "createdAt" FROM memory_identity.active_credentials actor JOIN memory_identity.runtime_account_emails recipient ON recipient.account_id=actor.account_id
                    JOIN memory_identity.shares sh ON sh.recipient_email_id=recipient.id JOIN memory_control.spaces shared ON shared.id=sh.space_id
                    ...
                    ORDER BY shared.created_at_ms,shared.id LIMIT ?)
            ) candidate JOIN memory_control.spaces s ON s.id=candidate.id ORDER BY candidate."createdAt",candidate.id LIMIT ?`;
        ...
        const current = await one<{ spaceIds: string; expiresAt: number }>(this.db, `SELECT coalesce((
            SELECT jsonb_agg(jsonb_build_object('id',s.id,'expiresAt',${accessExpiry('read')})) FROM memory_control.spaces s WHERE s.id IN (SELECT value FROM jsonb_array_elements_text(?::jsonb)) AND ${authority('read')}
          )::text,'[]') AS "spaceIds",least(c.expires_at,c.membership_expires_at) AS "expiresAt" FROM memory_identity.active_credentials c WHERE c.token_digest=? AND c.expires_at>${sqlNow()} AND c.membership_expires_at>${sqlNow()}`,
    ...
    async list(token: string, spaceId: string, input: {
        ...
        const all = await rows<Omit<Memory, 'provenance'> & {
            provenance: string;
        }>(this.db, `SELECT ${columns} FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id CROSS JOIN memory_identity.active_credentials c WHERE s.id=? AND r.erased_at IS NULL AND r.deleted_at IS ${input.deleted ? 'NOT ' : ''}NULL
   ${input.deleted ? '' : 'AND NOT EXISTS(SELECT 1 FROM memory_content.memories successor WHERE successor.supersedes_id=r.id)'}
   AND r.created_at<=? AND (r.created_at<? OR (r.created_at=? AND r.id>?)) AND ${authority('read')} ORDER BY r.created_at DESC,r.id LIMIT ?`, [spaceId, before, before, before, after, ...params(hash, at, 'read'), limit + 1]);
    ...
    async retention(token: string, spaceId: string, days: number): Promise<void> {
        ...
        await interactive(this.db, token, this.clock, true);
        const at = this.clock();
        const result = await this.db.prepare(`INSERT INTO memory_ops.space_policies(space_id,retention_days) SELECT s.id,? FROM memory_control.spaces s CROSS JOIN memory_identity.active_credentials c WHERE s.id=? AND ${authority('delete')} AND ${recentSql()} ON CONFLICT(space_id) DO UPDATE SET retention_days=excluded.retention_days`).bind(days, id(spaceId), ...params(hash, at, 'delete'), at - 300000, at).run();
