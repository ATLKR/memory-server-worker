...
const retryableIngest = `EXISTS(SELECT 1 FROM memory_content.release_ingests i WHERE i.id=release_jobs.id
...
export class Jobs {
    ...
    async claim(): Promise<Job | null> {
        ...
        await this.env.DB.prepare(`UPDATE memory_jobs.release_jobs
            ...
            WHERE id IN (SELECT id FROM memory_jobs.release_jobs WHERE state='leased' AND lease_until<=${sqlNow()} AND attempt>=5
                ORDER BY lease_until,id LIMIT 100)`).bind(at, at, at).run();
        await this.env.DB.prepare(`UPDATE memory_jobs.release_jobs SET state='done',lease_token=NULL,lease_until=NULL,last_error=NULL
            WHERE id IN (SELECT candidate.id FROM (
                SELECT id FROM (SELECT id FROM memory_jobs.release_jobs WHERE state='pending' AND kind='ingest' AND cleanup_only=0
                    AND available_at<=${sqlNow()} ORDER BY available_at,id LIMIT 100)
                UNION ALL SELECT id FROM (SELECT id FROM memory_jobs.release_jobs WHERE state='leased' AND kind='ingest' AND cleanup_only=0
                    AND attempt<5 AND lease_until<=${sqlNow()} ORDER BY lease_until,id LIMIT 100)
            ) candidate WHERE NOT EXISTS(SELECT 1 FROM memory_content.release_ingests i WHERE i.id=candidate.id
        ...
        const include = (kind: 'upsert' | 'delete' | 'ingest', cleanupOnly: number) => {
            branches.push(`(SELECT id FROM memory_jobs.release_jobs
        ...
        if (!branches.length)
            return null;
        values.push(at);
        return this.env.DB.prepare(`UPDATE memory_jobs.release_jobs SET state='leased',lease_token=?,lease_until=${sqlNow()}+120000,attempt=attempt+1
    WHERE id=(SELECT id FROM memory_jobs.release_jobs candidate WHERE id IN (SELECT value FROM jsonb_array_elements_text(jsonb_build_array(${branches.join(',')})))
      AND (kind<>'ingest' OR EXISTS(SELECT 1 FROM memory_content.release_ingests i WHERE i.id=candidate.id
        AND i.state='queued' AND i.ciphertext IS NOT NULL AND i.expires_at>${sqlNow()}))
    ...
    async live(job: Job): Promise<boolean> {
        const current = await one<{ leaseUntil: number }>(this.env.DB, `SELECT lease_until AS "leaseUntil" FROM memory_jobs.release_jobs WHERE id=? AND lease_token=? AND state='leased' AND lease_until>${sqlNow()}`, [job.id, job.leaseToken, this.clock()]);
    ...
    async indexable(job: Job): Promise<boolean> {
        ...
        const row = await one<{ eligible: number; suspended: number; leaseUntil: number }>(this.env.DB, `SELECT EXISTS(
            SELECT 1 FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id
            WHERE r.id=j.memory_id AND r.space_id=j.space_id AND r.revision=j.revision
                ...
                AND (s.owner_account_id IS NULL OR EXISTS(SELECT 1 FROM memory_identity.runtime_accounts a WHERE a.id=s.owner_account_id AND ${liveAccountSql('a')}))
                AND (s.organization_id IS NULL OR EXISTS(SELECT 1 FROM memory_control.runtime_organizations o WHERE o.id=s.organization_id AND o.disabled_at IS NULL))
            ) AS eligible,EXISTS(
                SELECT 1 FROM memory_content.memories r JOIN memory_control.spaces s ON s.id=r.space_id JOIN memory_identity.runtime_accounts a ON a.id=s.owner_account_id
                JOIN memory_identity.runtime_provider_identities p ON p.account_id=a.id JOIN memory_ops.lifecycle_applied_state l
            ...
            ) AS suspended,j.lease_until AS "leaseUntil" FROM memory_jobs.release_jobs j WHERE j.id=? AND j.memory_id=? AND j.space_id=? AND j.revision=?
    ...
    async renew(job: Job): Promise<void> {
        ...
        const result = await this.env.DB.prepare(`UPDATE memory_jobs.release_jobs SET lease_until=greatest(lease_until,${sqlNow()}+120000)
        ...
        for (let i = 0; i < limit && this.clock() < stopAt; i++) {
            ...
            try {
                ...
                const result = complete
                    ? await this.env.DB.prepare(`UPDATE memory_jobs.release_jobs SET state='done',available_at=${sqlNow()},lease_token=NULL,lease_until=NULL,last_error=NULL WHERE id=? AND lease_token=? AND state='leased' AND lease_until>${sqlNow()}`).bind(now, job.id, job.leaseToken, now).run()
                    : await this.env.DB.prepare(`UPDATE memory_jobs.release_jobs SET state='pending',attempt=greatest(attempt-1,0),
                        available_at=CASE WHEN jsonb_array_length(cleanup_pending)>0 THEN ${sqlNow()}+5000 ELSE ${sqlNow()}+1 END,
                        lease_token=NULL,lease_until=NULL,
                        last_error=CASE WHEN jsonb_array_length(cleanup_pending)>0 THEN 'vector_delete_pending' ELSE NULL END
    ...
    async index(job: Job, outerStopAt = Number.MAX_SAFE_INTEGER): Promise<boolean> {
        ...
        const progress = await one<{ nextChunk: number; cleanupOnly: number; cleanupCursor: string; cleanupPending: string; cleanupRetryAt: number; cleanupRetryDelay: number }>(db,
            `SELECT next_chunk AS "nextChunk",cleanup_cursor AS "cleanupCursor",cleanup_pending::text AS "cleanupPending",
                cleanup_retry_at AS "cleanupRetryAt",cleanup_retry_delay AS "cleanupRetryDelay",cleanup_only AS "cleanupOnly" FROM memory_jobs.release_jobs
        ...
        const checkpoint = async (minimumRetryDelay = 0) => {
            const result = await db.prepare(`UPDATE memory_jobs.release_jobs SET next_chunk=?,cleanup_cursor=?,cleanup_pending=?,cleanup_retry_at=${minimumRetryDelay ? `greatest(?,${sqlNow()}+?)` : '?'},cleanup_retry_delay=?
                WHERE id=? AND lease_token=? AND state='leased' AND lease_until>${sqlNow()}`)
        ...
        if (current.deletedAt === null && !progress.cleanupOnly) {
            ...
            for (let n = progress.nextChunk; n < chunks.length; n++) {
                ...
                await db.prepare(`INSERT INTO memory_search.vector_refs(memory_id,vector_id,revision) SELECT ?,?,?
                    WHERE EXISTS(SELECT 1 FROM memory_jobs.release_jobs WHERE id=? AND lease_token=? AND state='leased' AND lease_until>${sqlNow()})
            ...
            if (!await this.live(job))
                throw Error('lease_lost');
            if (!await one(db, 'SELECT id FROM memory_content.memories WHERE id=? AND revision=? AND deleted_at IS NULL AND erased_at IS NULL', [current.id, current.revision]))
                return true;
        }
        ...
        while (true) {
            ...
            const refs = await rows<{ vectorId: string }>(db, `SELECT vector_id AS "vectorId" FROM memory_search.vector_refs
        ...
        if (current.erasedAt !== null) {
            const now = this.clock();
            const result = await db.prepare(`UPDATE memory_ops.erasure_ledger SET vector_erased_at=${sqlNow()} WHERE memory_id=?
                AND EXISTS(SELECT 1 FROM memory_jobs.release_jobs j WHERE j.id=? AND j.lease_token=? AND j.state='leased' AND j.lease_until>${sqlNow()})
                AND EXISTS(SELECT 1 FROM memory_content.memories r WHERE r.id=erasure_ledger.memory_id AND r.revision=? AND r.erased_at IS NOT NULL)`)
    ...
    async maintain(): Promise<void> {
        ...
        if (this.env.AUTO_ERASURE_ENABLED === 'true') {
            const progress = await one<{ cursor: string }>(db, "SELECT cursor FROM memory_ops.maintenance_progress WHERE name='source_erasure'", []);
            if (!progress)
            ...
            const candidates = await rows<{
                ...
                actor: string;
                deletedAt: number;
            }>(db, `SELECT r.id,r.actor_credential_id AS actor,r.deleted_at AS "deletedAt" FROM memory_content.memories r
            ...
            for (const row of candidates)
                await batch(db, erasureStatements(db, row.id, at, row.actor,
                    `r.deleted_at+coalesce((SELECT retention_days FROM memory_ops.space_policies WHERE space_id=r.space_id),30)*86400000<=${sqlNow()}
                        AND EXISTS(SELECT 1 FROM memory_ops.maintenance_progress WHERE name='source_erasure' AND cursor=?)`, [at, progress.cursor]));
            const last = candidates.at(-1), next = candidates.length === 20 && last ? JSON.stringify([last.deletedAt, last.id]) : '';
            await db.prepare("UPDATE memory_ops.maintenance_progress SET cursor=? WHERE name='source_erasure' AND cursor=?").bind(next, progress.cursor).run();
        }
        ...
        await db.prepare(`UPDATE memory_content.release_ingests SET ciphertext=NULL,proposals=NULL,state=CASE WHEN state IN ('approved','cancelled') THEN state ELSE 'expired' END
            WHERE id IN (SELECT id FROM memory_content.release_ingests WHERE expires_at<=${sqlNow()}
        ...
        for (const table of ['memory_ops.export_sessions', 'memory_identity.reauth_challenges'])
        ...
        await db.prepare(`DELETE FROM memory_identity.domain_challenges WHERE id IN (
            SELECT id FROM memory_identity.domain_challenges
            WHERE used_at IS NULL AND expires_at<=${sqlNow()} ORDER BY expires_at,id LIMIT 100)`).bind(at).run();
        await db.prepare(`DELETE FROM memory_ops.mail_budget WHERE (account_id,day) IN
            (SELECT account_id,day FROM memory_ops.mail_budget WHERE day<? ORDER BY day,account_id LIMIT 100)`).bind(new Date(at).toISOString().slice(0, 10)).run();
        ...
        if (this.env.BACKGROUND_JOBS_ENABLED === 'true' && this.env.MEMORY_INDEX) {
            const progress = await one<{ cursor: string }>(db, "SELECT cursor FROM memory_ops.maintenance_progress WHERE name='vector_resweep'", []);
            if (!progress)
                throw Error('maintenance_progress_missing');
            const candidates = await rows<{ id: string }>(db, `SELECT id FROM memory_jobs.release_jobs WHERE state='done' AND id>? ORDER BY id LIMIT 100`, [progress.cursor]);
            ...
            await batch(db, [stmt(db, `UPDATE memory_jobs.release_jobs SET state='pending',attempt=0,available_at=${sqlNow()},
                next_chunk=CASE WHEN kind='upsert' THEN next_chunk ELSE 0 END,
                cleanup_cursor='',cleanup_pending='[]'::jsonb,cleanup_retry_at=0,cleanup_retry_delay=60000,
                cleanup_only=CASE WHEN kind='upsert' THEN 1 ELSE 0 END WHERE id IN (
                SELECT j.id FROM memory_jobs.release_jobs j LEFT JOIN memory_ops.erasure_ledger e ON e.memory_id=j.memory_id
                JOIN memory_content.memories m ON m.id=j.memory_id AND m.revision=j.revision
                WHERE j.id IN (SELECT value FROM jsonb_array_elements_text(?::jsonb)) AND j.state='done' AND (
                    (j.kind='delete' AND m.deleted_at IS NOT NULL
                        AND (j.available_at<${sqlNow('?::bigint + 86400000')}-86400000 OR (m.erased_at IS NOT NULL AND e.vector_erased_at IS NULL)))
                    OR (j.kind='upsert' AND m.deleted_at IS NULL AND m.erased_at IS NULL AND j.available_at<${sqlNow('?::bigint + 86400000')}-86400000
                        AND EXISTS(SELECT 1 FROM memory_search.vector_refs v WHERE v.memory_id=m.id AND v.revision<m.revision)))
                ) AND EXISTS(SELECT 1 FROM memory_ops.maintenance_progress WHERE name='vector_resweep' AND cursor=?)`,
                [at, JSON.stringify(candidates.map(row => row.id)), at - 86400000, at - 86400000, progress.cursor]),
                stmt(db, "UPDATE memory_ops.maintenance_progress SET cursor=? WHERE name='vector_resweep' AND cursor=?", [next, progress.cursor])]);
        }
    }
}
