...
import { z } from 'zod';
import { MemoryService, MemoryDenied, MemoryInvalid, MemoryConflict, memoryResponseAuthority } from './memory.ts';
...
import { toolResponse, captureToolResponse } from './mcp-auth.ts';
...
async function result(operation: () => Promise<unknown>, capture: (value: unknown) => void): Promise<CallToolResult> {
  try {
...
export async function handleMcp(request: Request, memory: MemoryService, token: string, title: string, db: IdentityDatabase = memory.db, clock: () => number = memory.clock): Promise<Response> {
  ...
  const capture = (name: string, args: Record<string, unknown>) => (value: unknown) => captureToolResponse(context, name, args, value);
  const handler = createMcpHandler(() => {
    ...
    server.registerTool('memory_spaces', { title: 'List memory Spaces', description: 'List Spaces accessible to this credential.', inputSchema: z.object({}), annotations: read },
      () => result(() => memory.listSpaces(token), capture('memory_spaces', {})));
    server.registerTool('memory_list', { title: 'List memories', description: 'Page through current memories. A cursor continues an earlier page.',
      ...
      args => result(() => memory.list(token, args.spaceId, args), capture('memory_list', args)));
    server.registerTool('memory_search', { title: 'Search memories', description: 'Find current records by literal text substring. Returns bounded snippets; no semantic ranking or LLM call.',
      ...
      args => result(() => memory.search(token, args.spaceId, args), capture('memory_search', args)));
    server.registerTool('memory_get', { title: 'Read memory', description: 'Read a current memory and its revision.', inputSchema: z.object(item), annotations: read },
      args => result(() => memory.get(token, args.spaceId, args.memoryId), capture('memory_get', args)));
    server.registerTool('memory_add', { title: 'Add memory', description: 'Store a new canonical memory. Retries can create duplicates; read the result before retrying.',
      ...
      args => result(() => memory.create(token, args.spaceId, args), capture('memory_add', args)));
    server.registerTool('memory_update', { title: 'Update memory', description: 'Replace a memory only when expectedRevision is still current; preserves prior revision history.',
      ...
      args => result(() => memory.update(token, args.spaceId, args.memoryId, args), capture('memory_update', args)));
    server.registerTool('memory_delete', { title: 'Delete memory', description: 'Remove a memory from retrieval using its current revision. Historical copies remain under the retention policy; this is not permanent erasure.',
      ...
      args => result(async () => { await memory.remove(token, args.spaceId, args.memoryId, args.expectedRevision); return { deleted: true }; }, capture('memory_delete', args)));
    return server;
