...
export class WorkspaceService {
  ...
  async signIn(principal: WorkspacePrincipal, externalToken?: string): Promise<{ token: string; accountId: string; expiresAt: number }> {
    return this.safe(async () => {
      ...
      const at = this.now();
      if (!Number.isSafeInteger(principal.expiresAt) || principal.expiresAt <= at) invalid();
      ...
      await this.write(`INSERT INTO workspace_sign_ins(id,issuer,subject,new_account_id,credential_id,token_digest,
        ...
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`, [crypto.randomUUID(), issuer, subject, crypto.randomUUID(), credentialId,
      ...
      const created = await this.db.withSession('first-primary').prepare(`
        ...
        WHERE c.token_digest=? AND c.kind='session' AND c.expires_at>? AND c.permission=?`)
        .bind(issuer, subject, digest, at, granted).first<{ accountId: string; expiresAt: number }>();
      if (!created) throw new WorkspaceError(401, 'unauthorized');
      return { token, accountId: created.accountId, expiresAt: created.expiresAt };
    });
  }

  async snapshot(token: string): Promise<WorkspaceSnapshot> {
    return this.safe(async () => {
      ...
      const result = await this.db.withSession('first-primary').prepare(`
        SELECT c.account_id AS accountId,
          ...
          (SELECT json_group_array(json_object('id',s.id,'name',s.name,'organizationId',s.organization_id,
            'securityMode',s.security_mode,'canWrite',CASE WHEN c.permission='write'
              AND (s.account_id=c.account_id OR EXISTS(SELECT 1 FROM active_memberships writer
                WHERE writer.account_id=c.account_id AND writer.organization_id=s.organization_id
                  AND writer.expires_at>? AND writer.role IN ('owner','admin'))) THEN 1 ELSE 0 END))
            FROM spaces s WHERE s.account_id=c.account_id OR EXISTS(SELECT 1 FROM active_memberships m
              WHERE m.account_id=c.account_id AND m.organization_id=s.organization_id AND m.expires_at>?)) AS spaces,
          (SELECT json_group_array(json_object('id',m.organization_id,'name',coalesce(o.name,'Organization'),
            'role',m.role,'membershipId',m.id,'parentId',CASE WHEN EXISTS(
              SELECT 1 FROM active_memberships parent WHERE parent.account_id=c.account_id
                AND parent.organization_id=h.parent_organization_id AND parent.expires_at>?)
            ...
            WHERE m.account_id=c.account_id AND m.expires_at>?) AS organizations,
          (SELECT json_group_array(json_object('id',k.id,'label',coalesce(meta.label,'API key'),'kind',k.kind,
            ...
            WHERE k.kind IN ('personal_key','api_key') AND (k.account_id=c.account_id OR EXISTS(
              SELECT 1 FROM active_memberships admin WHERE admin.account_id=c.account_id
                AND admin.organization_id=km.organization_id AND admin.expires_at>? AND admin.role IN ('owner','admin')))) AS keys
        FROM active_credentials c WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}`)
        .bind(at, at, at, at, at, hash, at).first<{ accountId: string; emails: string; spaces: string; organizations: string; keys: string }>();
      if (!result) throw new WorkspaceError(401, 'unauthorized');
      const spaces: (Omit<WorkspaceSnapshot['spaces'][number], 'canWrite'> & { canWrite: number })[] = JSON.parse(result.spaces);
      return { account: { id: result.accountId, emails: JSON.parse(result.emails) },
        spaces: spaces.map(space => ({ ...space, canWrite: space.canWrite === 1 })),
        organizations: JSON.parse(result.organizations), keys: JSON.parse(result.keys) };
    });
  }
    ...
    return this.safe(async () => {
      ...
      if (parentId === null) {
        await this.write(`INSERT INTO workspace_organization_creations(id,name,actor_credential_id,email_id,membership_id,space_id,created_at)
          ...
          WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE} AND e.id=? AND e.revoked_at IS NULL`,
        ...
        await this.write(`INSERT INTO workspace_child_organization_creations
          ...
          WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}
            AND m.organization_id=? AND m.expires_at>? AND m.role IN ('owner','admin')
    ...
    return this.safe(async () => {
      ...
      await this.write(`INSERT INTO workspace_invitation_acceptances(id,invitation_id,actor_credential_id,email_id,created_at)
        ...
        WHERE i.token_digest=? AND i.accepted_at IS NULL AND i.expires_at>?
          AND c.expires_at>? AND ${INTERACTIVE} AND e.revoked_at IS NULL
          AND creator.expires_at>? AND creator.role IN ('owner','admin')
  ...
  async listMembers(token: string, orgId: string): Promise<{ id: string; accountId: string; email: string; role: string; expiresAt: number }[]> {
    return this.safe(async () => {
      ...
      const rows = await this.db.withSession('first-primary').prepare(`
        ...
        LEFT JOIN active_memberships target ON target.organization_id=actor.organization_id AND target.expires_at>?
        LEFT JOIN account_emails e ON e.id=target.email_id AND e.account_id=target.account_id AND e.revoked_at IS NULL
        WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}
          AND actor.organization_id=? AND actor.expires_at>? AND actor.role IN ('owner','admin')
        ...
        .bind(at, hash, at, organizationId, at).all<{
          id: string | null; accountId: string; email: string; role: string; expiresAt: number;
        }>();
      ...
      if (!rows.success || !rows.results.length) throw new WorkspaceError();
      return rows.results.flatMap(row => row.id === null ? [] : [{ ...row, id: row.id }]);
    });
  }
  ...
  async revokeMembership(token: string, orgId: string, membershipId: string): Promise<void> {
    return this.safe(async () => {
      ...
      await this.write(`INSERT INTO workspace_membership_revocations(id,actor_credential_id,organization_id,membership_id,created_at)
        ...
        WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}
          AND actor.expires_at>? AND actor.role IN ('owner','admin') AND target.revoked_at IS NULL
          AND (target.role<>'owner' OR (actor.role='owner' AND EXISTS(SELECT 1 FROM active_memberships other
            WHERE other.organization_id=actor.organization_id AND other.role='owner' AND other.expires_at>? AND other.id<>target.id)))`,
  ...
  async issueKey(token: string, input: { label: string; organizationId?: string; permission: 'read' | 'write'; expiresInDays: number }): Promise<{ id: string; token: string; expiresAt: number }> {
    return this.safe(async () => {
      ...
      const expiresAt = at + input.expiresInDays * DAY;
      const id = crypto.randomUUID(); const proof = randomToken(); const proofHash = await hashToken(proof);
      await this.write(`INSERT INTO workspace_key_issuances(id,actor_credential_id,organization_id,label,permission,token_digest,created_at,expires_at)
        ...
        WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}
          ...
          AND (? IS NULL OR EXISTS(SELECT 1 FROM active_memberships m
            WHERE m.account_id=c.account_id AND m.organization_id=? AND m.expires_at>?
        ...
        WHERE c.token_digest=? AND c.expires_at>? AND ${INTERACTIVE}
          ...
          AND (target.account_id=c.account_id OR EXISTS(SELECT 1 FROM active_memberships actor
            WHERE actor.account_id=c.account_id AND actor.organization_id=target_member.organization_id
              AND actor.expires_at>? AND actor.role IN ('owner','admin')))`,
